...
Deno.serve(async (req) => {
  ...
  try {
    const { action, lang = 'la', calendar = 'general-la', year, month, day } =
    ...
    if (action === 'readings') {
      ...
      if (!response.ok) {
        ...
        return responder.error(502, 'internal_error', {
          message: `Readings API error: ${response.status}`,
    ...
    if (!response.ok) {
      ...
      return responder.error(502, 'internal_error', {
        message: `Erro ao buscar calendário (${response.status})`,
        upstream_status: response.status,
      });
    }
    ...
    return responder.error(500, 'internal_error', { message: 'Erro interno. Tente novamente.' });
  }
});
