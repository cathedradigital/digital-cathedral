...
export function runMobileA11yAudit() {
  ...
  interactiveElements.forEach((el, idx) => {
    ...
    if (rect.width < 48 || rect.height < 48) {
      const label =
        el.getAttribute("aria-label") || el.textContent?.substring(0, 20) || `index ${idx}`;
      mobileIssues.push({
        type: "error",
  ...
  const lowOpacityTexts = document.querySelectorAll(
    ".text-foreground\\/40, .text-primary\\/30, .opacity-30",
  );
