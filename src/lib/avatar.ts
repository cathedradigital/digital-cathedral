...
type AvatarSource = { kind: "external"; url: string } | { kind: "path"; path: string } | null;
...
async function signAvatar(path: string, size: number): Promise<string | null> {
  ...
  const { data, error } = await supabase.storage.from("avatars").createSignedUrl(path, SIGN_TTL_S, {
...
export function useAvatarUrl(value: string | null | undefined, size = 192): string | null {
  ...
  useEffect(() => {
    ...
    if (source.kind === "external") {
      setUrl(source.url);
      lastKey.current = `ext|${source.url}`;
      return;
    }
    ...
    lastKey.current = key;
    let alive = true;
    signAvatar(source.path, size).then((signed) => {
