...
export const contrastConfig: ContrastConfig = {
  ...
  routes: [
    { path: "/" },
    {
      ...
      targets: [
        {
          name: "canonical_abbr value",
          selector: 'dd:has(button[aria-label*="canonical_abbr"]) span.font-mono',
        },
        { name: "bollsId value", selector: 'dd:has(button[aria-label*="bollsId"]) span.font-mono' },
        {
          name: "copy button label",
          selector: 'button[aria-label*="canonical_abbr"] span, button[aria-label*="bollsId"] span',
        },
      ],
    },
    { path: "/glossary" },
...
export function effectiveSelector(
  target: ContrastTarget,
  cfg: ContrastConfig = contrastConfig,
): string {
...
export function getRouteThresholds(
  route: ContrastRoute,
  cfg: ContrastConfig = contrastConfig,
): Required<ContrastRouteThreshold> {
  return {
...
export function describeExclusion(
  ...
  if (denied)
    return `denylist: ${denied.reason ?? denied.selector ?? denied.target ?? "matched rule"}`;
  if (
    cfg.allowlist.length > 0 &&
    !cfg.allowlist.some((r) => ruleMatches(r, route, target.name, target.selector))
  ) {
...
export function resolveRoutes(cfg: ContrastConfig = contrastConfig): ResolvedRoute[] {
  return cfg.routes.map((r) => {
    const merged = r.replaceCommon
      ? (r.targets ?? [])
      : [...cfg.commonTargets, ...(r.targets ?? [])];
    ...
    return {
      ...
      targets,
      excludedTargets,
      thresholds: getRouteThresholds(r, cfg),
    };
  });
}
...
export type ContrastConfigOverride = Partial<
  Pick<
    ContrastConfig,
    "maxNodesPerSelector" | "defaultThresholds" | "denySelectors" | "allowlist" | "denylist"
  >
> & {
...
export function normalizeImportedConfig(input: unknown): ContrastConfigOverride {
  ...
  const src =
    obj.contrast && typeof obj.contrast === "object"
      ? (obj.contrast as Record<string, unknown>)
      : obj;
  ...
  if (Array.isArray(src.denySelectors))
    out.denySelectors = src.denySelectors.filter((s): s is string => typeof s === "string");
...
export function applyConfigOverride(
  override: ContrastConfigOverride,
  cfg: ContrastConfig = contrastConfig,
): void {
  if (override.maxNodesPerSelector !== undefined)
    cfg.maxNodesPerSelector = override.maxNodesPerSelector;
  if (override.defaultThresholds)
    cfg.defaultThresholds = { ...cfg.defaultThresholds, ...override.defaultThresholds };
...
export function loadPersistedConfigOverride(
  cfg: ContrastConfig = contrastConfig,
): ContrastConfigOverride | null {
