...
export const liturgicalCalendarKey = (
  year: number,
  month: number,
  calendar = "general-la",
  lang = "la",
...
export async function cacheLiturgicalMonth(
  ...
  return putInStore(
    "liturgical-calendar",
    liturgicalCalendarKey(year, month, opts.calendar, opts.lang),
    data,
  );
}
...
export async function clearLiturgicalCalendarCache(): Promise<void> {
  try {
    ...
    window.dispatchEvent(
      new CustomEvent("cathedra-litcal-cache-updated", { detail: { stat: "cleared" } }),
    );
  } catch (e) {
...
export async function clearExpiredLiturgicalCalendarEntries(
  ...
  try {
    ...
    if (removed.length > 0) {
      window.dispatchEvent(new CustomEvent("cathedra_cache_updated"));
      window.dispatchEvent(
        new CustomEvent("cathedra-litcal-cache-updated", {
          detail: { stat: "expired-cleared", keys: removed },
        }),
      );
    }
  } catch (e) {
...
export async function deleteLiturgicalCalendarEntry(key: string): Promise<void> {
  try {
    ...
    window.dispatchEvent(new CustomEvent("cathedra_cache_updated"));
    window.dispatchEvent(
      new CustomEvent("cathedra-litcal-cache-updated", { detail: { stat: "entry-deleted", key } }),
    );
  } catch (e) {
...
export async function preloadCatechism(
  start: number,
  count: number,
