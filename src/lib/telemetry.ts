...
class Telemetry {
  ...
  private static checkThresholds() {
    ...
    if (summary.errorRate > thresholds.errorRate) {
      this.triggerAlert(
        ...
        `Atual: ${summary.errorRate.toFixed(1)}% (Limite: ${thresholds.errorRate}%)`,
        "critical",
      );
    }
    ...
    if (summary.avgResponseTime > thresholds.avgLatency) {
      this.triggerAlert(
        ...
        `Atual: ${Math.round(summary.avgResponseTime)}ms (Limite: ${thresholds.avgLatency}ms)`,
        "warning",
      );
    }
    ...
    if (summary.effectTriggers > thresholds.effectTriggers) {
      this.triggerAlert(
        "Excesso de useEffect triggers",
        `Atual: ${summary.effectTriggers}pm (Limite: ${thresholds.effectTriggers}pm)`,
        "warning",
      );
    }
  }
  ...
  private static triggerAlert(title: string, message: string, severity: "warning" | "critical") {
    const recentAlerts = this.events.filter(
      (e) => e.type === "alert" && e.metadata?.title === title && Date.now() - e.timestamp < 300000, // 5 minutos de cooldown
    );
  ...
  static getMetricsSummary() {
    ...
    const errors = minuteEvents.filter(
      (e) => e.type === "error" || e.type === "navigation_error",
    ).length;

    const requestEvents = minuteEvents.filter(
      (e) => e.type === "request" && e.responseTime !== undefined,
    );
