...
export default defineTool({
  ...
  inputSchema: {
    ...
    to: z
      ...
      .max(2865)
      .optional()
      .describe("Fim opcional de intervalo (inclusivo). Máx. 30 parágrafos por chamada."),
  },
  ...
  handler: async ({ paragraph, to }) => {
    ...
    if (end < from)
      return { content: [{ type: "text", text: "'to' deve ser >= 'paragraph'." }], isError: true };
    if (end - from + 1 > 30)
      return {
        content: [{ type: "text", text: "Intervalo máximo: 30 parágrafos." }],
        isError: true,
      };
    ...
    const { data, error } = await sb
      ...
      .select(
        "paragraph,content,texto_base,explicacao,interpretacao_profunda,aplicacao_pratica,reflexao_final",
      )
      .gte("paragraph", from)
      .lte("paragraph", end)
    ...
    if (!data || data.length === 0)
      return {
        content: [
          { type: "text", text: `Nenhum parágrafo encontrado no intervalo ${from}–${end}.` },
        ],
        isError: true,
      };
    return {
