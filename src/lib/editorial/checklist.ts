...
export type EditorialStatus =
  "draft" | "doctrinal_review" | "editorial_review" | "ice_pending" | "published" | "archived";
 ...
 */
export function validateEditorialCompleteness(item: EditorialCandidate): ChecklistViolation[] {
  ...
  if (!item.application || item.application.trim().length < 20) {
    v.push({
      code: "no_application",
      message: "Falta aplicação concreta para as próximas 24h.",
      blocking: true,
    });
  }
  if (!item.prayer || item.prayer.trim().length < 20) {
    v.push({
      code: "no_prayer",
      message: "Falta oração de encerramento (2–4 linhas).",
      blocking: true,
    });
  }
  ...
  if (nexus < MIN_NEXUS) {
    v.push({
      code: "nexus_below_min",
      message: `Nexus insuficiente (${nexus} < ${MIN_NEXUS}).`,
      blocking: true,
    });
  }
  if (nexus > MAX_NEXUS) {
    v.push({
      code: "nexus_above_max",
      message: `Nexus excede o teto (${nexus} > ${MAX_NEXUS}).`,
      blocking: true,
    });
  }
  ...
  if (ENCYCLOPEDIC_OPENERS.some((re) => re.test(firstLine))) {
    v.push({
      code: "encyclopedic_opener",
      message: "Abertura enciclopédica (proibida pelo Voice Guide § 1).",
      blocking: true,
    });
  }
  ...
  for (const term of FORBIDDEN_VOCAB) {
    if (lower.includes(term)) {
      v.push({
        code: "forbidden_vocab",
        message: `Vocabulário proibido: "${term}".`,
        blocking: true,
      });
    }
  }
  ...
  if (/[!]{2,}/.test(body) || /\p{Extended_Pictographic}/u.test(body)) {
    v.push({
      ...
      message: "Emoji ou exclamação múltipla no corpo editorial.",
      blocking: true,
    });
  }
  ...
  if (ice < MIN_ICE_SCORE) {
    v.push({
      code: "ice_below_threshold",
      message: `ICE ${ice} < ${MIN_ICE_SCORE}.`,
      blocking: true,
    });
  }
  ...
  if (item.constitution_version !== CONSTITUTION_VERSION) {
    v.push({
      code: "constitution_version_mismatch",
      message: `constitution_version deve ser "${CONSTITUTION_VERSION}".`,
      blocking: true,
    });
  }
  if (item.voice_version !== VOICE_VERSION) {
    v.push({
      code: "voice_version_mismatch",
      message: `voice_version deve ser "${VOICE_VERSION}".`,
      blocking: true,
    });
  }
  ...
  if (
    item.editorial_author &&
    item.editorial_reviewer &&
    item.editorial_author === item.editorial_reviewer
  ) {
    v.push({
      code: "author_equals_reviewer",
      message: "Autor e revisor devem ser distintos.",
      blocking: true,
    });
  }
