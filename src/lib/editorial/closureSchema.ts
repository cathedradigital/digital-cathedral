...
function normalizeAliases(
  ...
  if (raw.next && typeof raw.next === "object") {
    ...
    const label =
      typeof n.label === "string" ? n.label : typeof n.title === "string" ? n.title : "";
    ...
    if (label && href) {
      ...
      if (!("href" in n) && "url" in n)
        warnings.push("next.url convertido para next.href (alias legado)");
    } else if (n) {
...
export function validateEditorialClosure(raw: unknown): ClosureValidationReport {
  ...
  if (coerced == null) {
    return {
      ok: false,
      data: null,
      warnings: [...warnings, "closure vazio ou irrecuperável"],
      strategy: "none",
    };
  }
