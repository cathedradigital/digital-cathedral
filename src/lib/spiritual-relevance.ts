  ...
  return connections
    .map((item) => {
      ...
      if (context.psychology?.traits) {
        const traitMatches =
          itemTags.filter((t: string) => context.psychology?.traits?.[t]).length || 0;
  ...
  return connections
    .map((item) => {
      ...
      if (context.psychology) {
        ...
        if (context.psychology.traits) {
          const traitMatches = normalized.tags.filter(
            (t) => context.psychology?.traits?.[t],
          ).length;
          if (traitMatches > 0) {
...
export function deduplicateRelatio(connections: TagContent[]): TagContent[] {
  ...
  return connections.filter((item) => {
    ...
    const fingerprint = item.content_text
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "")
      .substring(0, 50);
