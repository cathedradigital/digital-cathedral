...
const CatechismVerification: React.FC = () => {
  ...
  const handleSync = async () => {
    ...
    try {
      const divergentOrMissing = results.filter(
        (r) => r.status === "missing" || r.status === "divergent",
      );
    ...
    <div className="w-full p-spacing-lg space-y-spacing-xl animate-in fade-in duration-500 pb-spacing-3xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-spacing-md">
        <div>
          ...
          <p className="text-premium-sm text-muted-foreground">
            Comparação entre seções do site e banco de dados oficial
          </p>
        </div>
        ...
        <div className="flex items-center gap-spacing-sm">
          {(stats.missing > 0 || stats.divergent > 0) && (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="default"
                  className="rounded-premium-full bg-emerald-600 hover:bg-emerald-700"
                >
              ...
              <AlertDialogContent className="rounded-premium-full border-border bg-card">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Isso irá atualizar o cache local dos parágrafos ausentes ou divergentes com o
                    conteúdo oficial do banco de dados. Isso garante que o usuário veja a versão
          ...
          >
            <Icons.RotateCcw
              className={`w-spacing-md h-spacing-md mr-spacing-xs ${loading ? "animate-spin" : ""}`}
            />
            Recarregar
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-spacing-md">
        ...
        <Card className="p-spacing-md bg-emerald-500/5 border-emerald-500/20">
          <span className="text-premium-xs font-black uppercase tracking-widest text-emerald-600">
            Sincronizados
          </span>
        ...
        <Card className="p-spacing-md bg-destructive/5 border-destructive/20">
          ...
          <div className="text-premium-2xl font-serif font-bold text-destructive">
            {stats.missing}
          </div>
        </Card>
        <Card className="p-spacing-md bg-amber-500/5 border-amber-500/20">
          <span className="text-premium-xs font-black uppercase tracking-widest text-amber-600">
            Divergentes
          </span>
          <div className="text-premium-2xl font-serif font-bold text-amber-600">
            {stats.divergent}
          </div>
        </Card>
      </div>
            ...
            <tbody>
              {loading ? (
                ...
                filteredResults.map((res, i) => (
                  ...
                  >
                    <td className="px-spacing-lg py-spacing-md font-bold font-serif text-primary">
                      §{res.paragraph}
                    </td>
                    <td className="px-spacing-lg py-spacing-md text-premium-xs font-medium text-muted-foreground">
                      {res.section}
                    </td>
                    <td className="px-spacing-lg py-spacing-md">
                      {res.status === "ok" ? (
                        <Badge
                          variant="secondary"
                          className="bg-emerald-500/10 text-emerald-600 border-0 uppercase text-premium-xs font-black"
                        >
                          OK
                        </Badge>
                      ) : res.status === "missing" ? (
                        <Badge
                          variant="destructive"
                          className="uppercase text-premium-xs font-black"
                        >
                          Ausente
                        </Badge>
                      ) : (
                        <Badge className="bg-amber-500 text-white border-0 uppercase text-premium-xs font-black">
                          Divergente
                        </Badge>
                      )}
                    </td>
