...
const CatechismIntegrity: React.FC = () => {
  ...
  const clearCache = async () => {
    if (
      !confirm(
        "Deseja realmente limpar TODO o cache do catecismo? Esta ação é irreversível e exigirá nova geração por IA.",
      )
    )
      return;
    ...
    setLoading(true);
    try {
      const { error } = await supabase.from("catechism_cache").delete().neq("id", "placeholder"); // Deleta tudo
  ...
  const filteredData = data.filter((item) => {
    ...
    if (filter === "error_402")
      return (
        item.status === "error_402" ||
        (item.status === "error" &&
          (item.last_error?.includes("402") || item.last_error?.includes("Créditos")))
      );
    if (filter === "not_cached") return item.status === "not_cached";
    if (filter === "empty")
      return item.status !== "not_cached" && (!item.content || item.content.length < 50);
    return true;
  });

  const stats = {
    error402: data.filter(
      (i) =>
        i.status === "error_402" ||
        (i.status === "error" &&
          (i.last_error?.includes("402") || i.last_error?.includes("Créditos"))),
    ).length,
    empty: data.filter(
      (i) =>
        i.status !== "not_cached" &&
        i.status !== "error_402" &&
        (!i.content || i.content.length < 50),
    ).length,
    notCached: notCachedCount,
    totalIssues: data.filter(
      (i) =>
        i.status === "error_402" ||
        i.status === "error" ||
        i.status === "not_cached" ||
  ...
  return (
    <div className="w-full p-spacing-lg space-y-spacing-xl animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-spacing-md">
        <div>
          <h1 className="text-premium-2xl font-serif font-bold text-foreground">
            Integridade do Conteúdo
          </h1>
          <p className="text-premium-sm text-muted-foreground">
            Parágrafos sem conteúdo ou com erro de créditos
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-spacing-sm">
          ...
          <div className="flex items-center gap-spacing-xs bg-muted/50 p-spacing-2xs rounded-premium border border-border">
            <div className="flex items-center gap-spacing-2xs">
              <span className="text-premium-xs font-bold text-muted-foreground uppercase px-spacing-2xs">
                De:
              </span>
              <input
                type="number"
                ...
                for (const p of missing) {
                  try {
                    ...
                    if (!error && (res?.status === "generated" || res?.status === "official")) {
                      ...
                      res?.status === "error_402" ||
                      (res?.error && res.error.includes("402"))
                    ) {
          ...
          >
            <Icons.RotateCcw
              className={`w-spacing-md h-spacing-md ${loading ? "animate-spin" : ""}`}
            />
          </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-spacing-md">
        <div className="bg-card border border-border rounded-premium p-spacing-md">
          ...
          <div className="text-premium-2xl font-serif font-bold text-foreground">
            {stats.notCached}
          </div>
        </div>
        <div className="bg-card border border-border rounded-premium p-spacing-md">
          <span className="text-premium-xs font-black uppercase tracking-widest text-orange-500">
            Erro 402 (Créditos)
          </span>
          <div className="text-premium-2xl font-serif font-bold text-foreground">
            {stats.error402}
        ...
        <div className="bg-card border border-border rounded-premium p-spacing-md">
          <span className="text-premium-xs font-black uppercase tracking-widest text-destructive">
            Incompletos
          </span>
        ...
        <div className="p-spacing-md border-b border-border bg-muted/30 flex items-center justify-between">
          <div className="flex items-center gap-spacing-xs bg-background/50 border border-border rounded-premium p-spacing-2xs">
            <Button
              ...
              className={`px-spacing-sm py-spacing-2xs text-premium-xs font-black uppercase tracking-widest rounded-premium-full transition-all ${filter === "all" ? "bg-primary text-primary-foreground shadow-premium-md" : "text-muted-foreground hover:text-foreground"}`}
            >
              Todos
            </Button>
            <Button
              onClick={() => setFilter("not_cached")}
              className={`px-spacing-sm py-spacing-2xs text-premium-xs font-black uppercase tracking-widest rounded-premium-full transition-all ${filter === "not_cached" ? "bg-primary text-primary-foreground shadow-premium-md" : "text-muted-foreground hover:text-foreground"}`}
            >
              Não Cacheado
            </Button>
            <Button
              onClick={() => setFilter("error_402")}
              className={`px-spacing-sm py-spacing-2xs text-premium-xs font-black uppercase tracking-widest rounded-premium-full transition-all ${filter === "error_402" ? "bg-primary text-primary-foreground shadow-premium-md" : "text-muted-foreground hover:text-foreground"}`}
            >
              Erro 402
            </Button>
            <Button
              onClick={() => setFilter("empty")}
              className={`px-spacing-sm py-spacing-2xs text-premium-xs font-black uppercase tracking-widest rounded-premium-full transition-all ${filter === "empty" ? "bg-primary text-primary-foreground shadow-premium-md" : "text-muted-foreground hover:text-foreground"}`}
            >
              Incompletos
            </Button>
          </div>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td
                    colSpan={4}
                    className="px-spacing-lg py-spacing-2xl text-center text-muted-foreground animate-pulse"
                  >
                    Carregando dados de integridade...
                  </td>
                </tr>
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="px-spacing-lg py-spacing-2xl text-center text-muted-foreground italic"
                  >
                    Nenhum parágrafo com problemas detectado.
                  </td>
                </tr>
              ) : (
                filteredData.map((item) => (
                  ...
                  >
                    <td className="px-spacing-lg py-spacing-md font-bold text-primary font-serif">
                      §{item.paragraph}
                    </td>
                    <td className="px-spacing-lg py-spacing-md">
                      {item.status === "error_402" ||
                      (item.status === "error" &&
                        (item.last_error?.includes("402") ||
                          item.last_error?.includes("Créditos"))) ? (
                        <span className="text-premium-xs text-orange-600 font-medium">
                          Falta de Créditos IA
                        </span>
                      ) : item.status === "not_cached" ? (
                        <span className="text-premium-xs text-blue-500 font-medium">
                          Nunca Acessado / Sem Cache
                        </span>
                      ) : (
                        <span className="text-premium-xs text-destructive font-medium">
                          Erro na Geração / Incompleto
                        </span>
                      )}
                    </td>
                    <td className="px-spacing-lg py-spacing-md">
