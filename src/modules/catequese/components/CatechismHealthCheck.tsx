...
const CatechismHealthCheck: React.FC = () => {
  ...
  const [results, setResults] = useState<
    { name: string; status: "pass" | "fail" | "pending"; message: string }[]
  >([]);
    ...
    <div className="w-full p-spacing-xl space-y-spacing-xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Verificação automática de tipos e consistência de dados.
          </p>
        </div>
        <Button onClick={runTests} disabled={isRunning} variant="outline" size="sm">
          {isRunning ? (
            <Icons.Loader className="w-spacing-md h-spacing-md animate-spin" />
          ) : (
      ...
      <div className="space-y-spacing-md">
        {results.map((res, i) => (
          <Card
            key={i}
            className="p-spacing-md flex items-center justify-between border-l-4 overflow-hidden"
            style={{ borderLeftColor: res.status === "pass" ? "#10b981" : "#ef4444" }}
          >
            <div className="flex items-center gap-spacing-md">
              <div
                className={`p-spacing-xs rounded-premium-full ${res.status === "pass" ? "bg-emerald-500/10 text-emerald-500" : "bg-destructive/10 text-destructive"}`}
              >
                {res.status === "pass" ? (
            ...
            <Badge
              variant={res.status === "pass" ? "secondary" : "destructive"}
              className="uppercase text-premium-xs font-black tracking-widest"
            >
              {res.status}
            </Badge>
          </Card>
      ...
      <div className="pt-spacing-xl border-t border-border">
        <h2 className="text-premium-sm font-bold uppercase tracking-widest mb-spacing-md">
          Fluxo de Navegação Sugerido
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-spacing-md">
          <Button
            variant="ghost"
            className="h-auto p-spacing-md flex flex-col items-start text-left border border-border"
          ...
          >
            ...
            <span className="text-premium-xs text-muted-foreground mt-spacing-2xs">
              Verifique o botão "Explorar"
            </span>
          </Button>
          <Button
            variant="ghost"
          ...
          >
            ...
            <span className="text-premium-xs text-muted-foreground mt-spacing-2xs">
              Teste busca e tags multi-select
            </span>
          </Button>
          <Button
            variant="ghost"
            className="h-auto p-spacing-md flex flex-col items-start text-left border border-border"
            onClick={() => navigate(`${AppRoute.CATECHISM_EXPLORER}?tags=fe&sort=number-desc`)}
          >
            ...
            <span className="text-premium-xs text-muted-foreground mt-spacing-2xs">
              Recarregue a página e veja se mantém
            </span>
          </Button>
        </div>
      </div>
