...
const Header: React.FC = () => {
  ...
  return (
    <header data-atrium-block="HEADER" className="pt-6 pb-2 flex flex-col gap-2">
      ...
      <p className="text-xs text-muted-foreground">
        {liturgy ? (
          <>
            Hoje · {liturgy.season} · {liturgy.weekday}
        ...
        {user.displayName && (
          <>
            {" "}
            · <span className="text-foreground/80">{user.displayName}</span>
          </>
        )}
      </p>
    </header>
