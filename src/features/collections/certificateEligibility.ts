...
export interface CertificateCriterion {
  id: "complete_all_items" | "register_reading" | "respect_order" | "has_prerequisites";
  label: string;
  met: boolean;
}
...
export interface CertificateStatus {
   ...
   */
  blockingReason: null | "not_eligible" | "no_items" | "items_pending" | "items_locked";
}

export function getItemStatus(progress: ProgressMap, itemId: string): CollectionProgressStatus {
...
export function computeItemLockStates(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const prevCompleted = prev ? getItemStatus(progress, prev.id) === "completed" : true;
    // Bloqueio propaga: se o anterior está bloqueado, este também fica.
    const locked = !!item.is_locked_until_prev && (!prevCompleted || !!prevState?.locked);
    out.push({
      item,
      status,
      locked,
...
export function computeCertificateStatus(
  ...
  const hasPrereqs =
    Array.isArray(collection?.prerequisites) && (collection?.prerequisites?.length ?? 0) > 0;

  const nextActionable = itemStates.find((s) => s.status !== "completed" && !s.locked);
