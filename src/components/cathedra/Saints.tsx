...
const Saints = React.forwardRef<HTMLDivElement, { legacyReader?: boolean }>((props, ref) => {
  ...
  const [viewMode, setViewMode] = useState<
    "daily" | "filtros" | "search" | "all" | "writers" | "popes" | "cloud"
  >("daily");
  ...
  useEffect(() => {
    if (id) {
      try {
        localStorage.setItem("cathedra:saints:last-id", id);
      } catch {
        /* ignore */
      }
  ...
  const handleGlobalSearch = async (query: string) => {
    ...
    try {
      ...
      toast.error("Erro na busca global", {
        description: "Não foi possível encontrar o santo no momento.",
      });
    } finally {
  ...
  const saintsForSelectedDate = useMemo(() => {
    ...
    if (
      ...
      officialSaint.name !== "Menu" &&
      officialSaint.name !== "Santo do Dia"
    ) {
      const officialName = officialSaint.name.toLowerCase();
      const match = dailyList.find(
        (s) =>
        ...
        return dailyList.map((s) =>
          s.id === match.id
            ? {
                ...
                works:
                  (officialSaint.writings || []).length > 0
                    ? (officialSaint.writings || []).map((w: string) => ({ title: w }))
                    : s.works,
              }
            : s,
        );
    ...
    <SpaceLayout>
      ...
      <script type="application/ld+json">
        {JSON.stringify({
          ...
          description:
            "Lista de santos e beatos da Igreja Católica organizados por data litúrgica.",
          publisher: {
            "@type": "Organization",
                ...
                <button
                  ...
                  onKeyDown={(e) =>
                    handleTabKeyDown(
                      ...
                      idx,
                      viewModes.length,
                      (newIdx) => setViewMode(viewModes[newIdx]),
                      "tab-",
                    )
                  }
                >
          ...
          {viewMode === "daily" ? (
            <motion.div
              ...
              {...getTabPanelProps(
                ...
                "tab-daily",
                viewMode === "daily",
                "space-y-spacing-xl outline-none",
              )}
                ...
                */}
                <span className="sr-only" role="status" aria-live="polite" aria-atomic="true">
                  {(() => {
                    ...
                    const dateLabel = format(selectedDate, "d 'de' MMMM 'de' yyyy", {
                      locale: ptBR,
                    });
                    if (isLoadingDaily) {
                ...
                {isDailyError ? (
                  <SaintsOfflineFallback
                    message={
                      dailyError instanceof Error
                        ? dailyError.message
                        : "Erro ao carregar o Santoral. Verifique a conexão com o banco de dados."
                    }
                  ...
                  <>
                    ...
                    {displaySaints.length > 1 && (
                      <section
                        aria-labelledby="tambem-celebrados"
                        className="space-y-spacing-lg pt-spacing-2xl"
                      >
                        <div className="flex items-baseline gap-spacing-md">
                          ...
                          <span className="text-premium-xs font-black uppercase tracking-[0.24em] text-secondary">
                            {displaySaints.length - 1}{" "}
                            {displaySaints.length - 1 === 1 ? "memória" : "memórias"}
                          </span>
                        </div>
                  ...
                  <section className="text-center py-spacing-3xl space-y-spacing-md border-y border-secondary/40">
                    ...
                    <h3 className="font-serif text-premium-2xl text-foreground italic">
                      O céu está repleto de heróis silenciosos.
                    </h3>
                    <p className="text-premium-sm font-serif italic text-muted-foreground max-w-xl mx-auto px-spacing-md">
                      Embora não tenhamos um santo específico catalogado para hoje em nossa base,
                      milhares de almas celebram na glória de Deus.
                    </p>
                  </section>
                )}
              </div>
                  ...
                  <>
                    ...
                    {isSearchingGlobal && (
                      <div className="text-center py-spacing-3xl space-y-spacing-md">
                        ...
                        <p className="text-premium-sm text-muted-foreground animate-pulse">
                          Consultando hagiografias históricas...
                        </p>
                      </div>
                    )}
                  </>
            ...
            >
              ...
              <div
                className="flex flex-wrap justify-center gap-spacing-sm max-w-spacing-4xl mx-auto"
                role="list"
              >
                {displaySaints.map((saint, i) => (
                  <div key={saint.id} role="listitem">
                    <BubbleTag
                      label={saint.name}
                      emoji={
                        saint.category === "pope" ? "👑" : saint.category === "doctor" ? "📖" : "⛪"
                      }
                      index={i}
                      onClick={() => handleOpenSaint(saint, false)}
...
const SaintCard: React.FC<{
  saint: SaintWithScore;
  onClick: () => void;
  onMouseEnter?: () => void;
}> = ({ saint, onClick, onMouseEnter }) => {
  return (
    <Button
      onClick={onClick}
    ...
    >
      ...
      <div className="flex-1 p-spacing-lg space-y-spacing-md">
        <div>
          <div className="flex items-center justify-between mb-spacing-xs">
            ...
            {saint.works && saint.works.length > 0 && (
              <div
                className="p-spacing-2xs bg-primary/5 rounded-premium text-primary"
                title="Possui obras escritas"
              >
                <Icons.BookOpen className="w-spacing-sm h-spacing-sm" />
              </div>
            )}
          </div>
          <h3 className="text-premium-xl font-serif font-bold text-foreground group-hover:text-primary transition-colors line-clamp-spacing-2xs">
            {saint.name}
          </h3>
        ...
        <p className="text-premium-xs text-muted-foreground line-clamp-spacing-xs leading-relaxed">
          {saint.bio}
        </p>
        ...
        <div className="flex flex-wrap gap-spacing-2xs mt-auto">
          {saint.virtues?.slice(0, 2).map((v) => (
            <span
              key={v}
              className="px-spacing-xs py-spacing-3xs bg-primary/5 text-primary text-premium-xs font-black uppercase rounded-premium-full border border-primary/10"
            >
              {v}
            </span>
          ))}
