...
const UserTransactionsPage: React.FC = () => {
    ...
    async (pageNum: number) => {
      ...
      try {
        ...
        if (searchTerm) {
          ...
          if (isNumeric) {
            query = query.or(
              `payment_id.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,amount.eq.${searchTerm}`,
            );
          } else {
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(
      `Total Aprovado: ${new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(total)}`,
      14,
      finalY + 10,
    );
      ...
      {error && transactions.length === 0 ? (
        ...
        <div className="grid gap-spacing-md">
              ...
              <Card className="overflow-hidden border-border/50 hover:border-primary/30 transition-all">
                <CardContent className="p-spacing-0">
                  <div className="p-spacing-md md:p-spacing-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-spacing-md">
                    <div className="flex items-center gap-spacing-md">
                      <div
                        className={`w-spacing-xl h-spacing-xl rounded-premium-full flex items-center justify-center ${tx.is_donation ? "bg-secondary/10 text-secondary" : "bg-primary/10 text-primary"}`}
                      >
                        {tx.is_donation ? (
                          <Icons.Heart className="w-spacing-md h-spacing-md fill-current" />
                        ) : (
                          <Icons.Star className="w-spacing-md h-spacing-md fill-current" />
                        )}
                      </div>
                      <div>
                        <p className="text-premium-sm font-bold text-foreground">
                          {tx.description ||
                            (tx.is_donation ? "Doação Voluntária" : "Assinatura PRO")}
                        </p>
                        <p className="text-premium-xs text-muted-foreground">
                          {format(new Date(tx.created_at), "dd 'de' MMMM 'de' yyyy, HH:mm", {
                            locale: ptBR,
                          })}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between w-full md:w-auto gap-spacing-lg">
                      <div className="text-right">
                        ...
                        <p className="text-premium-xs text-muted-foreground uppercase font-bold tracking-tighter">
                          Valor
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-spacing-2xs">
                        ...
                        {tx.payment_id && (
                          <p className="text-premium-xs font-mono text-muted-foreground">
                            ID: {tx.payment_id}
                          </p>
                        )}
                      </div>
          ...
          {hasMore && !loadingMore && !error && (
            <div
              ref={loaderRef}
              className="flex flex-col items-center justify-center py-spacing-xl space-y-spacing-md"
            >
        ...
        >
          <DialogHeader>
            <DialogTitle className="font-serif text-premium-xl flex items-center gap-spacing-sm">
              <div
                className={`w-spacing-xl h-spacing-xl rounded-premium-full flex items-center justify-center ${selectedTx?.is_donation ? "bg-secondary/10 text-secondary" : "bg-primary/10 text-primary"}`}
              >
                {selectedTx?.is_donation ? (
                  <Icons.Heart className="w-spacing-md h-spacing-md fill-current" />
                ) : (
                  <Icons.Star className="w-spacing-md h-spacing-md fill-current" />
                )}
            ...
            <div className="space-y-spacing-lg pt-spacing-md">
              ...
              <div className="space-y-spacing-sm bg-muted/30 p-spacing-md rounded-premium border border-border/50">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-premium-xs font-bold text-foreground">
                    {selectedTx.description ||
                      (selectedTx.is_donation ? "Doação Voluntária" : "Assinatura PRO")}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-premium-xs font-bold text-muted-foreground uppercase">
                ...
                {selectedTx.payment_id && (
                  <div className="flex justify-between items-center group">
                    ...
                    <div className="flex items-center gap-spacing-xs">
                      <span className="text-premium-xs font-mono text-foreground">
                        {selectedTx.payment_id}
                      </span>
                      <Button
                        variant="ghost"
                ...
                {selectedTx.coupon_code && (
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant="outline" className="text-premium-xs font-mono">
                      {selectedTx.coupon_code}
                    </Badge>
                  </div>
                )}
              ...
              {selectedTx.status === "approved" && (
                <div className="space-y-spacing-md">
                  <div className="p-spacing-md rounded-premium bg-green-500/5 border border-green-500/10 flex items-center gap-spacing-sm">
                    ...
                    <div>
                      <p className="text-premium-xs font-black uppercase text-green-600">
                        Aprovado
                      </p>
                      <p className="text-premium-xs text-green-600/80">
                        Sua contribuição já está ajudando nossa missão!
                      </p>
                    </div>
                  </div>
              ...
              {selectedTx.error_message && (
                <div className="p-spacing-md rounded-premium bg-destructive/5 border border-destructive/10">
                  <p className="text-premium-xs font-black uppercase text-destructive mb-spacing-2xs">
                    Motivo do Problema
                  </p>
                  <p className="text-premium-xs text-destructive/80 italic">
                    {selectedTx.error_message}
                  </p>
                </div>
              )}
