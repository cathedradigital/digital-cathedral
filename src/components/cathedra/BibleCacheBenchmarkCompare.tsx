...
function pctDelta(
  after: number,
  before: number,
): { value: number; display: string; dir: "down" | "up" | "flat" } {
...
function hitRate(summary: SummaryRow[]): { rate: number; totalN: number } {
  ...
  const hits = summary
    .filter((r) => r.cache_type === "L1_HIT_FRESH" || r.cache_type === "L1_HIT_STALE")
    .reduce((a, r) => a + r.n, 0);
...
function DeltaCell({
  ...
  delta: ReturnType<typeof pctDelta>;
  betterDown: boolean;
}) {
  ...
  return (
    <span className={`${cls} inline-flex items-center gap-1 font-medium`}>
...
export default function BibleCacheBenchmarkCompare() {
  ...
  const rows = useMemo(() => {
    ...
    return types.map((t) => {
      const b = before.summary.find((s) => s.cache_type === t) ?? {
        cache_type: t,
        n: 0,
        sql_avg_ms: 0,
        sql_p95_ms: 0,
      ...
      const a = after.summary.find((s) => s.cache_type === t) ?? {
        cache_type: t,
        n: 0,
        sql_avg_ms: 0,
        sql_p95_ms: 0,
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-sm flex items-center gap-2">
          <GitCompareArrows className="h-4 w-4 text-secondary" /> Comparação lado a lado de
          benchmarks
        </CardTitle>
        <CardDescription className="text-xs">
          Capture snapshots em momentos diferentes (antes/depois de uma mudança de TTL, deploy,
          etc.) e compare deltas % de p95, avg e cache_hit_rate. Snapshots ficam neste navegador
          (localStorage, máx {MAX_SNAPSHOTS}).
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {snaps.length < 2 ? (
          <p className="text-xs text-muted-foreground">
            {snaps.length === 0 ? "Nenhum snapshot ainda." : "1 snapshot salvo."} Capture pelo menos
            2 para comparar.
          </p>
        ) : (
          <>
            ...
            {before && after && (
              <>
                  ...
                  <div className="p-3 rounded-md border bg-muted/30 flex flex-wrap gap-4 items-center text-sm">
                    <div>
                      ...
                      <div className="font-mono">
                        {hitRateRow.hb.rate.toFixed(1)}% →{" "}
                        <span className="font-bold">{hitRateRow.ha.rate.toFixed(1)}%</span>
                      </div>
                    </div>
                    <div>
                      <DeltaCell delta={hitRateRow.delta} betterDown={false} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      n: {hitRateRow.hb.totalN.toLocaleString("pt-BR")} →{" "}
                      {hitRateRow.ha.totalN.toLocaleString("pt-BR")}
                    </div>
                  </div>
                )}
                      ...
                      {rows.map((r) => (
                        <TableRow key={r.type}>
                          ...
                          <TableCell className="text-right font-mono">
                            {r.b.total_avg_ms} → <strong>{r.a.total_avg_ms}</strong>
                          </TableCell>
                          <TableCell className="text-right">
                            <DeltaCell delta={r.d_avg} betterDown />
                          </TableCell>
                          <TableCell className="text-right font-mono">
                            {r.b.total_p95_ms} → <strong>{r.a.total_p95_ms}</strong>
                          </TableCell>
                          <TableCell className="text-right">
                            <DeltaCell delta={r.d_p95} betterDown />
                          </TableCell>
                          <TableCell className="text-right font-mono">
                            {r.b.sql_p95_ms} → <strong>{r.a.sql_p95_ms}</strong>
                          </TableCell>
                          <TableCell className="text-right">
                            <DeltaCell delta={r.d_sql_p95} betterDown />
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Verde = melhora (latência caiu ou hit_rate subiu). Vermelho = regressão. STALE com
                  n&lt;30 marcado como baixa confiança.
                </p>
              </>
            )}
          </>
        ...
        {snaps.length > 0 && (
          <details className="text-xs">
            <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
              Gerenciar snapshots ({snaps.length})
            </summary>
              ...
              {snaps.map((s) => (
                ...
                >
                  <span className="font-mono">
                    {s.label} · {s.since_days}d · {s.summary.reduce((a, r) => a + r.n, 0)} req
                  </span>
