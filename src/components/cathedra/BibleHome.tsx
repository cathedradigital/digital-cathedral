...
export const BibleHome: React.FC<BibleHomeProps> = ({
  onSelectBook,
  searchQuery,
  setSearchQuery,
}) => {
  ...
  const lastRead = React.useMemo(
    () => (lastReadRaw ? JSON.parse(lastReadRaw) : null),
    [lastReadRaw],
  );
      ...
      <div className="hidden md:flex md:w-[40%] sticky top-0 h-screen overflow-hidden bg-primary/5 border-r border-primary/5">
        ...
        <div className="relative z-10 w-full flex flex-col items-center justify-center p-spacing-2xl text-center space-y-spacing-xl">
          ...
          <div className="space-y-spacing-xs">
            <h2 className="font-display text-5xl text-primary tracking-tight leading-none italic">
              Sacra Scriptura
            </h2>
          ...
          <div className="max-w-xs mx-auto">
            ...
            <p className="text-sm font-serif italic text-primary/60 leading-relaxed">
              "Toda a Escritura é inspirada por Deus e útil para ensinar, para repreender, para
              corrigir e para formar na justiça."
            </p>
            <p className="mt-2 text-[10px] uppercase tracking-widest text-primary/40 font-bold">
              2 Timóteo 3, 16
            </p>
          </div>
        </div>
        ...
        <div className="max-w-lg mx-auto px-4 space-y-8 pb-24">
            ...
            {lastRead ? (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <h2 className="text-[9px] font-black uppercase tracking-widest text-secondary/80">
                    Bíblia • Continuar Lendo
                  </h2>
                  <div className="flex items-center gap-1.5">
                    {lastRead.timestamp && (
                      <span className="text-[8px] text-primary/30 font-bold uppercase">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <button
                key={item.label}
                onClick={item.action}
                className="flex flex-col items-center p-4 rounded-2xl border border-primary/5 bg-background hover:bg-primary/[0.02] transition-all group"
              >
                <item.icon className="w-5 h-5 text-primary/30 group-hover:text-secondary transition-colors mb-2" />
                <span className="text-[10px] font-bold uppercase tracking-tight text-primary/40 text-center">
                  {item.label}
                </span>
              </button>
            ))}
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 px-2">
              ...
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-primary/30">
                Cânone Sagrado
              </h2>
              <div className="h-px flex-1 bg-primary/5" />
            </div>
