...
const AdminCrmSegmentation: React.FC<Props> = ({
  users,
  onSelectUser,
  totalUsers,
  page,
  ...
  const segments: { key: Segment; label: string; icon: React.ReactNode; color: string }[] = [
    ...
    },
    {
      key: "new",
      label: "Novo",
      icon: <Icons.UserCheck className="w-spacing-md h-spacing-md" />,
      color: "text-primary",
    },
    {
      key: "active",
    ...
    },
    {
      key: "deep",
      label: "Profundo",
      icon: <Icons.Crown className="w-spacing-md h-spacing-md" />,
      color: "text-primary",
    },
    {
      key: "inactive",
  ...
  const filtered = segmentedUsers[segment].sort((a, b) => {
    const valA = a[sortBy] ?? "";
    const valB = b[sortBy] ?? "";
  ...
  const getStatusBadge = (u: UserProfile) => {
    ...
    switch (status) {
      case "Profundo":
        return (
          <Badge variant="default" className="text-premium-xs">
            Profundo
          </Badge>
        );
      case "Engajado":
      ...
      case "Ativo":
        return (
          <Badge variant="outline" className="border-primary/30 text-primary text-premium-xs">
            Ativo
          </Badge>
        );
      case "Novo":
        return (
          <Badge variant="outline" className="border-secondary/30 text-secondary text-premium-xs">
            Novo
          </Badge>
        );
      default:
        return (
          <Badge variant="destructive" className="text-premium-xs">
            Inativo
          </Badge>
        );
    }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-spacing-xs">
        {segments.map((s) => (
          ...
          >
            ...
            <p className="text-premium-xl font-bold mt-spacing-2xs">
              {segmentedUsers[s.key].length}
            </p>
          </Button>
        ))}
      </div>
        ...
        <CardHeader className="pb-spacing-sm">
          <div className="flex items-center justify-between">
            <CardTitle className="text-premium-sm">
              {segments.find((s) => s.key === segment)?.label} — {filtered.length} usuário
              {filtered.length !== 1 ? "s" : ""}
            </CardTitle>
            ...
            <table className="w-full text-premium-sm">
              <thead className="sticky top-spacing-0 bg-card z-10">
                <tr className="border-b border-border">
                  ...
                  <th className="text-center p-spacing-sm font-semibold hidden md:table-cell">
                    Reflexões
                  </th>
                  <th className="text-center p-spacing-sm font-semibold hidden lg:table-cell">
                    Jornada
                  </th>
                  <th className="text-center p-spacing-sm font-semibold hidden md:table-cell">
                    Última Ativ.
              ...
              <tbody>
                {filtered.map((u) => (
                  ...
                  >
                    <td className="p-spacing-sm">
                      <div className="flex items-center gap-spacing-xs">
                        ...
                        <div className="min-w-spacing-0">
                          ...
                          <p className="text-premium-small text-muted-foreground truncate">
                            {u.email}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="p-spacing-sm text-center hidden md:table-cell">
                      <span className="text-premium-xs font-medium">
                    ...
                    <td className="p-spacing-sm text-center hidden md:table-cell text-premium-xs text-muted-foreground">
                      {u.last_visit
                        ? hoursSince(u.last_visit) < 24
                          ? "Hoje"
                          : `${Math.floor(hoursSince(u.last_visit) / 24)}d atrás`
                        : "—"}
                    </td>
                    <td className="p-spacing-sm text-center">
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-spacing-lg px-spacing-xs text-premium-xs gap-spacing-2xs"
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td
                      colSpan={7}
                      data-test="estado-vazio-1"
                      className="p-spacing-xl text-center text-muted-foreground"
                    >
