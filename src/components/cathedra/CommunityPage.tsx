...
const CommunityPage: React.FC = () => {
  ...
  const { results: rawSearchResults, isPending: isSearchPending } = useFuzzySearch<Post>({
  ...
  const fetchPosts = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      setPosts(enriched.filter((p) => p.status === "approved" || p.user_id === user?.id));
    }
    setLoading(false);
  }, [category, user, tab]);
  ...
  const openPost = async (post: Post) => {
    ...
    if (data) {
      ...
      const { data: profiles } = (await supabase
        .from("public_profiles" as any)
        .select("id, name")
        .in("id", userIds.length ? userIds : [""])) as {
      ...
      <div className="w-full space-y-spacing-lg py-spacing-xl px-spacing-md">
        ...
        <div className="bg-card border border-border rounded-premium p-spacing-lg space-y-spacing-md">
          ...
          <div className="flex items-center gap-spacing-md pt-spacing-xs border-t border-border">
            ...
            >
              <Icons.Heart
                className={`w-spacing-md h-spacing-md ${selectedPost.user_liked ? "fill-primary text-primary" : ""}`}
              />
        ...
        <div className="space-y-spacing-sm">
          {replies.map((r) => (
            <div
              key={r.id}
              className="bg-card border border-border rounded-premium p-spacing-md ml-spacing-lg"
            >
        ...
        {user ? (
          ...
          <Button
            ...
            className="w-full py-spacing-sm bg-foreground text-background rounded-premium-full text-premium-sm font-bold hover:bg-primary hover:text-primary-foreground transition-all"
          >
  ...
  return (
    <ContemplativeLayout>
      <div className="desktop-main px-spacing-md">
          ...
          <>
            <div className="text-center space-y-spacing-md pt-spacing-md mb-spacing-xl">
              <div className="inline-flex items-center gap-spacing-xs px-spacing-md py-spacing-2xs bg-primary/5 rounded-premium border border-primary/10 shadow-premium-md mb-spacing-xs">
                <Icons.Message
                  className="w-spacing-md h-spacing-md text-primary"
                  aria-hidden="true"
                />
                <span className="text-premium-small font-black uppercase tracking-[0.3em] text-primary">
                  Communitas Fidelium
                </span>
              </div>
              <h1 className="text-premium-4xl md:text-premium-7xl font-black tracking-tighter text-foreground bg-gradient-to-b from-foreground to-foreground/70 bg-clip-text text-transparent leading-[0.9]">
                Comunidade
              </h1>
              <p className="text-muted-foreground font-serif italic max-w-spacing-2xl mx-auto text-premium-base sm:text-premium-xl leading-relaxed">
                Discussões, testemunhos e partilhas entre irmãos na fé.
              </p>
              <p className="text-premium-xs text-muted-foreground uppercase tracking-widest opacity-60">
                Conteúdo moderado para edificação mútua
              </p>
            </div>
          </>
        )}
        ...
        <div
          className="flex gap-spacing-xs justify-center mb-spacing-xl"
          role="tablist"
        ...
        >
          <Button
            {...getTabProps(
              ...
              "panel-forum",
              tab === "forum",
              `px-spacing-md py-spacing-xs rounded-premium-full text-premium-xs font-black uppercase tracking-widest transition-all focus-visible:ring-2 focus-visible:ring-primary outline-none ${
                tab === "forum"
            ...
            onKeyDown={(e) =>
              handleTabKeyDown(e, 0, 2, (idx) => setTab(idx === 0 ? "forum" : "ranking"), "tab-")
            }
          >
            <Icons.Message className="w-spacing-sm h-spacing-sm inline mr-spacing-2xs" />
            Fórum
          </Button>
          <Button
            {...getTabProps(
            ...
            onKeyDown={(e) =>
              handleTabKeyDown(e, 1, 2, (idx) => setTab(idx === 0 ? "forum" : "ranking"), "tab-")
            }
          >
            <Icons.Star className="w-spacing-sm h-spacing-sm inline mr-spacing-2xs" />
            Ranking
          </Button>
        </div>
          ...
          <div className="space-y-spacing-lg" {...getTabPanelProps("panel-forum", "tab-0", true)}>
            <div className="flex flex-col sm:flex-row gap-spacing-md mb-spacing-lg">
              ...
              <Button
                ...
                className="rounded-premium-full h-spacing-2xl px-spacing-lg font-black uppercase tracking-widest gap-spacing-xs bg-primary shadow-premium shadow-primary/20"
              >
              ...
              {posts.map((post) => (
                ...
                >
                  <CardContent className="p-spacing-lg">
                    ...
                    <h3 className="text-premium-lg font-bold text-foreground group-hover:text-primary transition-colors mb-spacing-xs">
                      {post.title}
                    </h3>
                    <p className="text-premium-sm text-muted-foreground line-clamp-spacing-xs italic mb-spacing-md">
                      {post.content}
                    </p>
                    <div className="flex items-center gap-spacing-md pt-spacing-md border-t border-border/40">
                      ...
                      <div className="flex items-center gap-spacing-2xs text-premium-xs font-bold text-muted-foreground">
                        <Icons.MessageSquare className="w-spacing-sm h-spacing-sm" />{" "}
            ...
            {lbLoading ? (
              ...
              <div className="grid grid-cols-1 gap-spacing-sm">
                {leaderboard.map((entry, idx) => (
                  <div
                    key={entry.id}
                    className="flex items-center gap-spacing-md p-spacing-md bg-card border border-border/50 rounded-[2rem]"
                  >
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-premium-xs font-black uppercase text-muted-foreground">
                        Pontos
                      </p>
                    </div>
                  </div>
