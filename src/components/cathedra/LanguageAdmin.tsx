...
const LanguageAdmin: React.FC = () => {
  ...
  const fetchAllowlist = async () => {
    setIsLoading(true);
    const { data, error } = await supabase.from("language_allowlist").select("*").order("term");
  ...
  const handleDelete = async (id: string) => {
    const { error } = await supabase.from("language_allowlist").delete().eq("id", id);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold text-primary uppercase tracking-widest">
            Gestão de Linguagem
          </h1>
          <p className="text-sm text-muted-foreground italic">
            Gerencie exceções para a auditoria de vernáculo português.
          </p>
        </div>
      ...
      <div className="bg-card border border-border rounded-3xl p-6 shadow-sm space-y-6">
        ...
        <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Button
            type="submit"
            className="h-full rounded-xl uppercase font-black tracking-widest text-[10px]"
          >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-2">
          ...
          <span className="text-[10px] font-bold text-primary/20">
            {allowlist.length} registros
          </span>
        </div>
