...
import {
  getSpiritualContext,
  rankConnectionsIntelligently,
  deduplicateRelatio,
  SpiritualContext,
} from "@/lib/spiritual-relevance";
...
const Relatio: React.FC<RelatioProps> = ({
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      ...
      try {
        ...
        const tagPromises = context.tags
          .slice(0, tagCount)
          .map((tag) => fetchNexusTagContent({ label: tag, slug: tag.toLowerCase() }));
        ...
        const filtered = all.filter((item) => {
          ...
          if (item.type === "catechism" && (relatioConfig as any).showCatechism === false)
            return false;
          if (item.type === "magisterium" && (relatioConfig as any).showMagisterium === false)
            return false;
        ...
        if ((relatioConfig as any).relevanceByProgress && spiritualContext) {
          ...
          ranked = unique.map((item) => ({
            ...item,
            reason: item.metadata?.is_theme_content
              ? "Tema Relacionado"
              : item.metadata?.tags?.some((t: string) => context.tags?.includes(t))
                ? "Contexto Similar"
                : "Tradição Conectada",
          }));
        }
  ...
  const updateDensity = (newDensity: "subtle" | "normal" | "deep") => {
    ...
    window.dispatchEvent(
      new CustomEvent("cathedra-relatio-density-changed", { detail: newDensity }),
    );
    toast.info(
      `Densidade: ${newDensity === "subtle" ? "Subtil" : newDensity === "normal" ? "Normal" : "Profunda"}`,
    );
  };
  ...
  return (
    <section
      ref={containerRef}
      className={cn(
        "mt-spacing-xl pt-spacing-xl md:mt-spacing-2xl md:pt-spacing-2xl border-t border-primary/[0.01] space-y-spacing-md md:space-y-spacing-lg mb-spacing-xl md:mb-spacing-2xl w-full",
        className,
      )}
      aria-labelledby="relatio-heading"
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-spacing-sm">
          ...
          <div>
            <h3
              id="relatio-heading"
              className="text-[9px] font-black uppercase tracking-[0.5em] text-primary/80 group-hover:text-primary transition-all duration-1000"
            >
              Relatio Contextual
            </h3>
          </div>
        ...
        <div className="flex items-center gap-spacing-md">
          <Button
            ...
            title={
              autoCollapse ? "Recolhimento automático ativado" : "Ativar recolhimento automático"
            }
          >
            ...
            <LayoutGroup id="relatio-cards">
                            ...
                            onClick={() => {
                              if (item.type === "bible") {
                                ...
                                const p =
                                  ...
                                  parseInt(item.title.match(/\d+/)?.[0] || "0");
                          ...
                          >
                            ...
                            <div className="flex flex-col h-full relative z-10">
                              <div className="flex items-start justify-between mb-spacing-md">
                                <div className="flex items-center gap-spacing-sm">
                                  <div className="w-spacing-xl h-spacing-xl rounded-premium bg-primary/[0.03] border border-primary/[0.05] flex-shrink-0 flex items-center justify-center text-primary/40 group-hover:text-primary group-hover:bg-primary/[0.05] transition-all duration-700">
                                    {item.type === "bible" && (
                                      <Icons.Bible
                                        className="w-spacing-md h-spacing-md"
                                        strokeWidth={1}
                                      />
                                    )}
                                    {item.type === "catechism" && (
                                      <Icons.CatechismShield
                                        className="w-spacing-md h-spacing-md"
                                        strokeWidth={1}
                                      />
                                    )}
                                    {item.type === "magisterium" && (
                                      <Icons.Magisterium
                                        className="w-spacing-md h-spacing-md"
                                        strokeWidth={1}
                                      />
                                    )}
                                    {(item.type === "journey" || item.type === "saint") && (
                                      <Icons.Compass
                                        className="w-spacing-md h-spacing-md"
                                        strokeWidth={1}
                                      />
                                  ...
                                  <div className="space-y-spacing-3xs">
                                    ...
                                    <div className="flex items-center gap-spacing-xs">
                                      ...
                                      <div
                                        className={cn(
                                          "w-spacing-2xs h-spacing-2xs rounded-premium-full",
                                          strength > 20
                                      ...
                                      <span
                                        className={cn(
                                          ...
                                          strength > 20 ? "text-secondary" : "text-primary/80",
                                        )}
                                      >
                                        {strengthLabel}
                                      </span>
                                ...
                                <div className="flex items-center gap-spacing-2xs opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                  ...
                                  <Button
                                    ...
                                    aria-label={
                                      isFav
                                        ...
                                        : `Salvar ${item.title} nos favoritos`
                                    }
                                    ...
                                    onClick={(e) => {
                                      ...
                                      toast.success(
                                        isFav ? "Removido dos favoritos" : "Conexão salva",
                                        {
                                          description: item.title,
                                          icon: (
                                            <Icons.Star className="w-spacing-md h-spacing-md fill-secondary text-secondary" />
                                          ),
                                        },
                                      );
                                    }}
                                  >
                                    <Icons.Star
                                      className={cn(
                                        "w-spacing-sm h-spacing-sm",
                                        isFav ? "fill-secondary text-secondary" : "text-primary/60",
                                      )}
                                      strokeWidth={1.5}
                                    />
                                  </Button>
                              ...
                              <div className="flex-1 space-y-spacing-xs">
                                <h4 className="text-premium-base font-bold font-serif text-primary/80 group-hover:text-primary transition-colors duration-500">
                                  {item.title}
                                </h4>
                              ...
                              <div className="mt-spacing-md pt-spacing-md border-t border-primary/[0.02] flex items-center justify-between opacity-0 group-hover:opacity-100 transition-all duration-700 translate-y-1 group-hover:translate-y-0">
                                ...
                                <Icons.ArrowRight
                                  className="w-spacing-sm h-spacing-sm text-primary/60"
                                  strokeWidth={1.5}
                                />
                              </div>
                            </div>
                  ...
                  {loading && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-spacing-md mt-spacing-md">
                      {[1, 2].map((i) => (
                        <div
                          key={`skeleton-${i}`}
                          className="h-[180px] w-full rounded-premium-lg bg-primary/[0.02] animate-pulse overflow-hidden relative"
                        >
              ...
              {connections.length > 4 && (
                <div className="flex flex-col items-center gap-spacing-md pt-spacing-md">
                  ...
                  >
                    <span className="mr-spacing-xs">
                      {showAll
                        ? "Recolher Conexões"
                        : `Ver mais ${connections.length - 4} conexões`}
                    </span>
                    <Icons.ChevronDown
                      className={cn(
                        "w-spacing-sm h-spacing-sm transition-transform duration-500",
                        showAll && "rotate-180",
                      )}
                    />
                  </Button>
