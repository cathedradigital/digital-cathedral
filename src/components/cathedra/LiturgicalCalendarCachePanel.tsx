...
const LiturgicalCalendarCachePanel: React.FC<Props> = ({ meta, onAfterClear }) => {
  ...
  const storageLevel: "ok" | "warn" | "danger" =
    storage.bytes >= STORAGE_DANGER_BYTES
      ? "danger"
      : storage.bytes >= STORAGE_WARN_BYTES
        ? "warn"
        : "ok";
    ...
    >
      ...
      <dl className="grid grid-cols-2 gap-spacing-sm text-premium-xs">
        <div>
          ...
          <dd data-testid="litcal-cache-hits" className="font-mono font-bold text-foreground">
            {stats.hits}
          </dd>
        </div>
        <div>
          <dt className="text-muted-foreground uppercase tracking-wider text-[10px]">Misses</dt>
          <dd data-testid="litcal-cache-misses" className="font-mono font-bold text-foreground">
            {stats.misses}
          </dd>
        </div>
        <div>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-spacing-xs mt-spacing-3xs">
          ...
          {storageLevel !== "ok" && (
            <span
              data-testid="litcal-cache-storage-warning"
      ...
      <div className="flex flex-wrap gap-spacing-xs pt-spacing-xs border-t border-border">
        ...
        >
          <Icons.Download
            className={`w-spacing-xs h-spacing-xs ${isExporting ? "animate-pulse" : ""}`}
          />
        ...
        {confirmingClearExpired ? (
          ...
          >
            <Icons.Cross
              className={`w-spacing-xs h-spacing-xs ${isClearingExpired ? "animate-spin" : ""}`}
            />
            {isClearingExpired
              ? "Removendo…"
        ...
        >
          <Icons.Cross
            className={`w-spacing-xs h-spacing-xs ${isClearing ? "animate-spin" : ""}`}
          />
