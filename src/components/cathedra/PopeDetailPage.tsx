...
import { useParams, Navigate } from "@/lib/rr-compat";
...
const GET_POPE_BY_ID = (id: string) => {
  const popes: Record<string, any> = {
    ...
    "benedict-xvi": {
      ...
      image:
        "https://images.unsplash.com/photo-1548625361-195feee1c4ce?q=80&w=2000&auto=format&fit=crop",
  ...
  return (
    <div className="flex flex-col md:flex-row w-full min-h-screen bg-background">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <ReaderShell
          ariaLabel={pope.name}
          hero={<EditorialHero title={pope.name} subtitle={pope.title} align="left" />}
        ...
        >
          <div className="space-y-spacing-2xl">
            <article className="prose prose-stone max-w-none">
              <p className="font-reader text-lg leading-relaxed text-foreground/90">{pope.bio}</p>
            </article>
          </div>
        </ReaderShell>
      </div>
