...
const A11yAuditPage: React.FC = () => {
  const [issues, setIssues] = useState<
    Array<{ message: string; type: "error" | "warning" | "info"; selector?: string }>
  >([]);
      ...
      format === "json"
        ...
        : "Tipo,Mensagem,Seletor,Conformidade\n" +
          issues
            .map(
              (i) =>
                `${i.type},"${i.message}","${i.selector || ""}",${i.type === "error" ? "Falha" : "Aviso"}`,
            )
            .join("\n");
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-premium-sm">
              Verificação automática de WCAG AA e áreas de toque.
            </p>
          </div>
            ...
            {issues.length > 0 ? (
              <div className="space-y-spacing-md">
                {issues.map((issue, i) => (
                  <div
                    key={i}
                    className="flex flex-col gap-spacing-xs p-spacing-md bg-muted/20 rounded-premium-md border border-border/10 group"
                  >
