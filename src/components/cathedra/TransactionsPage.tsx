...
const TransactionsPage: React.FC = () => {
  ...
  const lastAuditElementRef = useCallback(
  ...
  const logExport = async (
    ...
    if (!user) return;
    try {
      const filtersToLog = customFilters || {
        status: statusFilter,
        plan: planFilter,
        start: startDate,
        end: endDate,
  ...
  const exportToCSV = async (mode: "current" | "all" = "current", customFilters?: any) => {
    ...
    if (
      ...
      const confirmAll = window.confirm(
        "Você está tentando exportar TODAS as transações sem nenhum filtro. Isso pode demorar e gerar um arquivo muito grande. Deseja continuar?",
      );
      ...
      if (mode === "current" && !customFilters) {
        ...
        if (filters.search?.trim()) {
          if (filters.search.includes("@"))
            countQuery = countQuery.filter("profiles.email", "ilike", `%${filters.search}%`);
        ...
        if (filters.start)
          countQuery = countQuery.gte(
            "created_at",
            startOfDay(parseISO(filters.start)).toISOString(),
          );
        if (filters.end)
          countQuery = countQuery.lte("created_at", endOfDay(parseISO(filters.end)).toISOString());
        ...
        while (hasMore) {
          ...
          if (filters.search?.trim()) {
            if (filters.search.includes("@"))
              q = q.filter("profiles.email", "ilike", `%${filters.search}%`);
            else q = q.filter("profiles.name", "ilike", `%${filters.search}%`);
          }
          if (filters.status !== "all") q = q.eq("status", filters.status);
          if (filters.plan !== "all") q = q.eq("plan_id", filters.plan);
          if (filters.start)
            q = q.gte("created_at", startOfDay(parseISO(filters.start)).toISOString());
  ...
  const filteredJSON = useMemo(() => {
    ...
    const fullObj = {
      ...
      error: selectedTx.error_message
        ? typeof selectedTx.error_message === "string"
          ? JSON.parse(selectedTx.error_message)
      ...
      switch (s) {
        case "approved":
          return {
            ...
            icon: <Icons.CheckCircle2 className="w-spacing-sm h-spacing-sm mr-spacing-2xs" />,
          };
        case "pending":
          return {
            label: "Pendente",
            color: "bg-yellow-500/10 text-yellow-500",
            icon: <Icons.Clock className="w-spacing-sm h-spacing-sm mr-spacing-2xs" />,
          };
        case "rejected":
          return {
            label: "Recusado",
            color: "bg-red-500/10 text-red-500",
            icon: <Icons.XCircle className="w-spacing-sm h-spacing-sm mr-spacing-2xs" />,
          };
        default:
          return {
            label: s,
            color: "bg-muted text-muted-foreground",
            icon: <Icons.Info className="w-spacing-sm h-spacing-sm mr-spacing-2xs" />,
          };
      }
    };
    const { label, color, icon } = info(status);
    return (
      <Badge className={`${color} flex items-center py-spacing-2xs font-medium border-0`}>
        {icon}
        {label}
      </Badge>
    );
    ...
    <div className="max-w-6xl mx-auto space-y-spacing-xl py-spacing-xl px-spacing-md animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-spacing-md">
        <div className="space-y-spacing-xs">
          ...
          <p className="text-muted-foreground italic font-serif text-premium-sm">
            Controle financeiro e auditoria de pagamentos.
          </p>
        </div>
        ...
        <div className="flex items-center gap-spacing-sm">
          {isAdmin && (
            <>
              ...
              <Button
                ...
                className="rounded-premium-full gap-spacing-xs text-destructive border-destructive/20 hover:bg-destructive/5"
              >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                {exporting ? (
                  <Icons.Clock className="w-spacing-md h-spacing-md animate-spin" />
                ) : (
        ...
        <Card className="bg-primary/5 border-primary/10 rounded-premium p-spacing-lg space-y-spacing-sm">
          <div className="flex justify-between items-end">
            <div className="space-y-spacing-2xs">
              <p className="text-premium-xs font-bold uppercase tracking-widest text-primary">
                Buscando registros...
              </p>
              <p className="text-premium-xs text-muted-foreground italic">
                {exportProgress} de {totalToExport} transações carregadas.
              </p>
            </div>
            <div className="flex items-center gap-spacing-md">
              <Button
                ...
                className="h-spacing-xl text-premium-xs text-destructive hover:bg-destructive/10"
              >
                Cancelar Exportação
              </Button>
              <p className="text-premium-xl font-bold text-primary">
                {Math.round((exportProgress / totalToExport) * 100)}%
              </p>
            </div>
          </div>
      ...
      <Card className="rounded-[2.5rem] border-border/50 shadow-premium-hover overflow-hidden">
        <CardHeader className="bg-muted/30 border-b border-border/50 p-spacing-xl">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-spacing-md items-end">
            {isAdmin && (
              <div className="space-y-spacing-2xs">
                <label className="text-premium-xs font-bold uppercase tracking-widest text-muted-foreground">
                  Usuário
                </label>
                <Input
                  ...
                  disabled={loading || exporting}
                  className="rounded-premium-full h-spacing-xl bg-background/50"
                />
              </div>
            )}
            <div className="space-y-spacing-2xs">
              ...
              <Select
                ...
                disabled={loading || exporting}
              >
                <SelectTrigger className="rounded-premium-full h-spacing-xl bg-background/50">
                  <SelectValue />
                </SelectTrigger>
          ...
          {dateError && (
            <p className="text-premium-xs text-destructive mt-spacing-sm font-bold">{dateError}</p>
          )}
        </CardHeader>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell
                    colSpan={6}
                    className="h-spacing-4xl text-center italic font-serif text-muted-foreground"
                  >
                ...
                <TableRow>
                  <TableCell colSpan={6} className="h-spacing-4xl text-center">
                    <div className="flex flex-col items-center gap-spacing-sm">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="text-premium-xs uppercase font-bold"
                        onClick={() => {
                          setStatusFilter("all");
                          setPlanFilter("all");
                          setUserSearch("");
                  ...
                  <TableRow key={tx.id} className="group hover:bg-primary/5 transition-colors">
                    <TableCell className="py-spacing-md px-spacing-xl flex flex-col">
                      <span className="font-bold text-premium-sm">
                        {format(new Date(tx.created_at), "dd 'de' MMM", { locale: ptBR })}
                      </span>
                    ...
                    {isAdmin && (
                      <TableCell className="max-w-[150px] truncate">
                        <div className="flex flex-col">
                          <span className="font-bold text-premium-xs">
                            {tx.profiles?.name || "---"}
                          </span>
                          <span className="text-premium-xs text-muted-foreground">
                            {tx.profiles?.email}
                    ...
                    <TableCell className="font-serif italic text-premium-sm">
                      {tx.description}
                    </TableCell>
                    ...
                    <TableCell className="text-center flex justify-center py-spacing-md">
                      {getStatusBadge(tx.status)}
                    </TableCell>
                    <TableCell className="text-right px-spacing-xl">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => {
      ...
      <div className="flex items-center justify-center gap-spacing-lg mt-spacing-md">
        <Button
          ...
          className="rounded-premium-full px-spacing-lg shadow-premium-md"
        >
          Anterior
        </Button>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-spacing-3xl rounded-[2.5rem] bg-background ">
          <DialogHeader>
            <DialogTitle className="text-premium-2xl font-serif font-bold">
              Preview da Exportação
            </DialogTitle>
            <DialogDescription>Confira os dados antes de baixar o arquivo CSV.</DialogDescription>
          </DialogHeader>
          <div className="py-spacing-md overflow-x-auto">
            <Table className="border rounded-premium-full">
              <TableHeader className="bg-muted/50">
                <TableRow>
              ...
              <TableBody>
                {previewData.slice(0, 5).map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell className="text-premium-xs font-mono">
                      {new Date(tx.created_at).toISOString().split("T")[1].substring(0, 8)}
                    </TableCell>
                    <TableCell className="text-premium-xs truncate max-w-[120px]">
                      {tx.profiles?.email}
                    </TableCell>
                    <TableCell className="text-premium-xs text-right font-bold">
                      {tx.amount}
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <p className="text-premium-xs text-muted-foreground text-center">
            Mostrando as 5 primeiras de {previewData.length} transações.
          </p>
          <DialogFooter className="gap-spacing-sm">
            ...
            <Button
              onClick={() => executeDownload(previewData, previewMode)}
              className="rounded-premium-full bg-primary flex-1 font-bold"
            >
      ...
      <Dialog open={isCleanupOpen} onOpenChange={setIsCleanupOpen}>
        <DialogContent className="max-w-spacing-md rounded-[2.5rem] bg-background ">
          <DialogHeader>
            <DialogTitle className="text-premium-2xl font-serif font-bold text-destructive">
              Limpar Registros
            </DialogTitle>
            <DialogDescription>Ação irreversível de exclusão de dados.</DialogDescription>
          </DialogHeader>
          <div className="py-spacing-md space-y-spacing-md">
            <div className="p-spacing-md bg-destructive/5 rounded-premium border border-destructive/10 text-premium-xs text-destructive font-bold">
              Cuidado! Você apagará as transações de {startDate || "---"} até {endDate || "---"}.
            </div>
            <div className="space-y-spacing-xs">
              <label className="text-premium-xs font-bold uppercase tracking-widest text-muted-foreground">
                Digite 'CONFIRMAR' para liberar
              </label>
              <Input
                value={cleanupConfirmation}
                onChange={(e) => setCleanupConfirmation(e.target.value)}
                placeholder="CONFIRMAR"
                className="rounded-premium-full border-destructive/30"
              />
            </div>
          </div>
          <DialogFooter className="gap-spacing-sm">
            ...
            <Button
              ...
              disabled={cleanupLoading || cleanupConfirmation !== "CONFIRMAR"}
              className="rounded-premium-full bg-destructive hover:bg-destructive/90 flex-1 font-bold"
            >
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="max-w-spacing-2xl rounded-[2.5rem] bg-background ">
            ...
            <div className="space-y-spacing-lg py-spacing-md">
              <div className="grid grid-cols-2 gap-spacing-md">
                <div className="space-y-spacing-2xs">
                  ...
                  <p className="text-premium-xs font-mono bg-muted p-spacing-xs rounded-premium-full truncate">
                    {selectedTx.id}
                  </p>
                </div>
                <div className="space-y-spacing-2xs">
                  ...
                  <p className="text-premium-xs font-mono bg-muted p-spacing-xs rounded-premium-full truncate">
                    {selectedTx.payment_id || "N/A"}
                  </p>
                </div>
              </div>
              <div className="space-y-spacing-md">
                <div className="flex justify-between items-center">
                  <p className="text-premium-xs font-bold uppercase text-muted-foreground">
                    Webhook Payload & Logs
                  </p>
                  <div className="flex gap-spacing-xs items-center">
                    <div className="relative">
                      <Icons.Search className="absolute left-spacing-xs top-spacing-2xs/2 -translate-y-1/2 w-spacing-sm h-spacing-sm text-muted-foreground" />
                      <Input
                        placeholder="Buscar no JSON..."
                        value={payloadSearch}
                        onChange={(e) => setPayloadSearch(e.target.value)}
                        className="h-spacing-lg text-premium-xs pl-spacing-lg w-spacing-4xl rounded-premium-full"
                      />
                    </div>
                    <Badge variant="outline" className="text-premium-xs font-bold">
                      {filteredJSON?.count} matches
                    </Badge>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-spacing-lg text-premium-xs"
                      onClick={() =>
                        copyToClipboard(JSON.stringify(selectedTx.webhook_payload, null, 2))
                      }
                    >
                      {copied ? (
                        <Icons.Check className="w-spacing-sm h-spacing-sm text-green-500" />
                      ) : (
                ...
                <div className="max-h-spacing-4xl overflow-y-auto bg-slate-950 p-spacing-lg rounded-premium border border-white/5 custom-scrollbar">
                  <pre className="text-premium-small text-slate-300 font-mono whitespace-pre-wrap">
          ...
          <Button
            ...
            className="rounded-premium-full w-full font-bold h-spacing-2xl"
          >
            Fechar Detalhes
          </Button>
        </DialogContent>
      ...
      <Dialog open={isAuditOpen} onOpenChange={setIsAuditOpen}>
        <DialogContent className="max-w-spacing-4xl rounded-[2.5rem] bg-background  max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-premium-2xl font-serif font-bold flex items-center gap-spacing-sm">
              <div className="p-spacing-xs bg-primary/10 rounded-premium text-primary">
                <Icons.ShieldAlert className="w-spacing-lg h-spacing-lg" />
              </div>
              Histórico de Exportações
            </DialogTitle>
            <DialogDescription>
              Rastreabilidade de todos os arquivos CSV gerados por administradores.
            </DialogDescription>
          </DialogHeader>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-spacing-md my-spacing-lg">
            <div className="space-y-spacing-2xs">
              <label className="text-premium-xs font-bold uppercase text-muted-foreground">
                Admin
              </label>
              <Input
                placeholder="E-mail"
                ...
                className="h-spacing-xl rounded-premium-full text-premium-xs"
              />
            </div>
            <div className="space-y-spacing-2xs">
              <label className="text-premium-xs font-bold uppercase text-muted-foreground">
          ...
          <div className="py-spacing-xs">
              ...
              <TableBody>
                {auditLogs.length === 0 && !auditLoading ? (
                  <TableRow>
                    <TableCell
                      colSpan={5}
                      className="h-spacing-4xl text-center italic text-muted-foreground"
                    >
                      Nenhum log de exportação encontrado.
                    </TableCell>
                  </TableRow>
                ) : (
                  auditLogs.map((log: any) => (
                    <TableRow key={log.id} className="group">
                      <TableCell className="text-premium-xs font-medium">
                      ...
                      <TableCell className="text-premium-xs">
                        <div className="flex flex-col">
                          ...
                          <span
                            className={`text-premium-xs font-bold uppercase ${log.metadata?.status === "cancelled" ? "text-destructive" : "text-green-500"}`}
                          >
                            {log.metadata?.status === "cancelled" ? "Cancelado" : "Concluído"}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell className="text-premium-xs font-bold text-primary">
                        {log.metadata?.records_count}
                      </TableCell>
                      <TableCell className="text-premium-xs max-w-[200px] truncate italic text-muted-foreground">
                        {Object.entries(log.metadata?.filters || {})
                          .map(([k, v]) => (v !== "all" && v ? `${k}:${v}` : null))
                          .filter(Boolean)
                          .join(", ") || "Sem filtros"}
                      </TableCell>
                ...
                {auditLoading && (
                  <TableRow>
                    <TableCell
                      colSpan={5}
                      className="h-spacing-3xl text-center italic text-premium-xs"
                    >
                      Carregando...
                    </TableCell>
                  </TableRow>
            ...
            {hasMoreAudits && (
              <div
                ref={lastAuditElementRef}
                className="h-spacing-xl flex items-center justify-center"
              >
                {auditLoading && (
                  <Icons.Clock className="w-spacing-md h-spacing-md animate-spin text-muted-foreground" />
                )}
              </div>
            )}
          </div>
          <Button
            ...
            variant="outline"
            className="rounded-premium-full w-full font-bold h-spacing-2xl mt-spacing-md"
          >
            Fechar Auditoria
          </Button>
        </DialogContent>
