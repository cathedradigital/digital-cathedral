...
function buildShareUrl(hour: string, isoDate: string, isToday: boolean, blockId: string | null) {
  const origin =
    typeof window !== "undefined" ? window.location.origin : "https://www.cathedradigital.com.br";
...
export const BreviaryShareButtons: React.FC<Props> = ({
  ...
  const url = useMemo(() => {
    ...
    try {
      blockId = localStorage.getItem(bookmarkKey);
    } catch {
      /* silent */
    }
  ...
  return (
    <div
      className="flex items-center gap-spacing-2xs"
      data-testid="breviary-share"
      aria-label={`Compartilhar link do trecho — ${shareTitle}`}
    >
      <Button
        type="button"
        variant="pill"
