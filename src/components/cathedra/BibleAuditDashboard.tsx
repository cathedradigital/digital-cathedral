    ...
    <div className="space-y-8 py-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: "Livros",
            value: `${data.stats.coveredBooks}/${data.stats.totalBooks}`,
            icon: Icons.Book,
          },
          {
            label: "Capítulos",
            value: `${data.stats.coveredChapters}/${data.stats.totalChapters}`,
            icon: Icons.List,
          },
          {
            label: "Versículos",
            value: `${data.stats.coveredVerses}/${data.stats.totalVerses}`,
            icon: Icons.Type,
          },
          {
            label: "Integridade",
            value: `${Math.round((data.stats.coveredChapters / data.stats.totalChapters) * 100)}%`,
            icon: Icons.ShieldCheck,
          },
        ].map((stat, i) => (
          <div
            key={i}
            className="p-4 bg-background/50 backdrop-blur-sm border border-primary/5 rounded-2xl shadow-sm space-y-2"
          >
      ...
      <div className="bg-background/50 backdrop-blur-sm p-6 border border-primary/5 rounded-2xl shadow-sm space-y-4">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-primary/40">
          Detalhamento por Livro (Top 10 Lacunas)
        </h3>
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
