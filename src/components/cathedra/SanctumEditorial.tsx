...
const SanctumEditorial: React.FC<{ saint: Saint }> = ({ saint }) => {
  ...
  const anything =
    hasAnyLife ||
    (saint.timeline?.length ?? 0) > 0 ||
    (saint.virtues?.length ?? 0) > 0 ||
    hasIcono ||
    hasPatronages ||
    hasCuriosities ||
    hasMiracles ||
              ...
              <div className="bg-secondary/20 rounded-[1.5rem] p-spacing-lg border border-border space-y-spacing-sm">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-spacing-2xs">
                      {saint.iconography!.symbols!.map((s, i) => (
                        <span
                          key={i}
                          className="px-spacing-sm py-spacing-2xs rounded-premium-full bg-background border border-border text-premium-sm text-foreground"
                        >
                          {s}
                        </span>
                      ))}
                ...
                {saint.iconography!.attributes?.length ? (
                  <div>
                    ...
                    <p className="text-premium-sm text-foreground/80">
                      {saint.iconography!.attributes!.join(" · ")}
                    </p>
                  </div>
                ) : null}
                {saint.iconography!.colors?.length ? (
              ...
              <div className="bg-primary/5 rounded-[1.5rem] p-spacing-lg border border-primary/10">
                <ul className="flex flex-wrap gap-spacing-xs">
                  {saint.patronages!.map((p, i) => (
                    <li
                      key={i}
                      className="px-spacing-sm py-spacing-2xs rounded-premium-full bg-background border border-primary/20 text-premium-sm text-foreground"
                    >
                      {p}
                    </li>
                  ))}
        ...
        <section className="space-y-spacing-md">
          ...
          <div className="grid md:grid-cols-2 gap-spacing-md">
            {saint.miracles!.map((m, i) => (
              <article
                key={i}
                className="p-spacing-lg rounded-[1.5rem] bg-card border border-border"
              >
                {(m.title || m.year) && (
                  <header className="flex items-baseline justify-between mb-spacing-2xs">
                    <h5 className="text-premium-sm font-black text-foreground">
                      {m.title || "Sinal"}
                    </h5>
                    {m.year && (
                      <span className="text-premium-xs text-muted-foreground">{m.year}</span>
                    )}
                  </header>
                )}
                ...
                {m.place && (
                  <p className="mt-spacing-2xs text-premium-xs text-muted-foreground">{m.place}</p>
                )}
              </article>
            ))}
          </div>
        ...
        <section className="space-y-spacing-md">
          ...
          <div className="space-y-spacing-md">
            {saint.quotesRich!.map((q, i) => (
              <blockquote
                key={i}
                className="border-l-2 border-primary/40 pl-spacing-lg py-spacing-xs max-w-[68ch]"
              >
        ...
        <section className="space-y-spacing-md">
          ...
          <ul className="list-disc pl-spacing-lg space-y-spacing-2xs max-w-[68ch]">
            {saint.curiosities!.map((c, i) => (
              <li key={i} className="font-serif text-premium-md text-foreground/85 leading-relaxed">
                {c}
              </li>
            ))}
          </ul>
          ...
          <div className="grid md:grid-cols-2 gap-spacing-lg">
            {sp.prayer && (
              <div className="space-y-spacing-2xs">
                <p className="text-premium-xs font-black uppercase tracking-widest text-primary/80">
                  Oração
                </p>
                <p className="font-serif text-premium-md text-foreground/90 leading-relaxed">
                  {renderRich(sp.prayer)}
                </p>
              </div>
            )}
            {sp.purpose && (
              ...
              <div className="space-y-spacing-2xs">
                ...
                <ul className="list-decimal pl-spacing-md space-y-spacing-2xs">
                  {sp.examination.map((q, i) => (
                    <li
                      key={i}
                      className="font-serif text-premium-md text-foreground/85 leading-relaxed"
                    >
                      {q}
                    </li>
                  ))}
