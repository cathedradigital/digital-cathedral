...
const NavigationErrorInspector: React.FC = () => {
  ...
  const [evidenceStatus, setEvidenceStatus] = useState<
    Record<string, { ok: boolean; reason?: string; detail?: string; code?: string }>
  >({});
  ...
  const filteredAuditLogs = auditLogs.filter((log) => {
    const userMatch =
      !auditFilterUser ||
      (log.profiles?.name || "Admin").toLowerCase().includes(auditFilterUser.toLowerCase());
    const generalMatch =
  ...
  const downloadReport = (
    type: "errors" | "audit" | "broken" | "summary",
    formatExt: "json" | "csv" | "pdf",
  ) => {
    ...
    if (type === "summary") {
      ...
      dataToExport = errors
        .filter((e) => evidenceStatus[e.id] && !evidenceStatus[e.id].ok)
        .map((e) => ({
    ...
    if (formatExt === "pdf") {
      ...
      doc.text(
        `Filtro de Data: ${dateRange.from || "Sempre"} até ${dateRange.to || "Hoje"}`,
        14,
        35,
      );
      ...
      if (type === "summary") {
        ...
        doc.text(
          `Taxa de Integridade: ${((metrics.ok / (metrics.total || 1)) * 100).toFixed(1)}%`,
          14,
          70,
        );
        ...
        autoTable(doc, {
          ...
          body: dataToExport.map((a) => [
            a.profiles?.name || "Admin",
            a.request_id,
            format(new Date(a.inspected_at), "dd/MM/yy HH:mm"),
            a.masked_ip,
          ]),
        });
      } else {
        autoTable(doc, {
          ...
          body: dataToExport.map((e) => [
            e.metadata?.requestId || e.id.substring(0, 8),
            e.metadata?.route,
            e.metadata?.message?.substring(0, 50),
            e.metadata?.isMobile ? "Mobile" : "Desktop",
          ]),
        });
      }
    ...
    if (formatExt === "json") {
      ...
      if (type === "errors") {
        ...
        content =
          ...
          dataToExport
            .map((e) => `${e.requestId},"${e.route}",${e.status},"${e.reason}","${e.detail}"`)
            .join("\n");
      }
    }

    const blob = new Blob([content], {
      type: formatExt === "json" ? "application/json" : "text/csv",
    });
    ...
    <div className="max-w-7xl mx-auto p-spacing-lg space-y-spacing-xl pb-spacing-4xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-premium-sm">
              Diagnóstico de TypeErrors e falhas de navegação mobile.
            </p>
          </div>
          <div className="flex items-center gap-spacing-sm">
            <CathedraButton
              ...
              className={cn(
                "rounded-premium-full transition-all duration-300",
                auditMode &&
                  "ring-2 ring-primary ring-offset-2 ring-offset-background shadow-premium",
              )}
            >
            ...
            {activeTab === "errors" && (
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger
                  className={cn(
                    "w-[140px] h-9 rounded-premium-full bg-muted/20 border-border/10",
                    statusFilter !== "all" && "border-primary ring-1 ring-primary/30 bg-primary/5",
                  )}
                >
            ...
            {activeTab === "audit" && (
              <Input
                ...
                className={cn(
                  "max-w-[150px] rounded-premium-full h-9",
                  auditFilterUser && "border-primary ring-1 ring-primary/30 bg-primary/5",
                )}
              />
            )}
            <div
              className={cn(
                "flex gap-1 items-center bg-muted/20 p-1 rounded-premium-full border border-border/10 h-9 transition-all",
                ...
                <div className="flex gap-2">
                  ...
                  {(filter ||
                    ...
                    <div className="flex flex-wrap gap-2 items-center px-1">
                      <span className="text-[10px] font-black uppercase opacity-40 mr-2">
                        Filtros Ativos:
                      </span>
                      {filter && (
                        <Badge
                          variant="secondary"
                          className="rounded-premium-full px-3 py-1 bg-primary/10 text-primary border-primary/20 flex items-center gap-2"
                        >
                          Busca: {filter}
                          <Icons.X
            ...
            <Input
              ...
              className={cn(
                "max-w-[150px] rounded-premium-full h-9",
                filter && "border-primary ring-1 ring-primary/30 bg-primary/5",
              )}
            />
          </div>
        </div>
      ...
      {auditMode && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-spacing-md animate-in slide-in-from-top duration-500">
          <CathedraCard className="p-spacing-md flex items-center gap-spacing-md bg-primary/5 border-primary/20">
            <div className="p-2 bg-primary/10 rounded-premium-lg text-primary">
          ...
          <CathedraCard className="p-spacing-md flex items-center gap-spacing-md bg-orange-500/5 border-orange-500/20">
            <div className="p-2 bg-orange-500/10 rounded-premium-lg text-orange-600">
          ...
          <CathedraCard className="p-spacing-md flex items-center gap-spacing-md bg-green-500/5 border-green-500/20">
            <div className="p-2 bg-green-500/10 rounded-premium-lg text-green-600">
          ...
          <CathedraCard className="p-spacing-md flex items-center gap-spacing-md bg-red-500/5 border-red-500/20">
            <div className="p-2 bg-red-500/10 rounded-premium-lg text-red-600">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-spacing-lg">
        <TabsList className="bg-muted/30 p-1 rounded-premium-full border border-border/10">
          <TabsTrigger
            value="errors"
            className="rounded-premium-full text-premium-xs font-black uppercase tracking-widest px-spacing-xl"
          >
        ...
        <TabsContent
          value="errors"
          className="grid grid-cols-1 lg:grid-cols-3 gap-spacing-lg animate-in fade-in duration-500"
        >
            ...
            <ScrollArea className="flex-1">
              <div className="divide-y divide-border/10">
                    ...
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex gap-1 items-center">
                        <Badge variant="destructive" className="text-[8px] uppercase">
                          {err.metadata?.type || "UI_ERROR"}
                        </Badge>
                        {evidenceStatus[err.id] && !evidenceStatus[err.id].ok && (
                          <Badge
                            variant="outline"
                            className="text-[8px] text-orange-500 border-orange-500/20 bg-orange-500/5"
                          >
                      ...
                      <span className="text-[10px] font-mono opacity-40">
                        {format(new Date(err.created_at), "HH:mm:ss")}
                      </span>
                    </div>

                    <div className="text-premium-sm font-bold truncate text-primary/80">
                      {err.metadata?.route || "/"}
                    </div>
                    <div className="text-[10px] font-mono opacity-60 truncate mt-1">
                      ID: {err.metadata?.requestId || err.id.split("-")[0]}
                    </div>
                ...
                {filteredErrors.length === 0 && !loading && (
                  <div className="p-spacing-xl text-center opacity-40 italic text-premium-sm">
            ...
            {selectedError ? (
              <div className="space-y-spacing-xl animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-premium-xl font-bold text-primary">
                      Detalhes da Ocorrência
                    </h2>
                    <p className="text-muted-foreground text-premium-xs font-mono">
                      {selectedError.metadata?.requestId}
                    </p>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <Badge className="rounded-premium-full">
                      {selectedError.metadata?.viewport || "unknown"}
                    </Badge>
                    {auditMode && (
                      <CathedraButton
                        variant="outline"
                    ...
                    <div className="text-[10px] opacity-40 uppercase font-black">
                      {format(new Date(selectedError.created_at), "PPPP p")}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-spacing-md">
                  <div className="p-spacing-md bg-muted/20 rounded-premium-lg border border-border/10">
                    <span className="text-[9px] font-black uppercase opacity-40 block mb-1">
                      Rota
                    </span>
                    <span className="text-premium-sm font-bold">
                      {selectedError.metadata?.route}
                    </span>
                  </div>
                  <div className="p-spacing-md bg-muted/20 rounded-premium-lg border border-border/10">
                    <span className="text-[9px] font-black uppercase opacity-40 block mb-1">
                    ...
                    <span className="text-premium-sm font-bold">
                      {selectedError.metadata?.isMobile ? "Mobile" : "Desktop"}
                    </span>
                  </div>
                </div>

                <div className="space-y-spacing-sm">
                  <h3 className="text-premium-sm font-black uppercase tracking-widest opacity-60">
                    Mensagem de Erro
                  </h3>
                ...
                {selectedError.metadata?.screenshotUrl && (
                  <div className="space-y-spacing-sm">
                    ...
                    <div className="rounded-premium-xl border border-border/20 overflow-hidden shadow-premium">
                      <img
                        src={selectedError.metadata.screenshotUrl}
                        alt="Erro de UI"
              ...
              <div className="h-full flex flex-col items-center justify-center text-center opacity-30 space-y-spacing-md">
                ...
                <p className="font-serif italic text-premium-lg">
                  Selecione uma falha ao lado para inspecionar os metadados técnicos.
                </p>
              </div>
            )}
          </CathedraCard>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-spacing-lg">
            <CathedraCard className="xl:col-span-2 p-0 overflow-hidden">
              <div className="p-spacing-md bg-muted/20 border-b border-border/10 flex justify-between items-center">
                <span className="font-bold text-premium-xs uppercase tracking-widest">
                  Acessos dos Inspetores
                </span>
              </div>
                  ...
                  <tbody className="divide-y divide-border/10">
                    {filteredAuditLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-primary/[0.01] transition-colors">
                        ...
                        <td className="p-spacing-md font-mono text-primary/60">{log.request_id}</td>
            ...
            <CathedraCard className="p-0 overflow-hidden flex flex-col">
              <div className="p-spacing-md bg-muted/20 border-b border-border/10 flex justify-between items-center">
                <span className="font-bold text-premium-xs uppercase tracking-widest">
                  Trilha de Links Compartilhados
                </span>
              </div>
              <ScrollArea className="h-[60vh]">
                <div className="p-spacing-md space-y-spacing-md">
                  {shareTrail.map((item) => (
                    <div
                      key={item.id}
                      className="p-spacing-sm bg-muted/20 rounded-premium-lg border border-border/10 text-[10px] space-y-1"
                    >
                  ...
                  {shareTrail.length === 0 && (
                    <div className="p-spacing-xl text-center opacity-30 italic text-[10px]">
                      Nenhum link gerado nesta sessão.
                    </div>
                  )}
                </div>
              </ScrollArea>
