...
export const TagBubble: React.FC<TagBubbleProps> = ({
  ...
  const contextPath =
    navHistory.length > 1 ? navHistory.map((t) => t.label).join(" · ") : currentTag.label;
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      ...
      try {
        ...
        const remoteIdx =
          typeof remote.activeSectionIdx === "number" ? remote.activeSectionIdx : activeSectionIdx;
  ...
  useEffect(() => {
    ...
    setLiveMessage(
      sectionLiveMessage(activeSectionIdx, narrativeSections.length, current.preset.eyebrow),
    );
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (total <= 1) return;
      const nextIdx =
        dx < 0 ? Math.min(activeSectionIdx + 1, total - 1) : Math.max(activeSectionIdx - 1, 0);
        ...
        <div className="flex-1 overflow-y-auto px-spacing-xl pb-spacing-2xl scrollbar-none">
            ...
            <div className="space-y-spacing-2xl" data-testid="nexus-sections">
              {(focusMode
                ...
                <motion.section
                  key={section.kind}
                  ref={(el) => {
                    sectionRefs.current[section.kind] = el as unknown as HTMLElement | null;
                  }}
                  initial={{ opacity: 0, y: 6 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  <ul className="space-y-spacing-lg">
                    {section.items.map((c, i) => {
                      ...
                      const bibleChapter: number | undefined = isBible
                        ? Number(c.metadata?.chapter)
                        : undefined;
                      const bibleVerse: number | undefined =
                        isBible && c.metadata?.verse ? Number(c.metadata.verse) : undefined;
                        ...
                        <li key={c.id || i}>
                          ...
                          {meta && (
                            <p className="text-[10px] uppercase tracking-[0.28em] text-primary/45 mb-spacing-sm">
                              {[c.metadata?.author, c.metadata?.year || c.metadata?.date]
                                .filter(Boolean)
                                .join(" · ")}
                            </p>
                          )}
                          <NexusInlinePreview
                            ...
                            openHref={
                              link ??
                              (canPopover
                                ? `/bible?book=${bibleAbbr}&ch=${bibleChapter}${bibleVerse ? `&verse=${bibleVerse}` : ""}`
                                : null)
                            }
                          ...
                          {i < section.items.length - 1 && (
                            <div
                              aria-hidden
                              className="mt-spacing-lg h-px w-[24px] bg-primary/15"
                            />
                          )}
                        </li>
                      );
                  ...
                  {sIdx < arr.length - 1 && (
                    <div
                      aria-hidden
                      className="mt-spacing-2xl mx-auto h-px w-[40px] bg-secondary/40"
                    />
                  )}
                </motion.section>
              ))}
          ...
          {status !== "loading" && !focusMode && (
            ...
            >
              ...
              {allThemes &&
                allThemes.filter(
                  (t) => t.category === currentTag.category && t.id !== currentTag.id,
                ).length > 0 && (
                      ...
                      {allThemes
                        ...
                        .map((t) => (
                          <li key={t.id}>
                            ...
                            >
                              <span
                                className="w-[2px] h-[14px] bg-secondary/40 group-hover:bg-secondary transition-colors"
                                aria-hidden
                              />
          ...
          {navHistory.length > 1 && !focusMode && (
            <nav
              aria-label="Caminho percorrido"
              className="mt-spacing-2xl pt-spacing-lg border-t border-primary/10"
            >
              <span className="block text-[9px] uppercase tracking-[0.32em] text-primary/40 mb-spacing-sm">
                Caminho percorrido
              </span>
