                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {run.pages_failed > 0 && (
                        <Badge
                          variant="destructive"
                          className="h-spacing-md text-[9px] px-spacing-2xs font-black uppercase"
                        >
          ...
          <Tabs defaultValue="all" className="w-full">
            <div className="flex items-center justify-between mb-spacing-md">
              <TabsList className="bg-muted/30 p-spacing-2xs rounded-premium-full border border-border/10">
                <TabsTrigger
                  value="all"
                  className="rounded-premium-full text-premium-xs font-black uppercase tracking-wider px-spacing-md"
                >
                  Tudo
                </TabsTrigger>
                <TabsTrigger
              ...
              <Button
                size="sm"
                variant="outline"
                className="rounded-premium-full h-spacing-xl text-premium-xs font-black uppercase tracking-wider gap-spacing-xs"
              >
            ...
            <TabsContent value="failed" className="mt-spacing-0 space-y-spacing-md">
              ...
              {snapshots.filter((s) => s.status === "fail").length === 0 && (
                <div className="text-center py-spacing-2xl opacity-50 italic text-premium-sm">
                  Nenhuma falha encontrada nesta execução.
                </div>
              )}
            </TabsContent>
          </Tabs>
...
const StatsCard = ({ title, value, icon, status }: any) => (
  <Card className="border-border/10 bg-muted/10 rounded-premium overflow-hidden">
    <CardContent className="p-spacing-md flex items-center justify-between">
      ...
      <div
        className={`w-spacing-xs h-spacing-xs rounded-premium-full animate-pulse ${status === "valid" ? "bg-green-500" : "bg-red-500"}`}
      />
    </CardContent>
  </Card>
);
    ...
    <Card className="border-border/10 bg-muted/10 overflow-hidden rounded-premium group transition-all duration-300 hover:bg-muted/20">
      <CardHeader className="p-spacing-md border-b border-border/10 flex flex-row items-center justify-between">
        <div className="flex items-center gap-spacing-sm">
          <div
            className={`p-spacing-xs rounded-premium-sm ${snapshot.status === "pass" ? "bg-green-500/10 text-green-500" : snapshot.status === "fail" ? "bg-red-500/10 text-red-500" : "bg-blue-500/10 text-blue-500"}`}
          >
          ...
          <div>
            ...
            <p className="text-[10px] font-bold opacity-50 uppercase tracking-widest">
              {snapshot.route} • {snapshot.viewport}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-spacing-xs">
          ...
          {snapshot.status === "fail" && (
            <div className="flex gap-spacing-xs">
              ...
              >
                {isApproving ? (
                  <Icons.RefreshCw className="w-spacing-sm h-spacing-sm animate-spin" />
                ) : (
                  <Icons.Check className="w-spacing-sm h-spacing-sm mr-spacing-xs" />
                )}{" "}
                Aceitar
              </Button>
              <Button
          ...
          {snapshot.status === "approved" && (
            <Badge
              variant="outline"
              className="h-spacing-xl rounded-premium-full text-[9px] font-black uppercase tracking-wider border-blue-500/30 text-blue-500 bg-blue-500/5"
            >
        ...
        {showDiff && snapshot.diff_url ? (
          <div className="relative aspect-video bg-black/5 rounded-premium-sm overflow-hidden border border-border/10">
            <img
              ...
              alt="Diferença Visual"
              className="w-full h-full object-contain"
            />
            <div className="absolute top-spacing-xs left-spacing-xs px-spacing-xs py-spacing-2xs bg-red-500 text-white text-[8px] font-black uppercase tracking-widest rounded-premium-md">
              Diferenças em Vermelho
            </div>
          </div>
        ) : (
        ...
        {snapshot.typography_errors && snapshot.typography_errors.length > 0 && (
          <div className="p-spacing-sm bg-yellow-500/5 border border-yellow-500/20 rounded-premium-sm space-y-spacing-xs">
            <p className="text-[9px] font-black uppercase tracking-widest text-yellow-600 flex items-center gap-spacing-2xs">
              <Icons.Type className="w-spacing-sm h-spacing-sm" /> Discrepâncias Tipográficas
              Encontradas ({snapshot.typography_errors.length})
            </p>
