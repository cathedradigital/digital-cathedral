...
export default function BibleCacheTimeseriesDashboard() {
  ...
  const loadDiagRuns = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("bible-canon-diagnose", {
        body: { action: "list_runs", limit: 10 },
      });
  ...
  const totals = useMemo(() => {
    const t = agg.reduce(
      (a, r) => ({
        ...
        l1_fresh: a.l1_fresh + r.l1_fresh,
        l1_stale: a.l1_stale + r.l1_stale,
        l1_miss: a.l1_miss + r.l1_miss,
        l1_bypass: a.l1_bypass + r.l1_bypass,
      }),
      {
        total: 0,
        hits: 0,
  ...
  const drillSources = useMemo(
    () => Array.from(new Set(drillRows.map((r) => r.request_source ?? "").filter(Boolean))).sort(),
    [drillRows],
  );
  const drillInstances = useMemo(
    () => Array.from(new Set(drillRows.map((r) => r.instance_id ?? "").filter(Boolean))).sort(),
    [drillRows],
  );
  ...
  const runBenchmark = async () => {
    ...
    try {
      ...
      if (error) throw error;
      const files = (
        data as { files?: { summary_csv: string; detailed_csv: string; report_md: string } }
      ).files;
      ...
      downloadFile(
        `bible_cache_ttl_benchmark_detailed_${stamp}.csv`,
        "text/csv",
        files.detailed_csv,
      );
      downloadFile(
        `bible_cache_ttl_benchmark_report_${stamp}.md`,
        "text/markdown",
        files.report_md,
      );
  ...
  const runDiagnostic = async () => {
    ...
    try {
      ...
      toast.success(
        `Diagnóstico ${r.status} · ${r.total_findings} findings · ${r.total_chapters} capítulos em ${r.duration_ms}ms`,
      );
      loadDiagRuns();
    } catch (e) {
  ...
  const exportDiagnostic = async (runId: string, format: "csv" | "json") => {
    try {
      const { data, error } = await supabase.functions.invoke("bible-canon-diagnose", {
        body: { action: "export", run_id: runId, format },
      });
      if (error) throw error;
      ...
      <div className="container mx-auto p-6 max-w-2xl">
        <Alert className="border-amber-500/40 bg-amber-500/5">
          ...
          <AlertDescription className="mt-3 space-y-3">
            <p>
              Este painel expõe métricas operacionais sensíveis (correlation_id, latências por
              janela, distribuição L1). Apenas usuários com role{" "}
              <code className="px-1 py-0.5 rounded bg-muted">admin</code> podem acessá-lo.
            </p>
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            Clique em qualquer ponto para drilldown.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Janela de agregação, período histórico e filtro por livro
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        {[
          ...
          { label: "Invalidação (MISS)", value: `${totals.invalidationRate}%` },
          {
            ...
            extra: totals.staleLowConfidence ? (
              <Badge
                variant="outline"
                className="mt-1 text-[10px] border-amber-500/40 text-amber-700 dark:text-amber-300"
              >
                stale n&lt;{STALE_LOW_CONFIDENCE_THRESHOLD} · baixa confiança
              </Badge>
            ) : null,
          },
        ].map((s) => (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Clique em um ponto para abrir o drilldown da bucket
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            fresh • stale • miss • bypass — clique numa barra para drilldown
          </CardDescription>
        </CardHeader>
      ...
      <Card className="border-secondary/30">
        <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
          <div>
            ...
            <CardDescription className="text-xs">
              Valida existência, contagem de capítulos, capítulos vazios, idioma e metadata. Roda
              diariamente via cron e sob demanda.
            </CardDescription>
          </div>
        ...
        <CardContent>
          {diagRuns.length === 0 ? (
            <p className="text-xs text-muted-foreground py-4 text-center">
              Nenhuma execução registrada. Clique em "Rodar agora".
            </p>
          ) : (
            <Table>
              <TableHeader>
                  ...
                  return (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right tabular-nums text-xs">
                        {r.total_books_checked}
                      </TableCell>
                      <TableCell className="text-right tabular-nums text-xs">
                        {r.total_chapters_checked}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex gap-1 justify-end">
                          <Button
                            ...
                            variant="ghost"
                            className="h-7 px-2"
                            onClick={() => exportDiagnostic(r.id, "csv")}
                            title="Exportar CSV"
                          >
                          ...
                          <Button
                            ...
                            variant="ghost"
                            className="h-7 px-2"
                            onClick={() => exportDiagnostic(r.id, "json")}
                            title="Exportar JSON"
                          >
        ...
        <DialogContent className="max-w-5xl max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              {filteredDrill.length} de {drillRows.length} requests. Filtre por
              origem/instância/cold start e ordene por qualquer métrica.
            </DialogDescription>
          </DialogHeader>
                  ...
                  <div key={k} className={`p-3 rounded-lg border ${cls}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2 text-xs">
                      <div>
                        ...
                        <div className="font-bold tabular-nums">{s.avg}ms</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">p95</span>
                        <div className="font-bold tabular-nums">{s.p95}ms</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">max</span>
                        <div className="font-bold tabular-nums">{s.max}ms</div>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="overflow-auto flex-1 -mx-6 px-6">
            {drillLoading ? (
              ...
              <p className="text-sm text-muted-foreground py-8 text-center">
                Sem requests para o filtro atual.
              </p>
            ) : (
              <Table>
                <TableHeader>
                    ...
                    const levelCls =
                      r.cache_level === "L1"
                        ...
                        : r.cache_level === "L2"
                          ...
                          : r.cache_level === "DB"
                            ? "border-amber-500/40 text-amber-700 dark:text-amber-300"
                            : r.cache_level === "UNAVAILABLE"
                              ? "border-rose-500/40 text-rose-700 dark:text-rose-300"
                              : "border-muted text-muted-foreground";
                    const breakdownTitle =
                      (r.sql_breakdown ?? []).map((b) => `${b.label}: ${b.ms}ms`).join("\n") ||
                      "sem breakdown";
                    return (
                      <TableRow key={`${r.correlation_id ?? i}-${r.created_at}`}>
                        ...
                        <TableCell>
                          <span
                            className={`inline-block px-1.5 py-0.5 rounded border text-[10px] font-semibold ${levelCls}`}
                          >
                            {r.cache_level ?? "—"}
                          </span>
                        </TableCell>
                        <TableCell>
                          <span
                            className={`inline-block px-2 py-0.5 rounded border text-[10px] uppercase tracking-wider ${l1PhaseColor(r.l1_phase)}`}
                          >
                            {r.l1_phase ?? "—"}
                          </span>
                        </TableCell>
                        ...
                        <TableCell
                          className="text-right tabular-nums text-xs cursor-help"
