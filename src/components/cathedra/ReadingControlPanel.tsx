  ...
  const SettingsContent = () => (
    <div className="space-y-spacing-xl py-spacing-lg">
      <div className="text-center space-y-spacing-xs">
        <p className="text-[8px] text-primary/30 uppercase tracking-[0.5em] font-bold">
          Atmosphæra
        </p>
        <h2 className="text-premium-2xl font-display font-light text-primary uppercase tracking-[0.25em] leading-tight">
          Leitura
        </h2>
      </div>

      <div className="space-y-spacing-md">
        <p className="text-premium-xs font-black uppercase tracking-widest text-muted-foreground/60 px-spacing-2xs text-center">
          Tons da Alma
        </p>
          ...
          {themes.map((t) => (
            <button
              ...
              className={`w-full aspect-square rounded-premium-full border-2 transition-all duration-500 ${t.color} ${
                settings.theme === t.id
                  ? "ring-4 ring-primary/20 ring-offset-4 scale-110"
            ...
            >
              {settings.theme === t.id && (
                <Icons.Check
                  className={`w-spacing-md h-spacing-md mx-auto ${t.id === "dark" || t.id === "night" ? "text-white" : "text-primary"}`}
                />
              )}
            </button>
          ))}
          ...
          {(["small", "medium", "large", "extra-large"] as const).map((s) => (
            <button
              ...
              className={`flex-1 py-spacing-sm text-premium-xs font-bold rounded-premium transition-all duration-500 ${
                settings.fontSize === s
                  ? "bg-background text-primary shadow-premium scale-105"
      ...
      <div className="space-y-spacing-lg">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-spacing-xs py-spacing-md rounded-premium-full transition-all duration-700 font-serif ${
              settings.fontFamily === "serif"
                ...
                : "bg-primary/[0.01] text-primary/40 hover:bg-primary/[0.03]"
            }`}
          >
        ...
        <div className="grid grid-cols-1 gap-spacing-lg">
          <div className="space-y-spacing-sm">
            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 px-spacing-2xs">
              ...
              {(["tight", "normal", "wide"] as const).map((l) => (
                <button
                  ...
                  className={`flex-1 py-spacing-xs text-[9px] font-black uppercase tracking-widest rounded-premium-full transition-all duration-500 ${
                    settings.lineSpacing === l
                      ? "bg-background text-primary shadow-premium-md"
                      : "text-muted-foreground/40 hover:text-primary"
                  }`}
                >
      ...
      <div className="space-y-spacing-sm pt-spacing-md border-t border-primary/5">
        ...
        <div className="flex items-center justify-between p-spacing-md rounded-premium bg-muted/20">
          ...
          <input
            ...
            onChange={(e) =>
              updateSettings({
                nightSchedule: { ...settings.nightSchedule, enabled: e.target.checked },
              })
            }
            className="w-spacing-md h-spacing-md rounded-premium-full border-primary/20 text-primary focus:ring-primary cursor-pointer"
          />
          ...
          <div className="grid grid-cols-2 gap-spacing-sm">
            <label className="flex flex-col gap-spacing-2xs">
              ...
              <input
                ...
                onChange={(e) =>
                  updateSettings({
                    nightSchedule: { ...settings.nightSchedule, start: e.target.value },
                  })
                }
            ...
            <label className="flex flex-col gap-spacing-2xs">
              ...
              <input
                ...
                onChange={(e) =>
                  updateSettings({
                    nightSchedule: { ...settings.nightSchedule, end: e.target.value },
                  })
                }
                className="bg-muted/30 border border-primary/10 rounded-premium px-spacing-sm py-spacing-xs text-premium-sm"
              />
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="rounded-premium-full h-spacing-xl w-spacing-xl text-primary/40 hover:text-primary hover:bg-primary/5 transition-all"
            >
          ...
          <SheetContent
            side="bottom"
            className="h-[80vh] rounded-t-[2.5rem] border-t-primary/10 bg-background/95 backdrop-blur-2xl px-spacing-lg"
          >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              className="rounded-premium-full h-spacing-xl px-spacing-md flex items-center gap-spacing-xs border-primary/10 hover:border-primary/30 transition-all bg-card/40 backdrop-blur-md shadow-premium-md"
            >
          ...
          <DropdownMenuContent
            className="w-[400px] p-spacing-xl space-y-spacing-xl rounded-[3rem] shadow-premium-hover border-primary/5 bg-background/95 backdrop-blur-2xl"
            align="end"
          >
