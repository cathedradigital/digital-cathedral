...
const AdminSeoTab: React.FC = () => {
  ...
  const handleSaveSEO = async () => {
    ...
    if (
      ...
      (!formData.business_phone && !formData.business_whatsapp) ||
      !formData.opening_hours
    ) {
      toast.error("Erro de Validação NAP", {
        description:
          "Todos os campos de SEO Local (Nome, Endereço, Telefone/Whats e Horários) são obrigatórios para garantir a indexação.",
      });
      return;
    }
            ...
            <CardHeader className="bg-muted/30 border-b border-border/50">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-spacing-md">
                <div className="flex items-center gap-spacing-xs text-primary">
                  ...
                  <div>
                    <CardTitle className="text-premium-lg font-serif">
                      Configurações Técnicas de SEO
                    </CardTitle>
                    <CardDescription>
                      Gerencie meta tags, títulos e indexação global.
                    </CardDescription>
                  </div>
                </div>
                <Button
            ...
            <CardContent className="p-spacing-lg space-y-spacing-lg">
              <div className="grid grid-cols-1 gap-spacing-lg">
                <div className="space-y-spacing-xs">
                  <Label className="text-premium-xs font-black uppercase tracking-widest opacity-70">
                    Título Base do Site (Google Title)
                  </Label>
                  <Input
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-spacing-md">
                    ...
                    <div className="space-y-spacing-xs">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, business_whatsapp: e.target.value })
                        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-spacing-md pt-spacing-lg border-t border-border/50">
                  <div className="space-y-spacing-xs">
                    <Label className="text-premium-xs font-black uppercase tracking-widest opacity-70 flex items-center gap-spacing-2xs">
                      <Icons.Search className="w-spacing-sm h-spacing-sm" /> GSC Verification
                      Icons.Code
                    </Label>
                    <Input
                      value={formData.gsc_verification_code || ""}
                      onChange={(e) =>
                        setFormData({ ...formData, gsc_verification_code: e.target.value })
                      }
                  ...
                  <div className="space-y-spacing-xs">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, ga4_measurement_id: e.target.value })
                      }
              ...
              <div className="bg-background border border-border/50 rounded-premium p-spacing-lg shadow-premium-md space-y-spacing-xs">
                ...
                <p className="text-[14px] text-[#4d5156] leading-relaxed line-clamp-spacing-xs">
                  {formData.site_description ||
                    "Aprofunde sua fé católica com Bíblia Sagrada, Catecismo da Igreja e IA teológica."}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-spacing-lg">
          <Card className="border-border/50 shadow-premium-md h-fit">
            <CardHeader className="bg-muted/10 border-b border-border/50">
              <CardTitle className="text-md font-serif flex items-center gap-spacing-xs">
                <Icons.LineChart className="w-spacing-md h-spacing-md text-primary" /> GA4 Debug
                Status
              </CardTitle>
            </CardHeader>
            <CardContent className="p-spacing-md space-y-spacing-sm">
              <div className="flex items-center justify-between text-premium-xs">
                ...
                <Badge variant="outline" className="font-mono">
                  {formData.ga4_measurement_id || "Não configurado"}
                </Badge>
              </div>
              <div className="flex items-center justify-between text-premium-xs">
                ...
                {typeof window !== "undefined" && (window as any).gtag ? (
                  ...
                  <Badge
                    variant="destructive"
                    className="bg-destructive/10 text-destructive border-destructive/20 gap-spacing-2xs"
                  >
              ...
              <div className="pt-spacing-xs space-y-spacing-xs border-t border-border/50">
                <p className="text-premium-xs font-black uppercase tracking-widest opacity-60">
                  Rastreamento Ativo:
                </p>
                <div className="grid grid-cols-2 gap-spacing-xs">
                  {["Page Views", "Clicks", "Submits", "Session"].map((ev) => (
                    <div
                      key={ev}
                      className="flex items-center gap-spacing-2xs text-premium-xs font-medium"
                    >
                      <div
                        className={`w-spacing-2xs h-spacing-2xs rounded-premium-full ${(window as any).gtag ? "bg-emerald-500" : "bg-muted"}`}
                      />
                      {ev}
                    </div>
                  ))}
          ...
          <Card className="border-border/50 shadow-premium-md h-fit">
            <CardHeader className="bg-amber-500/5 border-b border-border/50">
              <CardTitle className="text-md font-serif flex items-center gap-spacing-xs">
                <Icons.MapPin className="w-spacing-md h-spacing-md text-amber-600" /> Validação NAP
                & Local SEO
              </CardTitle>
            </CardHeader>
            <CardContent className="p-spacing-md space-y-spacing-md">
              <div className="space-y-spacing-xs">
                <div className="flex items-center justify-between text-premium-xs font-black uppercase tracking-widest opacity-60">
                  ...
                  (formData.business_phone || formData.business_whatsapp) &&
                  formData.opening_hours ? (
                    <span className="text-emerald-600 flex items-center gap-spacing-2xs">
                      <Icons.CheckCircle2 className="w-spacing-sm h-spacing-sm" /> Válido
                    </span>
                  ) : (
                    <span className="text-destructive flex items-center gap-spacing-2xs">
                      <Icons.AlertCircle className="w-spacing-sm h-spacing-sm" /> Incompleto
                    </span>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-spacing-xs">
                  <Badge
                    variant={formData.business_name ? "secondary" : "outline"}
                    className="text-premium-xs h-spacing-md justify-start"
                  >
                    Name: {formData.business_name ? "OK" : "Missing"}
                  </Badge>
                  <Badge
                    variant={formData.business_address ? "secondary" : "outline"}
                    className="text-premium-xs h-spacing-md justify-start"
                  >
                    Address: {formData.business_address ? "OK" : "Missing"}
                  </Badge>
                  <Badge
                    variant={
                      formData.business_phone || formData.business_whatsapp
                  ...
                  >
                    Phone:{" "}
                    {formData.business_phone || formData.business_whatsapp ? "OK" : "Missing"}
                  </Badge>
                  <Badge
                    variant={formData.opening_hours ? "secondary" : "outline"}
                    className="text-premium-xs h-spacing-md justify-start"
                  >
                    Hours: {formData.opening_hours ? "OK" : "Missing"}
                  </Badge>
                </div>
              </div>
              ...
              <div className="pt-spacing-xs border-t border-border/50">
                <div className="flex items-center justify-between mb-spacing-xs">
                  ...
                  >
                    <Icons.Eye className="w-spacing-sm h-spacing-sm mr-spacing-2xs" /> Verificar no
                    Site
                  </Button>
                </div>
                <div
                  className={`p-spacing-xs rounded-premium-full border flex items-center justify-between ${
                    domVerified === "ok"
          ...
          <Card className="border-border/50 shadow-premium-md h-fit">
            <CardHeader className="bg-primary/5 border-b border-border/50">
              <CardTitle className="text-md font-serif flex items-center gap-spacing-xs">
                <Icons.Sparkles className="w-spacing-md h-spacing-md text-primary" /> IA SEO
                Insights
              </CardTitle>
            </CardHeader>
            <CardContent className="p-spacing-md space-y-spacing-md">
            ...
            <CardContent className="p-spacing-md space-y-spacing-sm">
              ...
              <div className="space-y-spacing-xs">
                <a
                  ...
                  target="_blank"
                  rel="noopener"
                  className="flex items-center justify-between text-premium-xs hover:text-primary hover:underline group"
                >
                  Icons.Search Console{" "}
                  <Icons.ExternalLink className="w-spacing-sm h-spacing-sm opacity-0 group-hover:opacity-100 transition-opacity" />
                </a>
                <a
                  href="/sitemap.xml"
                  target="_blank"
