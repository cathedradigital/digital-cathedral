...
const JornadasPage = React.forwardRef<HTMLDivElement>((_props, ref) => {
  ...
  const fuzzyScoreMap = useMemo(() => {
    ...
    fuzzySearch.results.forEach((r) => {
      map[r.id] = (r as any).similarityScore ?? 0;
    });
    return map;
  }, [fuzzySearch.results]);
  ...
  const filteredJourneys = useMemo(() => {
    return journeys
      .filter((j) => {
        ...
        if (filterDifficulty !== "all" && !difficultyMatches(j.difficulty, filterDifficulty))
          return false;
        return true;
      })
      .sort((a, b) => {
  ...
  const stats = useMemo(() => {
    ...
    const completed = Object.entries(progressMap).filter(
      ([jid, count]) => count >= (stepsCountMap[jid] || Infinity),
    ).length;
      ...
      <div className="space-y-spacing-lg w-full pb-spacing-4xl">
        ...
        <div className="space-y-spacing-md">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-spacing-4xl rounded-premium bg-muted/40 animate-pulse"
  ...
  const activeJourneys = journeys.filter(
    (j) => progressMap[j.id] > 0 && progressMap[j.id] < (stepsCountMap[j.id] || 0),
  );
  ...
  return (
    <ContemplativeLayout>
      <SEOHead
        ...
        description="Percorra jornadas de transformação espiritual com conteúdos guiados de formação católica."
        path="/jornadas"
        keywords="jornada espiritual, formação católica, crescimento espiritual"
        breadcrumbs={[
          { name: "Início", path: "/" },
        ...
        <div className="w-full space-y-spacing-xl md:space-y-spacing-2xl">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button
                    variant="outline"
                    size="sm"
                    className="rounded-premium-full gap-spacing-xs text-[10px] font-black uppercase tracking-widest border-primary/20 hover:bg-primary/5"
                  >
                ...
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    <DialogTitle className="font-display text-premium-2xl text-primary">
                      Lembrete de Jornada
                    </DialogTitle>
                  </DialogHeader>
                  <div className="py-spacing-md space-y-spacing-lg">
                    <div className="space-y-spacing-sm">
                      <label className="text-premium-xs font-bold uppercase tracking-widest text-primary/60">
                      ...
                      <div className="flex gap-spacing-xs">
                        ...
                        <Button
                          onClick={async () => {
                            const time = (
                              document.getElementById("journey-time") as HTMLInputElement
                            ).value;
          ...
          {stats.started > 0 && (
            ...
            >
              <div className="text-center">
                <p className="text-premium-base sm:text-premium-lg font-black text-foreground">
                  {stats.total}
                </p>
                <p className="text-premium-xs sm:text-premium-xs font-black uppercase tracking-widest text-muted-foreground">
                  Jornadas
                </p>
              </div>
              <div className="w-px h-spacing-xl bg-border" />
              <div className="text-center">
                <p className="text-premium-base sm:text-premium-lg font-black text-primary">
                  {stats.started}
                </p>
                <p className="text-premium-xs sm:text-premium-xs font-black uppercase tracking-widest text-muted-foreground">
                  Iniciadas
              ...
              <div className="w-px h-spacing-xl bg-border" />
              <div className="text-center">
                <p className="text-premium-base sm:text-premium-lg font-black text-emerald-500">
                  {stats.completed}
                </p>
                <p className="text-premium-xs sm:text-premium-xs font-black uppercase tracking-widest text-muted-foreground">
                  Concluídas
          ...
          {!Object.keys(progressMap).length && (
            ...
            >
              <CathedraCard
                padding="md"
                className="premium-card bg-gradient-to-r from-primary/5 to-transparent overflow-hidden flex items-center gap-spacing-md sm:gap-spacing-lg"
              >
                ...
                <div className="flex-1">
                  <p className="text-premium-sm font-semibold text-foreground">
                    Inicie sua caminhada
                  </p>
                  <p className="text-premium-xs text-muted-foreground">
                    O Logos pode recomendar a melhor jornada para sua alma.
                  </p>
                </div>
                <Button
                  size="sm"
                ...
                >
                  Diagnóstico{" "}
                  <Icons.ArrowRight className="w-spacing-md h-spacing-md ml-spacing-2xs" />
                </Button>
              </CathedraCard>
            </motion.div>
          )}
          ...
          >
            <AnimatePresence mode="popLayout">
              {filteredJourneys.map((journey) => {
                ...
                const gradientClass =
                  CATEGORY_COLORS[journey.category] || "from-muted/20 to-transparent";

                return (
                  <motion.div key={journey.id} variants={cardVariants} layout exit="exit">
                    <motion.div
                    ...
                    >
                      ...
                      >
                        {/* Gradient accent */}
                        <div
                          className={`absolute inset-0 bg-gradient-to-br ${gradientClass} opacity-60 pointer-events-none`}
                        />
                        ...
                        <div className="p-spacing-sm sm:p-spacing-md space-y-spacing-xs sm:space-y-spacing-sm relative">
                          ...
                          <div className="flex items-start justify-between gap-spacing-sm">
                            <div className="flex-1 min-w-spacing-0">
                              <div className="flex items-center gap-spacing-2xs sm:gap-spacing-xs flex-wrap mb-spacing-2xs">
                                <div className="w-spacing-lg h-spacing-lg sm:w-spacing-xl sm:h-spacing-xl rounded-premium-full sm:rounded-premium bg-muted/80 flex items-center justify-center opacity-80 group-hover:opacity-100 transition-opacity">
                                  {CATEGORY_ICONS[journey.category] || (
                                    <Icons.BookOpen className="w-spacing-md h-spacing-md" />
                                  )}
                                </div>
                                <div className="flex-1 min-w-spacing-0">
                                  <h2 className="text-premium-sm sm:text-premium-base font-bold font-serif text-foreground truncate">
                                    {journey.title}
                                  </h2>
                                  {journey.subtitle && (
                                    <p className="text-premium-xs text-muted-foreground font-serif italic truncate">
                                      {journey.subtitle}
                                    </p>
                                  )}
                                </div>
                            ...
                            <div className="flex items-center gap-spacing-xs flex-shrink-0">
                              ...
                              {journey.is_premium && (
                                <Badge
                                  variant="secondary"
                                  className="bg-primary/10 text-primary border-primary/20 text-premium-xs px-spacing-xs"
                                >
                                  <Icons.Sparkles className="w-spacing-sm h-spacing-sm mr-spacing-3xs" />{" "}
                                  PRO
                                </Badge>
                              )}
                          ...
                          {/* Description */}
                          {journey.description && (
                            <p className="text-premium-xs text-muted-foreground leading-relaxed line-clamp-spacing-xs">
                              {journey.description}
                            </p>
                          )}
                          ...
                          {journey.tags && journey.tags.length > 0 && (
                            <div className="flex flex-wrap gap-spacing-2xs">
                              {journey.tags.slice(0, 3).map((tag: string) => (
                                <span
                                  key={tag}
                                  className="text-premium-xs px-spacing-xs py-spacing-3xs rounded-premium-full bg-muted text-muted-foreground font-medium"
                                >
                                  {tag}
                                </span>
                              ))}
                          ...
                          <div className="flex items-center gap-spacing-xs sm:gap-spacing-sm text-premium-xs sm:text-premium-xs text-muted-foreground flex-wrap">
                            <span className="flex items-center gap-spacing-2xs">
                              ...
                              {journey.estimated_days}d
                            </span>
                            <span
                              className={`px-spacing-xs py-spacing-3xs rounded-premium-full text-premium-xs font-bold ${DIFFICULTY_COLORS[journey.difficulty] || "bg-muted text-muted-foreground"}`}
                            >
                              {DIFFICULTY_LABELS[journey.difficulty] || journey.difficulty}
                            </span>
                            <span className="flex items-center gap-spacing-2xs">
                              <Icons.Layout className="w-spacing-sm h-spacing-sm" /> {totalSteps}{" "}
                              etapas
                            </span>
                          </div>
