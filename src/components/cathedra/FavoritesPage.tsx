    ...
    <div className="w-full space-y-spacing-xl">
      <div className="text-center space-y-spacing-sm">
        ...
        <p className="text-muted-foreground font-serif italic">
          Seus versículos, orações e estudos salvos.
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[40vh] space-y-spacing-md text-center">
          ...
          <p className="text-muted-foreground font-serif italic">
            Ao navegar pela Bíblia, Catecismo e orações, toque no ícone de coração para salvar seus
            conteúdos favoritos aqui.
          </p>
        </div>
      ) : (
        <div className="space-y-spacing-sm">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="bg-card border border-border rounded-premium p-spacing-md group hover:border-primary/30 transition-all"
            >
              <div className="flex items-start justify-between gap-spacing-md">
                <div className="space-y-spacing-2xs flex-1">
                  ...
                  <p className="text-premium-sm text-muted-foreground font-serif line-clamp-spacing-xs">
                    {item.content}
                  </p>
                </div>
                <Button
                  onClick={() => removeFavorite(item.id)}
                  className="p-spacing-xs rounded-premium-full opacity-0 group-hover:opacity-100 hover:bg-destructive/10 transition-all"
                >
