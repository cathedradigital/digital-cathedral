...
const SagradoSummary: React.FC<SagradoSummaryProps> = ({
  onSelectBook,
  activeBook,
  setActiveBook,
}) => {
  ...
  const allBooks = [...BIBLE_DATA["Antigo Testamento"], ...BIBLE_DATA["Novo Testamento"]].flatMap(
    (cat) => cat.books,
  );
          ...
          {["Antigo Testamento", "Novo Testamento"].map((testament) => (
            <div key={testament} className="mb-4">
              <div className="px-6 py-2 sticky top-0 bg-white/10 backdrop-blur-md z-10">
                <span className="text-[9px] font-black uppercase tracking-[0.2em] text-secondary/70">
                  {testament}
                </span>
              </div>
              {BIBLE_DATA[testament as keyof typeof BIBLE_DATA].map((category) => (
                <div key={category.name} className="space-y-0.5">
                  <div className="px-8 py-2 text-[9px] font-black uppercase tracking-[0.15em] text-primary/25">
                    {category.name}
                  </div>
                      ...
                      <span
                        className={cn(
                          ...
                          activeBook?.abbr === book.abbr
                            ...
                            : "font-normal group-hover:translate-x-0.5",
                        )}
                      >
                        {book.name}
                      </span>
                      <span className="text-[9px] font-black tracking-widest opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 transition-opacity text-secondary/60">
                        {book.abbr}
                      </span>
                      {activeBook?.abbr === book.abbr && (
          ...
          <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-8 duration-1000 ease-out">
            <div className="p-16 border-b border-primary/5 flex flex-col items-center text-center bg-gradient-to-b from-white/20 to-transparent">
              <div className="mb-6 flex items-center gap-4">
                ...
                <span className="text-[10px] font-black tracking-[0.6em] text-secondary/60 uppercase">
                  LIVRO SAGRADO
                </span>
                <div className="h-px w-8 bg-secondary/20" />
              </div>
              <h3 className="font-display text-6xl text-primary/90 mb-4 tracking-tight">
                {activeBook.name}
              </h3>
            ...
            <div className="flex-1 overflow-y-auto p-12 no-scrollbar">
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
                {Array.from({ length: activeBook.chapters }, (_, i) => i + 1).map((num) => (
                  ...
                  >
                    <span className="font-display text-3xl text-primary/20 group-hover:text-secondary group-focus-visible:text-secondary transition-colors mb-0.5">
                      {num}
                    </span>
                    <span className="text-[7px] font-black uppercase tracking-[0.2em] text-primary/10 group-hover:text-primary/40 group-focus-visible:text-primary/40">
                      CAP
                    </span>
                  </button>
                ))}
