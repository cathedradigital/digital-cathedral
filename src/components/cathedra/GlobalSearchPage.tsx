...
const GlobalSearchPage = React.forwardRef<HTMLDivElement>((_props, ref) => {
  ...
  const saints = useFuzzySearch<Saint>({
  ...
  const glossary = useFuzzySearch<GlossaryRow>({
  ...
  const community = useFuzzySearch<CommunityPost>({
  ...
  const journeys = useFuzzySearch<Journey>({
  ...
  const errors = [saints.error, glossary.error, community.error, tags.error, journeys.error].filter(
    Boolean,
  ) as Error[];
  ...
  const { activeIndex: tagsActiveIndex, handleKeyDown: handleTagsKeyDown } = useRovingTabindex(
  ...
  return (
    <ContemplativeLayout maxW="max-w-4xl w-full">
      <SEOHead
          ...
          {hasBibleFilter && (
            ...
            >
              ...
              <p className="mt-spacing-xs text-premium-sm text-muted-foreground/80">
                {hasQuery ? (
                  <>
                    Mostrando resultados de <span className="text-secondary">"{query.trim()}"</span>{" "}
                    restritos à passagem selecionada.
                  </>
                ) : (
              ...
              <div className="mt-spacing-md flex flex-wrap items-center gap-spacing-xs">
                <Button
                  onClick={() =>
                    navigate(
                      `/bible?book=${encodeURIComponent(bibleLivro)}&chapter=${encodeURIComponent(bibleCapitulo)}`,
                    )
                  }
                  variant="default"
                  className="min-h-11"
                ...
                <Button onClick={clearBibleFilter} variant="ghost" className="min-h-11">
            ...
            <div className="mt-spacing-lg">
              <Tabs defaultValue="santos" className="mt-spacing-lg">
                <TabsList className="w-full flex-wrap h-auto gap-spacing-2xs bg-muted/50 p-spacing-2xs rounded-premium-full opacity-60">
                  <TabsTrigger value="santos" disabled className="text-premium-xs flex-1 min-h-11">
                    Santos (...)
                  </TabsTrigger>
                  <TabsTrigger
                    value="glossario"
                    disabled
                    className="text-premium-xs flex-1 min-h-11"
                  >
                    Glossário (...)
                  </TabsTrigger>
                  <TabsTrigger
          ...
          {hasError && (
            ...
            >
              ...
              <p className="text-premium-sm text-muted-foreground/70 max-w-[42ch] mx-auto">
                {errors[0]?.message?.slice(0, 160) ||
                  "Ocorreu um erro ao consultar a sabedoria. Tente novamente em instantes."}
              </p>
          ...
          {hasQuery && !isAllEmpty && (
            <Tabs defaultValue="santos" className="mt-spacing-lg">
              <TabsList className="w-full flex-wrap h-auto gap-spacing-2xs bg-muted/50 p-spacing-2xs rounded-premium-full">
                <TabsTrigger
                  value="santos"
                  className="text-premium-xs flex-1 min-w-[80px] min-h-11"
                >
                  Santos ({counts.santos})
                </TabsTrigger>
                <TabsTrigger
                  value="glossario"
                  className="text-premium-xs flex-1 min-w-[80px] min-h-11"
                >
                  Glossário ({counts.glossario})
                </TabsTrigger>
                <TabsTrigger
                  value="comunidade"
                  className="text-premium-xs flex-1 min-w-[80px] min-h-11"
                ...
                <TabsTrigger value="temas" className="text-premium-xs flex-1 min-w-[80px] min-h-11">
                  Temas ({counts.temas})
                </TabsTrigger>
                <TabsTrigger
                  value="jornadas"
              ...
              <TabsContent value="santos" className="space-y-spacing-xs mt-spacing-md">
                ...
                {saints.results?.length === 0 && !saints.isPending && (
                  <EmptyState text="Nenhum santo encontrado." />
                )}
              ...
              <TabsContent value="glossario" className="space-y-spacing-xs mt-spacing-md">
                ...
                {glossary.results?.length === 0 && !glossary.isPending && (
                  <EmptyState text="Nenhum termo encontrado." />
                )}
              ...
              <TabsContent value="comunidade" className="space-y-spacing-xs mt-spacing-md">
                ...
                {community.results?.length === 0 && !community.isPending && (
                  <EmptyState text="Nenhuma discussão encontrada." />
                )}
              ...
              <TabsContent value="temas" className="space-y-spacing-xs mt-spacing-md">
                <div className="flex flex-wrap gap-spacing-xs" role="list" ref={tagsRef}>
                  {tags.results?.map((t, idx) => (
                    <Button
                      ...
                      onKeyDown={(e) =>
                        handleTagsKeyDown(e, idx, () => navigate(`${AppRoute.TEMAS}/${t.slug}`))
                      }
                    ...
                    >
                      ...
                      <span className="text-premium-xs font-medium text-foreground group-hover:text-primary">
                        {t.label}
                      </span>
                ...
                {tags.results?.length === 0 && !tags.isPending && (
                  <EmptyState text="Nenhum tema encontrado." />
                )}
              ...
              <TabsContent value="jornadas" className="space-y-spacing-xs mt-spacing-md">
                ...
                {journeys.results?.length === 0 && !journeys.isPending && (
                  <EmptyState text="Nenhuma jornada encontrada." />
                )}
