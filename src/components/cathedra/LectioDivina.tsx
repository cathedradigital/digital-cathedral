...
import { useLectioProgress, getLectioProgress, getLastLectio } from "./lectio/useLectioProgress";
...
const LectioDivina: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedPassage && currentStep !== "intro" && currentStep !== "conclusio") {
      const match = selectedPassage.match(
        /^([a-zA-ZáéíóúÁÉÍÓÚ123]+)\s+(\d+)(?:,(\d+)(?:-(\d+))?)?$/,
      );
      if (match) {
              ...
              if (startVerse !== null) {
                if (endVerse !== null) {
                  verses = verses.filter(
                    (v: any) => v.number >= startVerse && v.number <= endVerse,
                  );
                } else {
    ...
    <>
      ...
      {showLectioNav && (
        <>
          ...
          <div className="fixed right-4 top-20 z-30 hidden md:block">{notesSheet}</div>
