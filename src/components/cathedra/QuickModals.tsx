...
export const BibleModal: React.FC<QuickModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setText("");
    supabase.functions.invoke("bible-text", { body: { book, chapter } }).then(({ data, error }) => {
    ...
    <ModalShell title="Bíblia — Consulta Rápida" onClose={onClose}>
      ...
      {loading ? (
        ...
        <div className="max-h-[50vh] overflow-y-auto pr-spacing-xs custom-scrollbar">
          <p className="font-serif text-foreground/90 leading-relaxed whitespace-pre-line text-premium-sm">
            {text}
          </p>
        </div>
      )}
      <div className="mt-spacing-lg pt-spacing-md border-t border-border">
        <Button
          onClick={() => {
            navigate(`${AppRoute.BIBLE}?book=${book}&ch=${chapter}`);
            onClose();
          }}
    ...
    <ModalShell title="Documentos — Consulta Rápida" onClose={onClose}>
      ...
      <div className="max-h-[50vh] overflow-y-auto space-y-spacing-2xs">
        {filtered.map((d) => (
          <div
            key={d.title}
            className="flex items-center justify-between p-spacing-sm rounded-premium hover:bg-muted transition-colors"
          >
            <div>
...
const ModalShell: React.FC<{ title: string; onClose: () => void; children: React.ReactNode }> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-spacing-lg bg-card border border-border rounded-premium shadow-premium-hover p-spacing-lg overflow-hidden animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-spacing-md">
          <h3
            id="modal-title"
            className="text-premium-sm font-black uppercase tracking-widest text-primary"
          >
            {title}
          </h3>
          <Button
...
const LoadingSkeleton = () => (
  <div className="space-y-spacing-sm py-spacing-md">
    {Array.from({ length: 4 }).map((_, i) => (
      <div
        key={i}
        className="h-spacing-md bg-muted rounded animate-pulse"
