...
const DeepContentSection: React.FC<DeepContentSectionProps> = ({ content, title, contentType }) => {
  ...
  const saveReflection = async () => {
    ...
    try {
      const { error } = await supabase.from("reading_reflections" as any).insert({
        user_id: user.id,
    ...
    const s = [
      {
        ...
        value: content.textoBase,
        isPremium: false,
      },
      {
        id: "explicacao",
        label: "Explicação",
        icon: <Icons.Info className="w-spacing-md h-spacing-md" />,
        value: content.explicacao,
        isPremium: true,
      },
      {
        id: "interpretacaoProfunda",
        label: "Sentido Profundo",
        icon: <Icons.Sparkle className="w-spacing-md h-spacing-md" />,
        value: content.interpretacaoProfunda,
        isPremium: true,
      },
      {
        id: "aplicacaoPratica",
        label: "Vida Prática",
        icon: <Icons.Zap className="w-spacing-md h-spacing-md" />,
        value: content.aplicacaoPratica,
        isPremium: true,
      },
      {
        id: "reflexaoFinal",
        ...
        value: content.reflexaoFinal,
        isPremium: true,
      },
      {
        id: "exercicio",
        label: "Exercício de Fé",
        icon: <Icons.PenTool className="w-spacing-md h-spacing-md" />,
        value: content.exercicio,
        isPremium: true,
      },
    ];
  ...
  return (
    <section
      className="space-y-spacing-xl animate-in fade-in slide-in-from-bottom-spacing-md duration-500"
      aria-label="Aprofundamento teológico"
    >
      {title && (
        <div className="flex items-center gap-spacing-sm mb-spacing-lg">
          <div className="h-px flex-1 bg-border/40" />
          <h3 className="text-premium-xs font-black uppercase tracking-[0.3em] text-primary whitespace-nowrap">
            {title}
          </h3>
          <div className="h-px flex-1 bg-border/40" />
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-spacing-sm mb-spacing-md">
                <div
                  className={`p-spacing-xs rounded-premium-full ${
                    isLocked
                ...
                <h4
                  className={`text-premium-xs font-black uppercase tracking-widest ${
                    isLocked
                ...
                >
                  ...
                  {section.isPremium && !isLocked && (
                    <span className="ml-spacing-xs px-spacing-2xs py-spacing-3xs rounded-premium-full bg-primary/10 text-primary text-premium-xs">
                      PRO
                    </span>
                  )}
                </h4>
              ...
              <div className="relative">
                {section.value ? (
                  <div
                    className={`font-serif leading-relaxed ${isLocked ? "blur-[6px] select-none pointer-events-none opacity-40" : ""} ${section.id === "textoBase" ? "text-premium-lg italic text-foreground" : "text-foreground/90 text-premium-sm"}`}
                  >
                    {section.value.split("\n\n").map((paragraph, pIdx) => (
                      <p key={pIdx} className={pIdx > 0 ? "mt-spacing-sm" : ""}>
                        {parseTheologicalReferences(paragraph).map((seg, sIdx) => {
                          ...
                          if (seg.type === "catechismRef") {
                            return <CatechismPopover key={sIdx} paragraph={seg.paragraph!} />;
                          }
                ...
                <div className="mt-spacing-lg pt-spacing-lg border-t border-border/40 space-y-spacing-md">
                  <div className="flex items-start gap-spacing-sm">
                    ...
                    <p className="text-premium-xs italic text-muted-foreground">
                      Silencie e deixe que esta pergunta ecoe em seu coração.
                    </p>
                  </div>
                  ...
                  {!hasReflected ? (
                    <div className="space-y-spacing-sm pt-spacing-xs">
                      ...
                      <div className="flex justify-end">
                        ...
                        >
                          {isSubmitting ? (
                            <Icons.Loader className="w-spacing-sm h-spacing-sm animate-spin" />
                          ) : (
                            <Icons.Send className="w-spacing-sm h-spacing-sm" />
                          )}
                          Guardar Reflexão
                        </Button>
                      </div>
                    ...
                    <div className="flex items-center gap-spacing-xs px-spacing-md py-spacing-xs bg-emerald-500/5 border border-emerald-500/20 rounded-premium-full text-emerald-600 text-[10px] font-black uppercase tracking-widest animate-in fade-in zoom-in-95 duration-500">
                      <Icons.CheckCircle2 className="w-spacing-sm h-spacing-sm" /> Reflexão
                      Integrada ao Diário
                    </div>
                  )}
                </div>
