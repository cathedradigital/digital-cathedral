    ...
    <>
      {segments.map((seg, i) => {
        ...
        if (seg.type === "catechismRef" && seg.paragraph) {
          return (
            <CatechismPopover key={i} paragraph={seg.paragraph} onNavigate={onNavigateCatechism} />
          );
        }
...
const CopyButton: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <Button
      onClick={handleCopy}
      className="mt-spacing-xs inline-flex items-center gap-spacing-2xs text-premium-xs text-muted-foreground hover:text-primary transition-colors"
    >
      {copied ? (
        <Icons.Check className="w-spacing-sm h-spacing-sm" />
      ) : (
...
const StudyMode: React.FC = () => {
  ...
  const handleNavigateToBible = useCallback(
    (abbr: string, chapter: number) => {
      navigate(`/bible?book=${abbr}&ch=${chapter}`);
    },
    [navigate],
  );

  const handleNavigateToCatechism = useCallback(
    (paragraph: number) => {
  ...
  const saveMessages = useCallback(
    async (conversationId: string, newMessages: Message[]) => {
      if (!user) return;
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: `⚠️ ${e.message || "Erro ao consultar a IA. Tente novamente."}`,
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-spacing-2xl max-w-spacing-2xl mx-auto py-spacing-2xl">
      ...
      <div className="space-y-spacing-md">
        <h1 className="text-premium-3xl md:text-premium-5xl font-serif font-black text-primary">
          Estudo e Verdade
        </h1>
        <p className="text-premium-lg text-muted-foreground font-serif italic">
          "Para garantir a integridade absoluta da doutrina e a soberania da sua experiência
          espiritual, a Cathedra Digital optou por não utilizar serviços de Inteligência
          Artificial."
        </p>
      </div>
      ...
      <div className="bg-card border border-border p-spacing-xl rounded-[2.5rem] shadow-premium-md space-y-spacing-lg">
        <p className="text-premium-sm text-foreground/80 leading-relaxed">
          O <strong>Modo Estudo</strong> está sendo reformulado para focar exclusivamente em{" "}
          <strong>Curadoria Humana</strong> e <strong>Fontes Oficiais</strong> da Igreja, permitindo
          que você navegue pela Tradição e pelo Magistério com total segurança.
        </p>
