...
const ReadingJournal: React.FC = () => {
  ...
  const exportPDF = () => {
    ...
    notes.forEach((note, i) => {
      ...
      doc.text(
        `${note.content_type.toUpperCase()} - ${new Date(note.created_at).toLocaleDateString()}`,
        20,
        y,
      );
      y += 7;
  ...
  return (
    <div className="max-w-spacing-4xl mx-auto py-spacing-2xl px-spacing-md space-y-spacing-xl min-h-screen pb-spacing-4xl">
      <SEOHead
        title="Diário & Histórico | Cathedra"
        description="Gerencie suas marcas de leitura, histórico e anotações espirituais."
        path="/diario"
      />
      ...
      <div className="text-center space-y-spacing-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-spacing-lg max-w-spacing-2xl mx-auto w-full">
          <Card className="bg-primary/[0.03] border-primary/10 rounded-[3rem] overflow-hidden shadow-premium-md group hover:bg-primary/[0.05] transition-all">
            <CardContent className="p-spacing-xl flex flex-col items-center text-center gap-spacing-md">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary/40 mb-spacing-xs">
                  Sequência de Graça
                </p>
                <h3 className="text-premium-5xl font-display text-primary">{streak}</h3>
                <p className="text-premium-xs font-bold text-primary/60 mt-spacing-2xs uppercase tracking-widest">
              ...
              <div className="pt-spacing-md border-t border-primary/5 w-full">
                <p className="text-[9px] font-black uppercase tracking-widest text-primary/60">
                  Recorde: {profile?.max_streak || streak} dias
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="bg-secondary/[0.02] border-secondary/10 rounded-[4rem] overflow-hidden shadow-premium-md group hover:bg-secondary/[0.04] transition-all duration-1000">
              <CardContent className="p-spacing-2xl flex flex-col items-center gap-spacing-xl">
                <div className="w-spacing-4xl h-spacing-4xl rounded-premium-full bg-secondary/5 flex items-center justify-center relative">
                  <Icons.Target
                    className={`w-spacing-2xl h-spacing-2xl text-secondary/60 group-hover:rotate-12 transition-transform duration-700 ${daysActiveThisWeek >= weeklyGoal ? "animate-bounce" : ""}`}
                  />
                ...
                <div className="w-full text-center space-y-spacing-md">
                  <p className="text-[10px] font-black uppercase tracking-[0.5em] text-secondary/30">
                    Meta da Semana
                  </p>
                  <h3 className="text-premium-6xl font-display text-secondary leading-none">
                    {daysActiveThisWeek}{" "}
                    <span className="text-premium-2xl opacity-20">/ {weeklyGoal}</span>
                  </h3>
                  <p className="text-premium-sm font-serif italic text-secondary/60">
                    Dias em contemplação
                  </p>
                </div>
                <div className="w-full space-y-spacing-md">
                  <div className="h-spacing-xs w-full bg-secondary/5 rounded-premium-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${Math.min(100, (daysActiveThisWeek / weeklyGoal) * 100)}%`,
                      }}
                  ...
                  <div className="pt-spacing-lg flex justify-between px-spacing-xs">
                    {["D", "S", "T", "Q", "Q", "S", "S"].map((day, i) => {
                      ...
                      const isActive = [...marks, ...notes].some(
                        (item) =>
                          new Date(item.created_at || (item as any).updated_at)
                            .toISOString()
                            .split("T")[0] === dateStr,
                      );
                      ...
                      return (
                        <div key={i} className="flex flex-col items-center gap-spacing-sm">
                          <span
                            className={`text-[8px] font-black uppercase tracking-widest ${isToday ? "text-secondary" : "text-secondary/20"}`}
                          >
                            {day}
                          </span>
                          <div
                            className={`w-spacing-xl h-spacing-xl rounded-premium-full border flex items-center justify-center transition-all duration-700 ${
                              isActive
                          ...
                          >
                            {isActive && (
                              <Icons.CheckCircle2 className="w-spacing-sm h-spacing-sm" />
                            )}
                          </div>
                        </div>
                      );
                  ...
                  <p className="text-[9px] font-black uppercase tracking-[0.3em] text-secondary/20 text-center pt-spacing-md">
                    {daysActiveThisWeek >= weeklyGoal
                      ? "Propósito cumprido!"
                      : `Siga firme por mais ${weeklyGoal - daysActiveThisWeek} dias.`}
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <h1 className="text-premium-4xl md:text-premium-5xl font-display text-primary">
          Diário de Jornada
        </h1>
        <p className="text-muted-foreground font-serif italic">
          "Guarda o que viste, para que não se apague do teu coração."
        </p>
      </div>
      ...
      <Tabs defaultValue="history" className="w-full" onValueChange={setActiveTab}>
        <div className="flex items-center justify-between gap-spacing-md mb-spacing-xl">
          <TabsList className="bg-muted/50 p-spacing-2xs rounded-premium-full">
            <TabsTrigger
              value="history"
              className="rounded-premium-full px-spacing-lg py-spacing-xs text-premium-xs font-bold uppercase tracking-widest data-[state=active]:bg-primary data-[state=active]:text-white"
            >
              Histórico
            </TabsTrigger>
            <TabsTrigger
          ...
          {activeTab === "notes" && notes.length > 0 && (
            <Button
              ...
              variant="outline"
              size="sm"
              className="rounded-premium-full gap-spacing-xs text-premium-xs uppercase tracking-widest font-bold border-primary/20 hover:bg-primary/5"
            >
        ...
        <TabsContent value="history" className="space-y-spacing-md">
          {filteredMarks.length === 0 ? (
            <div className="text-center py-spacing-3xl bg-muted/20 rounded-[2rem] border-2 border-dashed border-border/40">
              ...
              <p className="text-muted-foreground font-serif italic">
                Nenhuma marca de leitura encontrada.
              </p>
            </div>
          ) : (
            filteredMarks.map((mark) => (
              <Card
                key={mark.id}
                className="group overflow-hidden hover:border-primary/30 transition-all border-border/40 bg-card/60 backdrop-blur-sm rounded-premium shadow-premium-md hover:shadow-premium"
              >
                  ...
                  <div className="flex items-center gap-spacing-md min-w-spacing-0">
                    <div
                      className={`w-spacing-xl h-spacing-xl rounded-premium-full flex items-center justify-center shrink-0 ${
                        mark.content_type === "bible"
                          ? "bg-blue-500/10 text-blue-600"
                          : mark.content_type === "catechism"
                    ...
                    <div className="min-w-spacing-0">
                      <div className="flex items-center gap-spacing-xs mb-spacing-3xs">
                        ...
                        {mark.is_last_read && (
                          <Badge
                            variant="secondary"
                            className="bg-primary/10 text-primary border-primary/20 text-[8px] h-spacing-md"
                          >
                            ÚLTIMO PONTO
                          </Badge>
                        )}
                      ...
                      <div className="flex items-center gap-spacing-xs text-[10px] text-muted-foreground uppercase tracking-widest font-medium">
                        ...
                        {new Date(mark.updated_at).toLocaleDateString("pt-BR")}
                      </div>
                    </div>
                  </div>
        ...
        <TabsContent value="notes" className="space-y-spacing-md">
          {filteredNotes.length === 0 ? (
            ...
            filteredNotes.map((note) => (
              <Card
                key={note.id}
                className="overflow-hidden hover:border-primary/30 transition-all border-border/40 bg-card rounded-premium shadow-premium-md"
              >
                <CardContent className="p-spacing-lg space-y-spacing-md">
                  <div className="flex items-start justify-between gap-spacing-md">
                    <div className="flex items-center gap-spacing-xs">
                      <Badge
                        variant="outline"
                        className="bg-primary/5 text-primary border-primary/20 text-[9px] font-black uppercase tracking-widest"
                      >
                        {note.content_type}
                      </Badge>
                      ...
                      onClick={() => {
                        const url =
                          note.content_type === "bible"
                            ? `/bible?book=${note.book_abbr}&ch=${note.chapter}`
                            : note.content_type === "catechism"
                              ? `/catechism?p=${note.paragraph}`
                              : "/";
                        navigate(url);
                      }}
                    >
