...
const OrdinaryBlockView: React.FC<{
  block: PrayerBlock;
  anchorId: string;
  celebrationMode?: boolean;
  ...
  return (
    <article id={anchorId} data-block-id={anchorId} className="scroll-mt-24 py-spacing-sm">
      {block.title && (
      ...
      {block.rubric && (
        <p className="mb-spacing-2xs font-serif italic text-premium-xs text-primary/80">
          {block.rubric}
        </p>
      )}
      {block.body && (
...
const HourSection: React.FC<{ hour: BreviaryHourBundle; celebrationMode?: boolean }> = ({
  ...
  return (
    <section
      id={`hour-${hourSlug}`}
      aria-labelledby={`hour-${hourSlug}-title`}
    ...
    >
      <header className="text-center border-b border-border/50 pb-spacing-md mb-spacing-md">
        ...
        <h2
          ...
          className={cn(
            ...
            celebrationMode
              ? "text-premium-4xl md:text-premium-5xl"
              : "text-premium-2xl md:text-premium-3xl",
          )}
        >
          {title}
        </h2>
      ...
      {/* Salmodia (Próprio) */}
      {office?.psalmody?.length
        ? office.psalmody.map((p: OfficePsalmody, idx) => (
            <SlotCard
              key={`psalm-${idx}`}
...
export const BreviaryContinuousReader: React.FC<Props> = ({
  ...
  const heroTitle =
    hours.length > 1 ? "Liturgia das Horas" : (primaryHour?.title ?? "Liturgia das Horas");
  const heroKicker =
    hours.length > 1
  ...
  return (
    <ReaderShell
      hero={<EditorialHero kicker={heroKicker} title={heroTitle} align="center" size="md" />}
      headerContext={
        <LiturgicalContext
          date={dateLabel}
      ...
      continuation={(() => {
        const closure = resolveEditorialClosure(
          prayer as unknown as { editorial_closure?: unknown },
        );
