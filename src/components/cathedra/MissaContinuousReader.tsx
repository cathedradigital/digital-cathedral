...
const PrayerBlockView: React.FC<{
  block: PrayerBlock;
  index: number;
  celebrationMode?: boolean;
}> = ({ block, index, celebrationMode }) => {
  const isMainPrayer =
    block.kind === "prayer" || block.kind === "intro" || block.kind === "closing";
...
export const MissaContinuousReader: React.FC<Props> = ({
  ...
  const selectOption = (group: string, key: string) => {
    ...
    try {
      localStorage.setItem(OPTION_KEY(group), key);
    } catch {}
  };
  ...
  const renderGroupedItem = (item: GroupedItem, index: number): React.ReactNode => {
    ...
    const groupMeta = GROUP_META[item.optionGroup] ?? {
      kicker: item.optionGroup,
      title: "Escolha",
    };
  ...
  const renderSlotsAfterSection = (sectionSlug: string) => {
    ...
    if (sectionSlug === "comunhao")
      return <PostCommunionSlot proper={proper} loading={properLoading} />;
    return null;
  };
