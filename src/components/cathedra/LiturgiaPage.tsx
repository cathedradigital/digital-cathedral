...
const LiturgiaPage: React.FC = () => {
  ...
  const {
    ...
    isFetching: isMeditationFetching,
    retry: retryMeditation,
  } = useLiturgyMeditation(selectedIso, readings ?? null);
    ...
    <div className="flex flex-col md:flex-row w-full min-h-screen">
            ...
            <div className="flex items-center gap-spacing-xs px-spacing-md py-spacing-2xs rounded-full border border-primary/10 bg-background/50 backdrop-blur-sm">
              <div
                className={cn(
                  ...
                  String(readings.colorToken) === "green"
                    ? "bg-green-600"
                    : String(readings.colorToken) === "red"
                      ? "bg-red-600"
                      : String(readings.colorToken) === "white"
                        ? "bg-stone-200"
                        : String(readings.colorToken) === "violet"
                          ? "bg-violet-700"
                          : "bg-secondary",
                )}
              />
              <span className="text-[9px] font-black uppercase tracking-widest text-primary/60">
                {readings.cor || readings.colorToken}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border/40 py-spacing-sm px-spacing-md flex justify-center">
          <div
            className="bg-muted/40 p-spacing-2xs rounded-[2.5rem] border border-border/40 flex gap-spacing-2xs w-full max-w-lg shadow-premium-md"
            role="tablist"
            aria-label="Navegação da Liturgia"
          >
            {[
              {
                id: "liturgia",
              ...
              <Button
                ...
                {...getTabProps(
                  ...
                  `panel-${tab.id}`,
                  activeTab === tab.id,
                  `flex-1 flex items-center justify-center gap-spacing-2xs px-spacing-xs py-spacing-sm rounded-premium-full text-[10px] sm:text-premium-sm font-black uppercase tracking-wider transition-all focus-visible:ring-2 focus-visible:ring-primary outline-none min-w-0 ${
                    activeTab === tab.id
                      ? "bg-background shadow-premium-hover text-primary hover:bg-background"
                ...
                onKeyDown={(e) =>
                  handleTabKeyDown(
                    ...
                    idx,
                    3,
                    (newIdx) => setSearchParams({ tab: tabList[newIdx] }),
                    "tab-",
                  )
                }
              >
          ...
          <Suspense
            fallback={
              <div className="flex justify-center py-spacing-3xl">
                <Icons.Loader2 className="w-spacing-xl h-spacing-xl text-secondary animate-spin" />
              </div>
            }
          >
            {activeTab === "liturgia" && (
              <ReaderShell
                ...
                nexus={
                  nexus ? (
                    <NexusPanel output={nexus} kicker={`Conexões de ${formattedDate}`} />
                  ) : undefined
                }
                continuation={
                  readings && readings.evangelho ? (
                    <div className="mt-spacing-xl">
                      <ReaderContinuation
                        ...
                        suggestions={
                          nexus?.suggestions.length && nexus.suggestions.length > 0
                            ? nexus.suggestions
                            : undefined
                        }
                      />
              ...
              >
                <div
                  {...getTabPanelProps(
                    ...
                    "tab-liturgia",
                    activeTab === "liturgia",
                    "w-full space-y-spacing-xl animate-in fade-in duration-500 outline-none",
                  )}
                >
                  <div className="space-y-spacing-md">
                    ...
                    {isOfflineData && (
                      <div className="flex items-center justify-center gap-spacing-xs text-premium-xs font-black uppercase tracking-widest text-muted-foreground bg-muted/50 rounded-premium px-spacing-md py-spacing-xs mx-auto w-fit">
                        <Icons.WifiOff className="w-spacing-sm h-spacing-sm" />{" "}
                  ...
                  {profile?.diocese && (
                    <motion.div
                      ...
                      className="bg-secondary/5 border border-secondary/20 rounded-premium p-spacing-md flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-spacing-sm">
                        ...
                        <div>
                          <p className="text-premium-xs font-black uppercase tracking-widest text-secondary/60">
                            Sua Diocese
                          </p>
                          <h3 className="text-premium-sm font-bold text-primary">
                            {profile.diocese}
                          </h3>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-premium-xs font-medium text-muted-foreground uppercase tracking-widest">
                          Estado
                        </p>
                        <p className="text-premium-xs font-bold text-primary">{profile.estado}</p>
                      </div>
                    </motion.div>
                  )}
                    ...
                    <div className="space-y-spacing-xl">
                      {readings.primeiraLeitura && (
                        <LiturgyReadingCard
                          ...
                          onOpenBible={() =>
                            navigate(parseRefToRoute(readings.primeiraLeitura!.referencia))
                          }
                          onOpenLectio={() =>
                            navigateToLectio(readings.primeiraLeitura!.referencia)
                          }
                          delay={0.1}
                        />
                      )}
                      ...
                      {readings.segundaLeitura && (
                        <LiturgyReadingCard
                          ...
                          onOpenBible={() =>
                            navigate(parseRefToRoute(readings.segundaLeitura!.referencia))
                          }
                      ...
                      {readings.evangelho && (
                        <LiturgyReadingCard
                          ...
                          onOpenBible={() =>
                            navigate(parseRefToRoute(readings.evangelho!.referencia))
                          }
                  ...
                  {saint && (
                    <motion.div
                      ...
                      className="bg-muted/30 border border-border rounded-[2rem] p-spacing-xl flex flex-col items-center text-center space-y-spacing-md"
                    >
                      <div className="w-spacing-3xl h-spacing-3xl rounded-premium-full overflow-hidden border-2 border-secondary p-spacing-2xs shadow-premium shadow-secondary/10">
                        <SacredImage
                          ...
                          className="w-full h-full object-cover rounded-premium-full"
                        />
                      </div>
                      <div className="space-y-spacing-2xs">
                        ...
                        <h3 className="text-premium-xl font-display font-black text-primary">
                          {saint.name}
                        </h3>
                        {saint.title && (
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="text-muted-foreground hover:bg-primary/5 h-spacing-xl"
                        onClick={() =>
                          navigate(
                            saint.slug ? `${AppRoute.SAINTS}/${saint.slug}` : AppRoute.SAINTS,
                          )
                        }
                      >
                        Conhecer História{" "}
                  ...
                  {prayerOfDay && (
                    <motion.div
                      ...
                      className="premium-card p-spacing-xl space-y-spacing-md relative overflow-hidden group"
                    >
                      <div className="absolute top-0 right-0 p-spacing-xl opacity-5 group-hover:opacity-10 transition-opacity pointer-events-none">
                        <Icons.Church className="w-spacing-3xl h-spacing-3xl" />
                      </div>
                      <div className="flex items-center gap-spacing-sm relative z-10">
                        ...
                        <div>
                          ...
                          {prayerOfDay.kicker && (
                            <p className="text-premium-xs font-medium text-muted-foreground mt-spacing-3xs">
                              {prayerOfDay.kicker}
                            </p>
                          )}
                        </div>
                      ...
                      <div className="relative z-10">
                        <h3 className="text-premium-xl md:text-premium-2xl font-display font-black text-primary">
                          {prayerOfDay.title}
                        </h3>
                        {prayerOfDay.subtitle && (
                          <p className="text-premium-sm font-serif italic text-muted-foreground mt-spacing-2xs">
                            {prayerOfDay.subtitle}
                          </p>
                        )}
                      </div>
                      <div className="flex items-center justify-between pt-spacing-md border-t border-border/40 relative z-10">
                        {prayerOfDay.estimated_seconds ? (
                          <span className="text-premium-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-spacing-2xs">
                            <Icons.Clock className="w-spacing-sm h-spacing-sm" />{" "}
                            {Math.max(1, Math.round(prayerOfDay.estimated_seconds / 60))} min
                          </span>
                        ) : (
                          <span />
                        )}
                        <Button
                          variant="secondary"
                          size="sm"
                          className="rounded-premium-full h-spacing-xl px-spacing-xl bg-secondary/10 border-none hover:bg-secondary/20 text-primary shadow-premium-md"
                          onClick={() => navigate(`/oracao/${prayerOfDay.slug}?from=liturgia`)}
                        >
                          Rezar agora{" "}
                          <Icons.ChevronRight className="w-spacing-md h-spacing-md ml-spacing-xs" />
                        </Button>
            ...
            {activeTab === "missal" && (
              <div
                ...
                className="animate-in fade-in slide-in-from-bottom-spacing-md duration-500 outline-none pt-spacing-xl max-w-5xl mx-auto"
                tabIndex={0}
              >
                <MissalPage />
              </div>
