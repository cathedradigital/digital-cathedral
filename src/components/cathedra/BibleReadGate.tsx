...
export const BibleReadGate: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  if (gate?.blocked && gate.status === "error") {
    ...
    const isNetworkError =
      gate.status === "error" &&
      (String(gate.blocking_findings || "").includes("fetch") || !navigator.onLine);
        ...
        <div className="max-w-md w-full rounded-lg border border-destructive/20 bg-card text-card-foreground p-6 shadow-premium-sm">
          <div className="flex items-start gap-3">
            <ShieldAlert
              className={cn(
                "h-6 w-6 shrink-0",
                isNetworkError ? "text-destructive" : "text-amber-600",
              )}
              aria-hidden
            />
            <div className="space-y-2">
              ...
              {!isNetworkError && (
                <div className="text-xs text-muted-foreground border-t border-border pt-2 mt-2 space-y-0.5">
                  <div>
                    <span className="font-medium">Status:</span> {gate.status}
                  </div>
                  <div>
                    <span className="font-medium">Última verificação:</span> {lastRun}
                  </div>
                  <div>
                    <span className="font-medium">Pendências:</span> {gate.blocking_findings}
                  </div>
                </div>
              )}
