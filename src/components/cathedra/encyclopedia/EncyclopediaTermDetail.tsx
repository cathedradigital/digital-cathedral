      ...
      <AnimatePresence mode="wait">
        {!selectedTerm ? (
          ...
          >
            ...
            <h3 className="text-premium-xl font-bold text-foreground mb-spacing-xs">
              Selecione um termo
            </h3>
            <p className="text-muted-foreground text-premium-sm">
              Escolha um termo da lista para explorar sua profundidade teológica.
            </p>
          </motion.div>
        ) : (
          <motion.div
          ...
          >
            ...
            <div>
              <Badge
                variant="secondary"
                className="bg-primary/10 text-primary border-none text-premium-xs uppercase tracking-[0.15em] font-black mb-spacing-xs"
              >
            ...
            {(selectedTerm.bibleVerses || selectedTerm.reference) && (
              <div className="bg-blue-500/5 border border-blue-500/10 rounded-premium p-spacing-md space-y-spacing-xs">
                <div className="flex items-center gap-spacing-xs">
                  ...
                  <p className="text-premium-xs font-black uppercase tracking-widest text-blue-600">
                    📖 Bíblia
                  </p>
                </div>
                  ...
                  {(selectedTerm.bibleVerses || [selectedTerm.reference!]).flatMap((v, vIdx) => {
                    ...
                    return (
                      <Badge
                        key={vIdx}
                        variant="outline"
                        className="bg-blue-500/5 text-blue-700 dark:text-blue-400 border-blue-500/20 rounded-premium-full text-premium-xs font-semibold"
                      >
                        {v}
                      </Badge>
                    );
            ...
            {selectedTerm.catechismReferences && (
              <div className="bg-amber-500/5 border border-amber-500/10 rounded-premium p-spacing-md space-y-spacing-xs">
                <div className="flex items-center gap-spacing-xs">
                  ...
                  <p className="text-premium-xs font-black uppercase tracking-widest text-amber-600">
                    📘 Catecismo
                  </p>
                </div>
                <div className="flex flex-wrap gap-spacing-xs">
                  {selectedTerm.catechismReferences.flatMap((r, rIdx) => {
                    ...
                    return (
                      <Badge
                        key={rIdx}
                        variant="outline"
                        className="bg-amber-500/5 text-amber-700 dark:text-amber-400 border-amber-500/20 rounded-premium-full text-premium-xs font-semibold"
                      >
                        {r}
                      </Badge>
                    );
            ...
            {selectedTerm.magisteriumReferences && (
              <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-premium p-spacing-md space-y-spacing-xs">
                <div className="flex items-center gap-spacing-xs">
                  ...
                  <p className="text-premium-xs font-black uppercase tracking-widest text-emerald-600">
                    📜 Magistério
                  </p>
                </div>
                <div className="flex flex-wrap gap-spacing-xs">
                  {selectedTerm.magisteriumReferences.map((m, idx) => {
                    ...
                    return (
                      <MagisteriumPopover
                        ...
                        label={m}
                        onNavigate={(search) =>
                          navigate(`/magisterio?search=${encodeURIComponent(search)}`)
                        }
                      />
                    );
                  })}
            ...
            <div className="flex flex-col gap-spacing-sm pt-spacing-xs">
              ...
              >
                {selectedTerm.journey_id ? (
                  <Icons.Compass className="w-spacing-md h-spacing-md" />
                ) : (
