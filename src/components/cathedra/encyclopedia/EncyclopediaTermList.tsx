...
const EncyclopediaTermList: React.FC<EncyclopediaTermListProps> = ({
  ...
  return (
    <div className="md:col-span-4 space-y-spacing-2xs max-h-[60vh] overflow-y-auto pr-spacing-xs scrollbar-thin">
      {terms.length === 0 && (
        <p className="text-center text-muted-foreground text-premium-sm py-spacing-xl italic">
      ...
      {terms.map((t) => {
        ...
        return (
          ...
          >
            ...
            <Icons.ChevronRight
              className={`w-spacing-md h-spacing-md shrink-0 transition-transform ${isActive ? "text-primary rotate-90" : "text-muted-foreground/40 group-hover:text-muted-foreground"}`}
            />
          </Button>
        );
      })}
