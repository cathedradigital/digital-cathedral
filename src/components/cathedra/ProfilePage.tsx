...
import {
  ...
  ESTADO_NOME,
  DIOCESES_POR_ESTADO,
  MOVIMENTOS_PASTORAIS,
} from "@/data/dioceses-brasil";
      ...
      <div className="flex items-start justify-between gap-spacing-md">
        <div className="flex items-center gap-spacing-md">
          <div className="w-spacing-3xl h-spacing-3xl rounded-premium-full bg-primary/10 border-2 border-primary/30 flex items-center justify-center shrink-0">
            <span className="text-premium-2xl" aria-hidden="true">
              🔥
            </span>
          </div>
          <div>
      ...
      <div className="grid grid-cols-3 gap-spacing-sm pt-spacing-2xs">
        {STREAK_MILESTONES.map((m) => {
          ...
          return (
            ...
            >
              <p
                className={`text-premium-lg mb-spacing-2xs ${unlocked ? "" : "grayscale"}`}
                aria-hidden="true"
              >
                {m.badge}
              </p>
...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    if (profile) {
      ...
      supabase
        .from("profiles")
        .select("bio, estado, diocese, paroquia, movimento_pastoral")
        .eq("id", profile.id)
        .single()
        .then(({ data }) => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      const [postsRes, likesRes, notesRes, historyRes, donRes, auditRes, achRes] =
        await Promise.all([
          supabase
      ...
      const uniqueDays = new Set((historyRes.data || []).map((h: any) => h.visited_at.slice(0, 10)))
        .size;
      setStats({
      ...
      ((achRes.data as any[]) || []).forEach((a) => {
        map[a.achievement_id] = a.earned_at;
      });
      setEarnedMap(map);
    };
    fetchAll();
  ...
  const filteredActivity = useMemo(
    () =>
      activityFilter === "all"
        ? activityItems
        : activityItems.filter((i) => i.kind === activityFilter),
  ...
  const handleSave = async () => {
    ...
    try {
      try {
        ...
        console.error("BG Push update failed:", err);
      }
          ...
          {showLevelUp && (
            ...
            >
              ...
              >
                ...
                <h2 className="text-premium-xl font-black text-foreground mb-spacing-2xs">
                  Nível Alcançado!
                </h2>
                <p className="text-premium-2xl font-black text-primary mb-spacing-xs">
                  {levelName}
                </p>
                <p className="text-premium-xs text-foreground/70">
                  Nível {currentLevelIdx + 1} · {totalXp} XP
                </p>
              </motion.div>
            </motion.div>
          )}
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-spacing-lg">
            <div className="relative w-spacing-4xl h-spacing-4xl group shrink-0">
              <Avatar className="w-spacing-3xl h-spacing-3xl sm:w-spacing-4xl sm:h-spacing-4xl border-4 border-primary/20 shrink-0">
                ...
                <AvatarFallback className="text-premium-2xl font-black bg-foreground text-background">
                  {initials}
                </AvatarFallback>
              </Avatar>
              <button
            ...
            <div className="flex-1 min-w-0 text-center sm:text-left space-y-spacing-sm w-full">
              <div>
                <h1 className="text-premium-2xl font-black text-foreground truncate">
              ...
              <div className="space-y-spacing-2xs">
                <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-foreground/75">
                  <span>
                    Nível {currentLevelIdx + 1} · {levelName}
                  </span>
                ...
                <p className="text-[10px] text-foreground/70">
                  {nextLevel
                    ? `${nextLevel.minXp - totalXp} XP para ${nextLevel.name}`
        ...
        <Tabs defaultValue="overview" className="space-y-spacing-lg">
          ...
          <TabsContent value="overview" className="space-y-spacing-lg mt-0">
            ...
            >
              <div className="flex items-center gap-spacing-md">
                ...
                <div>
                  ...
                  <p className="text-premium-xs text-foreground/70">
                    Versículos, orações e trechos salvos
                  </p>
                </div>
              </div>
            ...
            <CathedraCard className="p-spacing-xl space-y-spacing-md">
              <h2 className="text-premium-xs font-black uppercase tracking-widest text-foreground/75">
                Minhas Doações & Apoio
              </h2>
              <p className="text-premium-xs text-foreground/75 leading-relaxed italic">
                "Sua contribuição é o que nos permite continuar levando a Luz da Verdade a milhares
                de corações."
              </p>
                  ...
                  <li key={b.id} className="flex items-start gap-spacing-md py-spacing-md">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-spacing-sm flex-wrap">
                        <p
                          className={`text-premium-sm font-bold ${b.unlocked ? "text-foreground" : "text-foreground/60"}`}
                        >
                          {b.label}
                        </p>
                        <span
                      ...
                      <p className="text-[11px] text-foreground/70 mt-spacing-2xs">
                        {b.description}
                      </p>
          ...
          <TabsContent value="activity" className="space-y-spacing-lg mt-0">
            <CathedraCard className="p-spacing-xl space-y-spacing-lg">
              ...
              <div
                className="flex gap-spacing-2xs flex-wrap"
                role="tablist"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between gap-spacing-sm">
                          ...
                          {typeof item.amount === "number" && (
                            <p className="text-premium-sm font-black text-primary tabular-nums">
                              {fmtBRL(item.amount)}
                            </p>
                          )}
                        </div>
              ...
              <CathedraCard className="p-spacing-xl space-y-spacing-lg">
                ...
                <div className="space-y-spacing-md">
                  ...
                  <div className="flex items-center justify-between p-spacing-sm bg-primary/5 rounded-premium border border-primary/20">
                    <div className="space-y-spacing-2xs">
                      <div className="flex items-center gap-spacing-xs flex-wrap">
                        ...
                        <span className="px-spacing-2xs py-spacing-3xs rounded-premium-full bg-primary text-primary-foreground text-[8px] font-black uppercase tracking-wider">
                          Novo
                        </span>
                      </div>
                      <p className="text-[10px] text-foreground/75 font-medium">
                        Meditações e avisos direto no seu WhatsApp.
                      </p>
                    </div>
                    <Switch
                  ...
                  {whatsappEnabled && (
                    ...
                    >
                      <label className="text-[10px] font-black uppercase tracking-widest text-foreground/75 pl-spacing-2xs">
                        Número (com DDD)
                      </label>
                      <div className="relative">
                        <span className="absolute left-spacing-md top-1/2 -translate-y-1/2 text-foreground/70 text-premium-sm font-bold">
                          +55
                        </span>
                        <input
                          type="tel"
                  ...
                  <div className="flex items-center justify-between p-spacing-sm bg-muted/30 rounded-premium border border-border/50">
                    <div className="space-y-spacing-2xs">
                      ...
                      <p className="text-[10px] text-foreground/70">
                        Sua jornada diária começa aqui.
                      </p>
                    </div>
                    <input
                      type="time"
                  ...
                  <div className="flex items-center justify-between p-spacing-sm bg-muted/30 rounded-premium border border-border/50">
                    ...
                    <div className="flex items-center gap-spacing-xs">
                      <span className="text-premium-sm font-bold text-primary tabular-nums">
                        {weeklyGoal}d
                      </span>
                      <input
                        type="range"
              ...
              <CathedraCard className="p-spacing-xl space-y-spacing-md">
                ...
                <div className="border-t border-border pt-spacing-md space-y-spacing-md">
                  <div className="flex items-center gap-spacing-xs">
                    ...
                    <h3 className="text-premium-xs font-black uppercase tracking-widest text-foreground/75">
                      Localização Eclesial
                    </h3>
                  </div>
                  <p className="text-[10px] text-foreground/70 -mt-spacing-xs">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-spacing-md">
                    ...
                    <div className="space-y-spacing-2xs">
                      ...
                      >
                        <option value="">
                          {estado ? "Selecione a diocese..." : "Selecione o estado primeiro"}
                        </option>
                        {estado &&
                          DIOCESES_POR_ESTADO[estado]?.map((d) => (
                    ...
                    <div className="space-y-spacing-2xs">
                      <label className="text-premium-xs font-bold text-foreground">
                        Movimento / Pastoral
                      </label>
                      <select
