...
const LogosAI: React.FC<LogosAIProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      variant === "integrated" &&
      context &&
      context === lastLoadedContextRef.current
    ) {
      if (history.length > 0) {
    ...
    (skipConfirm = false) => {
      ...
      if (!skipConfirm) {
        const confirmed = window.confirm(
          "Deseja redefinir o silêncio e limpar o histórico desta seção?",
        );
        if (!confirmed) return;
      }
  ...
  const exportHistory = React.useCallback(() => {
    ...
    downloadAnchorNode.setAttribute(
      "download",
      `logos_ia_history_${context || "geral"}_${new Date().getTime()}.json`,
    );
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <CathedraCard
              padding="none"
              className="bg-card/40 backdrop-blur-3xl border-primary/[0.05] p-spacing-lg md:p-spacing-2xl lg:p-spacing-3xl space-y-spacing-md md:space-y-spacing-xl relative overflow-hidden shadow-premium rounded-[3rem] w-full"
            >
              ...
              <div className="absolute top-spacing-md right-spacing-md md:top-spacing-lg md:right-spacing-lg flex items-center gap-spacing-xs">
                ...
                <CathedraButton
                  ...
                  size="sm"
                  onClick={onClose}
                  aria-label="Fechar Logos AI"
                  className="rounded-premium-full text-primary/60 hover:text-primary transition-colors h-spacing-xl w-spacing-xl px-spacing-0"
                  icon={<Icons.X className="w-spacing-sm h-spacing-sm" />}
                />
              </div>
              ...
              <div className="flex items-center justify-between mb-spacing-lg md:mb-spacing-xl opacity-30">
                <div className="flex items-center gap-spacing-sm md:gap-spacing-md">
                  ...
                  <div>
                    <h4 className="text-[8px] font-black uppercase tracking-[0.6em] text-primary/40">
                      Logos IA
                    </h4>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-spacing-2xs">
                  <div
                    className={`w-spacing-2xs h-spacing-2xs rounded-premium-full ${
                      settings.totalSilence
                        ? "bg-red-400"
                        : history.length > 0
              ...
              <div className="space-y-spacing-md md:space-y-spacing-xl w-full">
                ...
                {history.slice(-visibleMessages).map((msg, i) => (
                  ...
                  >
                    <div
                      className={`max-w-full text-premium-sm md:text-premium-lg leading-[1.7] md:leading-[1.8] tracking-wide ${
                        msg.role === "user"
                    ...
                    >
                      ...
                      {msg.role === "assistant" ? (
                        <TheologicalText text={msg.content} />
                      ) : (
                        msg.content
                      )}
                    </div>
                ...
                {history.length > 0 && !isLoading && !isTyping && (
                  ...
                  >
                    <div className="md:col-span-2">
                      <p className="text-[9px] font-black uppercase tracking-[0.2em] text-gold/40 mb-spacing-md">
                        Conexões Orientadas
                      </p>
                    </div>
                    {[
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-primary/60 group-hover:text-gold-light">
                          {conn.label}
                        </span>
                      </Link>
                    ))}
                    ...
                    <div className="w-full space-y-spacing-md">
                      ...
                      {isTyping && !isLoading && (
                        <div className="flex gap-spacing-sm opacity-20 py-spacing-md items-center">
                          ...
                          <span className="text-[8px] font-black uppercase tracking-widest ml-spacing-xs text-primary/40">
                            Logos está contemplando...
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="w-full pt-spacing-lg md:pt-spacing-xl border-t border-primary/5">
                ...
                >
                  <input
                    ...
                    placeholder={
                      settings.totalSilence
                        ? "Silêncio Total Ativo..."
                        : "Bíblia, Catecismo ou perguntar sobre a fé..."
                    }
                    aria-label={
                      settings.totalSilence
                        ? "Logos IA desativada no Modo Silêncio"
                        : "Digite sua dúvida teológica para a Logos IA"
                    }
                ...
                <div className="flex flex-col items-center gap-spacing-xs mt-spacing-lg">
                  ...
                  <p className="text-[7px] text-center text-primary/40 uppercase tracking-[0.5em] font-black">
                    {settings.totalSilence
                      ? "O silêncio é a oração perfeita"
                      : "Silêncio é entendimento"}
                  </p>
                </div>
              </div>
            </CathedraCard>
          ...
          >
            <div className="p-spacing-lg md:p-spacing-xl border-b border-border/5 flex items-center justify-between">
              <div className="flex items-center gap-spacing-md">
                ...
                <div>
                  <h3 className="text-premium-sm font-bold uppercase tracking-[0.4em] text-primary">
                    Logos IA
                  </h3>
                  <p className="text-[9px] text-muted-foreground/60 uppercase font-black tracking-widest mt-spacing-2xs">
                    Mentor Espiritual
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-spacing-xs">
                ...
                <CathedraButton
                  ...
                  aria-label="Fechar Logos AI"
                  className="rounded-premium-full hover:bg-primary/[0.02] text-primary/60 hover:text-primary transition-colors h-spacing-xl w-spacing-xl px-spacing-0"
                  icon={<Icons.X className="w-spacing-md h-spacing-md" />}
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-spacing-lg md:p-spacing-xl space-y-spacing-xl md:space-y-spacing-xl scrollbar-hide">
                ...
                <div className="text-center py-spacing-3xl space-y-spacing-xl">
                  ...
                  <p className="text-premium-lg text-muted-foreground/40 font-serif italic leading-relaxed max-w-[280px] mx-auto">
                    ...
                    <span className="text-premium-sm uppercase tracking-widest font-black mt-spacing-md block">
                      {settings.totalSilence
                        ? "Modo Silêncio Ativo"
                        : "Como posso iluminar sua jornada?"}
                    </span>
                  </p>
                </div>
              )}
              ...
              {history.slice(-visibleMessages).map((msg, i) => (
                <div
                  key={i}
                  className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"} gap-spacing-sm animate-in fade-in slide-in-from-bottom-spacing-xs duration-500`}
                >
                  <div
                    className={`max-w-[90%] p-spacing-lg md:p-spacing-xl rounded-premium-lg text-premium-sm md:text-premium-base leading-relaxed ${
                      msg.role === "user"
