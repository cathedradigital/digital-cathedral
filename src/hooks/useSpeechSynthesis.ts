...
export const useSpeechSynthesis = (lang: Language) => {
  ...
  const speak = useCallback(
    (text: string) => {
      ...
      const betterVoice = voices
        .filter((v) => v.lang.startsWith(targetLang.split("-")[0]))
        .sort((a, b) => {
          const aName = a.name.toLowerCase();
          const bName = b.name.toLowerCase();
          const aScore =
            (aName.includes("google") ? 10 : 0) +
            (aName.includes("premium") ? 20 : 0) +
            (aName.includes("natural") ? 30 : 0) +
            (aName.includes("enhanced") ? 15 : 0);
          const bScore =
            (bName.includes("google") ? 10 : 0) +
            (bName.includes("premium") ? 20 : 0) +
            (bName.includes("natural") ? 30 : 0) +
            (bName.includes("enhanced") ? 15 : 0);
  ...
  const toggleSpeakFromElement = useCallback(
    (elementId: string = "main-content") => {
