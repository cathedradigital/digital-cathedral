...
export default function BibleSprint1Admin() {
  ...
  const sprint1Passed =
    ...
    totals.empty === 0 &&
    totals.missing === 0 &&
    totals.contaminated === 0;

  const bookLink = (abbrev: string, chapter = 1) =>
    `/bible/${encodeURIComponent(abbrev)}/${chapter}`;
  ...
  const filtered = (statuses: string[]) => coverage.filter((r) => statuses.includes(r.status));
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ok={totals.ok}/73 · partial={totals.partial} · empty={totals.empty} · missing=
            {totals.missing} · contaminated={totals.contaminated} · over={totals.over}
          </CardDescription>
        </CardHeader>
      </Card>
        ...
        <TabsContent value="jobs">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Cada job mostra <code>verification.sprint1.passed</code>. A ativação/promoção a
                primária só passa quando o último job concluído marcar <strong>passed=true</strong>.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                  ...
                  {jobs.map((j) => {
                    ...
                    return (
                      <TableRow key={j.id}>
                        ...
                        <TableCell>
                          {src ? `${src.code} — ${src.name}` : j.source_id?.slice(0, 8)}
                        </TableCell>
                        <TableCell>
              ...
              <p className="text-xs text-muted-foreground mt-4">
                💡 O botão <strong>Testar ativação</strong> dispara um{" "}
                <code>UPDATE status=active, is_primary=true</code>. O trigger{" "}
                <code>enforce_bible_source_sprint1_gate</code> bloqueia com erro{" "}
                <em>"Sprint 1 gate"</em> enquanto o último job não passar.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
