...
const DEFAULT_OG_IMAGE =
  "https://gpwrpmoniglarqwfyryp.supabase.co/storage/v1/object/public/public-assets/og-home.png";
...
const SEOVerificationPage = () => {
  ...
  const scanAll = async () => {
    ...
    if (scanMode === "render") {
      toast.info(
        "Modo Renderização JS ativo. A varredura será mais lenta para processar os metadados dinâmicos.",
      );
    }
        ...
        <div className="bg-muted/30 p-spacing-md rounded-premium-sm border border-border/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-spacing-md">
          <div className="flex items-center gap-spacing-md">
            <div className="p-spacing-xs bg-primary/10 rounded-premium-lg">
              <Icons.RefreshCcw
                className={`w-spacing-md h-spacing-md text-primary ${isScanningAll ? "animate-spin" : ""}`}
              />
            </div>
            <div>
              <h2 className="font-medium">Auditoria de Rotas ({pages.length})</h2>
              <p className="text-premium-sm text-muted-foreground">
                O cache de imagens é invalidado mensalmente.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-spacing-xs items-center">
            ...
            >
              {isScanningAll ? (
                <Icons.Loader2 className="w-spacing-md h-spacing-md mr-spacing-xs animate-spin" />
              ) : (
                <Icons.Search className="w-spacing-md h-spacing-md mr-spacing-xs" />
              )}
              Varrer Todas
            </Button>
              ...
              <div key={page.path} className="space-y-spacing-md">
                <div className="flex items-center justify-between px-spacing-xs">
                  ...
                  <div className="flex gap-spacing-xs">
                    ...
                    >
                      <Icons.RefreshCcw
                        className={`w-spacing-sm h-spacing-sm mr-spacing-xs ${page.status === "scanning" ? "animate-spin" : ""}`}
                      />
                      Scan
                    </Button>
                    <Button
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => copyToClipboard(`${BASE_URL}${page.path}`, "Link")}
                      title="Copiar Link"
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => copyToClipboard(getMetaTagsCode(page), "Tags HTML")}
                      title="Copiar Tags HTML"
                    >
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2">
                        ...
                        <div className="p-spacing-lg space-y-spacing-lg border-r border-border/50">
                          <div className="space-y-spacing-md">
                            <div>
                              <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-spacing-2xs">
                                Google Icons.Search Preview
                              </div>
                              <div className="p-spacing-md bg-white dark:bg-[#1a1a1a] rounded-premium-lg border border-border/40 shadow-premium-md">
                                ...
                                <div className="text-[14px] text-[#4d5156] dark:text-[#bdc1c6] line-clamp-spacing-xs">
                                  {page.description ||
                                    "Descrição não detectada. Execute o Scan para ler os metadados reais da página."}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-spacing-md">
                              <div className="p-spacing-sm bg-muted/30 rounded-premium-lg border border-border/30">
                                <div className="text-[10px] font-bold text-muted-foreground uppercase mb-spacing-2xs">
                                  Title Length
                                </div>
                                <div className="flex items-center justify-between">
                                  <span className="font-mono text-premium-lg">
                                    {(page.title?.length || 0) + 18}
                                  </span>
                                  {(page.title?.length || 0) + 18 <= 60 &&
                                  (page.title?.length || 0) > 0 ? (
                                ...
                                <div className="text-[10px] text-muted-foreground mt-spacing-2xs">
                                  Ideal: 50-60 chars
                                </div>
                              </div>
                              <div className="p-spacing-sm bg-muted/30 rounded-premium-lg border border-border/30">
                            ...
                            {(page.status === "missing" || !page.title || !page.description) && (
                              <div className="p-spacing-md bg-amber-500/10 border border-amber-500/20 rounded-premium-lg">
                                <h3 className="text-premium-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-widest mb-spacing-xs flex items-center gap-spacing-xs">
                                  <Icons.AlertCircle className="w-spacing-sm h-spacing-sm" />{" "}
                                  Sugestões de Melhoria
                                </h3>
                                <ul className="text-[11px] text-amber-800 dark:text-amber-300 space-y-spacing-2xs list-disc pl-spacing-md">
                                  {!page.title && (
                                    <li>
                                      O título está ausente. Use o componente SEOHead para definir
                                      um título único.
                                    </li>
                                  )}
                                  {page.title && page.title.length + 18 > 60 && (
                                    <li>
                                      O título está muito longo e será cortado no Google. Reduza
                                      para menos de 60 caracteres.
                                    </li>
                                  )}
                                  {!page.description && (
                                    <li>
                                      A descrição está ausente. Adicione uma meta descrição de
                                      120-160 caracteres para melhorar o CTR.
                                    </li>
                                  )}
                                  {page.description && page.description.length < 120 && (
                                    <li>
                                      A descrição está curta demais. Tente ser mais persuasivo.
                                    </li>
                                  )}
                                  {!page.metaTags?.ogImage && (
                                    <li>
                                      Imagem Open Graph não detectada. Redes sociais usarão o
                                      fallback padrão.
                                    </li>
                                  )}
                                </ul>
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-spacing-xs">
                              <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-spacing-2xs">
                                <Icons.Share2 className="w-spacing-sm h-spacing-sm" />{" "}
                                Icons.Facebook / WhatsApp
                              </div>
                              <Button
                                variant="ghost"
                                size="xs"
                                ...
                                onClick={() =>
                                  copyToClipboard(
                                    getSocialTagsOnly(page, "facebook"),
                                    "Tags Facebook",
                                  )
                                }
                              >
                            ...
                            <div className="border border-border/50 rounded-premium-lg overflow-hidden bg-white dark:bg-[#1a1a1a] shadow-premium">
                              <div className="aspect-[1.91/1] relative group">
                                <img
                                  src={
                                    page.metaTags?.ogImage ||
                                    getDynamicImage(page.title, page.image)
                                  }
                                  alt="OG Preview"
                                ...
                                <div className="absolute top-spacing-xs right-spacing-xs flex gap-spacing-2xs">
                                  <Button
                                    size="icon"
                                    variant="secondary"
                                    className="h-spacing-lg w-spacing-lg rounded-premium-full opacity-80"
                                    onClick={() =>
                                      window.open(
                                        page.metaTags?.ogImage ||
                                          getDynamicImage(page.title, page.image),
                              ...
                              <div className="p-spacing-sm border-t border-border/50 bg-[#f2f3f5] dark:bg-[#242526]">
                                <div className="text-[11px] text-muted-foreground uppercase truncate tracking-tight">
                                  CATHEDRADIGITAL.COM.BR
                                </div>
                                <div className="text-premium-sm font-bold truncate mt-spacing-3xs">
                                  {page.title || page.name} — Cathedra Digital
                                </div>
                                <div className="text-premium-xs text-muted-foreground line-clamp-spacing-2xs mt-spacing-3xs">
                                  {page.description || "Descrição não disponível"}
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-spacing-xs">
                              <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-spacing-xs flex items-center gap-spacing-2xs">
                                <Icons.Twitter className="w-spacing-sm h-spacing-sm" />{" "}
                                Icons.Twitter Card (Large)
                              </div>
                              <Button
                                variant="ghost"
                                size="xs"
                                ...
                                onClick={() =>
                                  copyToClipboard(
                                    getSocialTagsOnly(page, "twitter"),
                                    "Tags Twitter",
                                  )
                                }
                              >
                            ...
                            <div className="border border-border/40 rounded-premium overflow-hidden bg-white dark:bg-[#15202b] shadow-premium-md">
                              ...
                              <div className="p-spacing-sm border-t border-border/20">
                                <div className="text-premium-sm font-bold truncate">
                                  {page.title || page.name}
                                </div>
                                <div className="text-[13px] text-muted-foreground line-clamp-spacing-xs mt-spacing-3xs">
                                  {page.description || "Descrição não disponível"}
                                </div>
                  ...
                  <Card className="border-border/50 shadow-premium">
                    <CardHeader className="pb-spacing-sm border-b border-border/50 flex flex-row items-center justify-between">
                      <div>
                        ...
                        <CardDescription className="text-premium-xs">
                          Paste these into your CMS or manual header if needed.
                        </CardDescription>
                      </div>
                      <div className="flex gap-spacing-xs">
                        <Button
        ...
        <div className="pt-spacing-2xl border-t border-border/50">
          <Card className="bg-primary/5 border-primary/20">
            ...
            <CardContent className="space-y-spacing-md text-premium-sm text-muted-foreground">
              <p>
                <strong>1. Cache de Imagem:</strong> As imagens dinâmicas utilizam o parâmetro{" "}
                <code>v=yyyy-mm</code>. Isso garante que as redes sociais não usem versões antigas
                por muito tempo, mas mantém a estabilidade durante o mês atual.
              </p>
              <p>
                <strong>2. Fallback Automático:</strong> O Cathedra Digital agora fornece múltiplos
                metatags <code>og:image</code>. Caso o serviço dinâmico falhe ou demore a responder,
                os crawlers automaticamente tentarão o segundo link (Imagem padrão da Icons.Home).
              </p>
              <p>
                <strong>3. Auditoria Real:</strong> Use o botão <strong>Scan</strong> para ler o
                HTML que está sendo servido atualmente para cada rota. Isso valida se o{" "}
                <code>react-helmet-async</code> está configurado corretamente para aquela página.
              </p>
            </CardContent>
          </Card>
        </div>
