...
const BibleDiagnosticRuns: React.FC = () => {
  ...
  const filteredRuns = useMemo(() => {
    ...
    return runs.filter((r) => {
      ...
      const keys = Object.keys(bookFilter)
        .filter((k) => k.startsWith(`${r.id}:`))
        .map((k) => k.split(":")[1].toLowerCase());
  ...
  const runNow = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success(
        `Diagnóstico concluído: ${data?.total_findings ?? 0} achados em ${fmtDuration(data?.duration_ms ?? 0)}`,
      );
      await loadRuns();
    } catch (e) {
  ...
  const diffSummary = useMemo(() => {
    ...
    const typeDiff = Array.from(allTypes)
      .map((t) => ({
        key: t,
        left: L.byType[t] ?? 0,
        right: R.byType[t] ?? 0,
        delta: (R.byType[t] ?? 0) - (L.byType[t] ?? 0),
      }))
      .sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta));
    const bookDiff = Array.from(allBooks)
      .map((b) => ({
        key: b,
        left: L.byBook[b] ?? 0,
        right: R.byBook[b] ?? 0,
        delta: (R.byBook[b] ?? 0) - (L.byBook[b] ?? 0),
      }))
      .sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta));
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Histórico das execuções do diagnóstico read-only dos 73 livros.
        </p>
      </header>
        ...
        <TabsContent value="compare" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Selecione as runs base (anterior) e atual. Regressões aparecem em vermelho.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="w-full mt-1 border border-input rounded-md p-2 bg-background"
                    value={leftId}
              ...
              <div className="flex gap-2">
                ...
                {diff && (
                  <span className="text-sm text-muted-foreground self-center">
                    Base: {diff.leftFindings.length} · Atual: {diff.rightFindings.length} · Δ{" "}
                    {diff.rightFindings.length - diff.leftFindings.length}
                  </span>
                )}
              </div>
              ...
              {diffSummary && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-base">Diff por tipo</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <Table>
                        <TableHeader>
                        ...
                        <TableBody>
                          {diffSummary.typeDiff.map((r) => (
                            <TableRow
                              key={r.key}
                              className={r.delta > 0 ? "bg-red-50 dark:bg-red-950/20" : ""}
                            >
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="text-base">Diff por livro</CardTitle>
                    </CardHeader>
