...
const CATALOGO: Record<string, Passage> = {
  jo15: {
    ...
    verses: [
      ...
      {
        n: 2,
        text: "Todo ramo que, estando em mim, não dá fruto, ele corta; e todo o que dá fruto limpa, para que produza mais fruto ainda.",
        anchor: true,
      },
  ...
  cic1234: {
    ...
    verses: [
      {
        n: 1234,
        text: "O sentido e a graça do sacramento do Batismo manifestam-se claramente nos ritos da sua celebração…",
        anchor: true,
      },
    ],
  ...
  laudes: {
    ...
    verses: [
      ...
      {
        n: 2,
        text: "Ao seu encontro caminhemos com louvores, e com cânticos de festa o celebremos.",
      },
    ],
    nexus: ["Sl 95", "Sl 148"],
  },
...
const Leitor: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {p.chapter && <div className="font-serif text-5xl text-primary/70 mb-4">{p.chapter}</div>}

      <article
        className={`font-serif text-lg leading-relaxed space-y-3 ${prece ? "text-foreground/90" : ""}`}
      >
        {p.verses.map((v) => (
          <p key={v.n}>
      ...
      {fav && !prece && (
        <p className="text-xs text-center text-muted-foreground mt-4">
          ♥ Salvo em{" "}
          <Link to={`${BASE}/minha-jornada`} className="text-primary underline">
            Minha Jornada
          </Link>
        </p>
      )}
