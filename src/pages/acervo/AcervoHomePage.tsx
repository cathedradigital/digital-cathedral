...
import {
  ...
  EditorialDivider,
  EditorialKicker,
} from "@/components/editorial/harmony";
import {
  SpaceLayout,
  SpaceHeader,
  SpaceEntrance,
...
const LibraryOfflineFallback: React.FC<{ onRetry: () => void }> = ({ onRetry }) => (
  <div className="py-spacing-2xl px-spacing-md text-center space-y-spacing-md bg-destructive/5 rounded-premium border border-destructive/10 animate-fade-in">
    ...
    <Button variant="outline" onClick={onRetry} className="rounded-premium-full gap-spacing-xs">
      ...
      <SpaceLayout>
        <EditorialHero density="expanded" align="center">
          ...
          <EditorialHero.Subtitle>
            Toda a riqueza da fé católica em um ambiente onde tudo se conecta.
          </EditorialHero.Subtitle>
        </EditorialHero>
        ...
        <div className="py-spacing-xl space-y-spacing-3xl">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <Button
                      ...
                      onClick={() =>
                        (window.location.href = `/buscar?q=${encodeURIComponent(trilha.label)}`)
                      }
                    >
            ...
            <section className="lg:col-span-8 space-y-spacing-md order-1 lg:order-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-spacing-md">
                ...
                <Link to="/papas" className="group">
                  <EditorialCard
                    density="dense"
                    className="h-full bg-primary/[0.01] hover:bg-primary/[0.03] border-primary/10"
                  >
                    <div className="flex items-center gap-spacing-md">
                      ...
                      <div>
                        ...
                        <EditorialCard.Title>
                          {currentPope?.name || "Papa Francisco"}
                        </EditorialCard.Title>
                        <EditorialCard.Description>
                          O sucessor de Pedro nos guia na fé.
                        </EditorialCard.Description>
                      </div>
                    </div>
                  </EditorialCard>
                  ...
                  >
                    <div className="flex items-center gap-spacing-md">
                      ...
                      <div>
                        ...
                        <EditorialCard.Title>
                          {todaySaint?.name || "Vidas Exemplares"}
                        </EditorialCard.Title>
                        <EditorialCard.Description>
                          O modelo de santidade para hoje.
                        </EditorialCard.Description>
                      </div>
                    </div>
                  </EditorialCard>
                ...
                <Link to="/liturgia" className="group">
                  <EditorialCard
                    density="dense"
                    className="h-full bg-primary/[0.01] hover:bg-primary/[0.03]"
                  >
                    <EditorialCard.Eyebrow>☀ Evangelho do Dia</EditorialCard.Eyebrow>
                    <EditorialCard.Title>
                      {liturgy?.liturgia || "A Palavra Viva"}
                    </EditorialCard.Title>
                    <EditorialCard.Description>
                      Reflexão e leitura litúrgica para hoje.
                    </EditorialCard.Description>
                  </EditorialCard>
                </Link>
                <Link to="/catechism" className="group">
                  ...
                  >
                    ...
                    <EditorialCard.Description>
                      Aprofunde-se no depósito da fé.
                    </EditorialCard.Description>
                  </EditorialCard>
                </Link>
              </div>
              ...
              {[
                ...
                <Link key={item.label} to={item.to} className="group flex flex-col">
                  <div className="p-spacing-lg rounded-premium border border-primary/10 bg-card/40 hover:bg-primary/[0.02] hover:border-gold/30 transition-all text-center space-y-3 h-full flex flex-col items-center justify-center relative shadow-sm">
                    ...
                    <h3 className="text-[12px] font-black uppercase tracking-widest text-primary group-hover:text-secondary">
                      {item.label}
                    </h3>
                    <p className="text-[9px] text-muted-foreground/60 uppercase tracking-tighter">
                      {item.desc}
                    </p>
                  </div>
                </Link>
          ...
          <section className="bg-primary/5 rounded-premium p-spacing-xl md:p-spacing-2xl border border-primary/10">
            <div className="max-w-3xl mx-auto text-center space-y-spacing-lg">
              ...
              <p className="type-lead opacity-80">
                Converse com o Logos e encontre conexões profundas entre a Bíblia, o Catecismo e a
                Tradição.
              </p>
              <Button
                size="lg"
                  ...
                  featured.slice(0, 3).map((item) => (
                    <Link key={item.id} to={item.href} className="group">
                      <EditorialCard density="dense" className="h-full">
                        ...
                        <EditorialCard.Description>
                          {item.author_label || "Obra fundamental"}
                        </EditorialCard.Description>
                      </EditorialCard>
                    </Link>
                  ))}
