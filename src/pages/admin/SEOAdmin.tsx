...
export default function SEOAdmin() {
  ...
  const loadAudits = async () => {
    const { data, error } = await supabase
      ...
      .limit(100);
    if (error) {
      toast.error("Falha ao carregar auditorias: " + error.message);
      return;
    }
  ...
  const loadSettings = async () => {
    const { data, error } = await supabase
      .from("seo_settings")
      .select("id,gsc_verification_code")
      .limit(1)
      .maybeSingle();
    if (error) {
      toast.error("Falha ao carregar settings: " + error.message);
      return;
    }
    if (data) {
      setSettingsId(data.id);
      setGscToken(data.gsc_verification_code || "");
    }
  };
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      await Promise.all([loadAudits(), loadSettings()]);
      setLoading(false);
    })();
  }, []);
    ...
    try {
      if (settingsId) {
        const { error } = await supabase
          .from("seo_settings")
          .update({ gsc_verification_code: gscInput.trim() })
          .eq("id", settingsId);
        if (error) throw error;
      } else {
        const { data, error } = await supabase
          .from("seo_settings")
          .insert({ gsc_verification_code: gscInput.trim() })
          .select("id")
          .single();
        if (error) throw error;
        setSettingsId(data.id);
  ...
  const exportPdf = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Relatório SEO — Cathedra Digital", 14, 18);
    doc.setFontSize(10);
    ...
    let y = 34;
    doc.setFontSize(12);
    doc.text("Sitemap & Robots", 14, y);
    y += 6;
    doc.setFontSize(10);
    if (validation) {
      const s = validation.sitemap,
        r = validation.robots;
      const rows = [
      ...
      doc.text("Rode 'Validar sitemap/robots' antes de exportar para incluir esses dados.", 14, y);
      y += 10;
    }

    doc.setFontSize(12);
    doc.text("Resumo das auditorias", 14, y);
    y += 4;
    if (summary) {
      autoTable(doc, {
        startY: y + 2,
        ...
        body: [
          [
            ...
            String(summary.avg),
            String(summary.withProblems),
            new Date(summary.lastAt!).toLocaleString("pt-BR"),
          ],
        ],
        styles: { fontSize: 9 },
      });
    ...
    doc.setFontSize(12);
    doc.text("Principais URLs com problemas", 14, y);
    autoTable(doc, {
      startY: y + 2,
      head: [["URL", "Score", "Findings"]],
      body: topProblems.map((a) => [
        a.url,
        String(a.score ?? "—"),
        a.findings.map((f) => `[${f.severity}] ${f.type}`).join("\n"),
      ]),
    ...
    <div className="container mx-auto py-8 space-y-6 max-w-6xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Sitemap, robots, auditorias e verificação Search Console.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Sitemap, robots, feed e verificação rápida de meta tags do último audit.
          </CardDescription>
        </CardHeader>
            ...
            if (!last)
              return (
                <p className="text-muted-foreground">
                  Sem auditorias ainda. Rode uma auditoria para ver os alertas.
                </p>
              );
              ...
              <div className="rounded border p-3 space-y-1">
                ...
                {missing.length === 0 ? (
                  <div className="flex items-center gap-2 text-emerald-600">
                    <CheckCircle2 className="h-4 w-4" /> Todas as meta tags principais presentes.
                  </div>
                ) : (
                  <div className="flex items-center gap-2 text-amber-600">
                    <AlertTriangle className="h-4 w-4" /> Faltando: {missing.join(", ")}
                  </div>
                )}
                {validation?.robots.disallow_all && (
                  <div className="flex items-center gap-2 text-destructive">
                    <XCircle className="h-4 w-4" /> robots.txt está bloqueando toda a indexação
                    (Disallow: /).
                  </div>
                )}
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>URLs auditadas</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <span className="text-3xl font-bold">{summary?.avg ?? "—"}</span>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Com problemas</CardDescription>
          </CardHeader>
          <CardContent>
            <span className="text-3xl font-bold text-amber-600">{summary?.withProblems ?? 0}</span>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>URLs no sitemap</CardDescription>
          </CardHeader>
          <CardContent>
            <span className="text-3xl font-bold">{validation?.sitemap.url_count ?? "—"}</span>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Salvo em <code>seo_settings.gsc_verification_code</code> e injetado no &lt;head&gt;.
            Exibido mascarado.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div>
            <Label htmlFor="gsc-input" className="text-xs text-muted-foreground">
              Novo token (conteúdo do meta google-site-verification)
            </Label>
      ...
      {validation && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Validado em {new Date(validation.checked_at).toLocaleString("pt-BR")}
            </CardDescription>
          </CardHeader>
          <CardContent className="grid md:grid-cols-2 gap-4 text-sm">
            <div className="space-y-1">
              <div className="font-medium flex items-center gap-2">
                {validation.sitemap.http_status === 200 && !validation.sitemap.invalid_xml ? (
              ...
              <div>
                ...
                <a
                  ...
                  href={validation.sitemap.url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  {validation.sitemap.url}
                </a>
              </div>
              <div>HTTP: {validation.sitemap.http_status}</div>
              <div>
                URLs: {validation.sitemap.url_count} · Duplicadas:{" "}
                {validation.sitemap.duplicate_count}
              </div>
              {validation.sitemap.invalid_xml && (
                <div className="text-destructive">XML inválido</div>
              )}
              {validation.sitemap.broken_sample.length > 0 && (
                <div className="text-amber-600 flex items-start gap-1">
                  <AlertTriangle className="h-4 w-4 mt-0.5" />{" "}
                  {validation.sitemap.broken_sample.length} URL(s) da amostra retornaram erro
                </div>
              )}
            </div>
            <div className="space-y-1">
              <div className="font-medium flex items-center gap-2">
                {validation.robots.http_status === 200 && !validation.robots.disallow_all ? (
                  <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                ) : (
              ...
              <div>
                ...
                <a
                  className="text-primary hover:underline break-all"
                  href={validation.robots.url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  {validation.robots.url}
                </a>
              </div>
              <div>
                HTTP: {validation.robots.http_status} · Linhas: {validation.robots.line_count}
              </div>
              {validation.robots.disallow_all && (
                <div className="text-destructive">Bloqueia todos os crawlers!</div>
              )}
              <div>
                Sitemap directive:{" "}
                <code className="text-xs">{validation.robots.sitemap_directive || "—"}</code>
              </div>
            </div>
            <p className="md:col-span-2 text-xs text-muted-foreground border-t pt-3">
              Regeneração real do <code>sitemap.xml</code> acontece no build (
              <code>scripts/generate-sitemap.ts</code>). Este botão valida o arquivo servido em
              produção.
            </p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Title, description, heading (H1), canonical e links quebrados. Páginas com qualquer
            falha ficam marcadas como "precisa de ajuste".
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-sm text-muted-foreground">
              Nenhuma auditoria ainda. Rode uma acima.
            </div>
          ) : (
            <div className="space-y-2">
              {checklist.map(({ row, checks, failed, broken }) => {
                const Item = ({ ok, label }: { ok: boolean; label: string }) => (
                  <span
                    className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded border ${ok ? "text-emerald-700 border-emerald-200 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-950/30 dark:border-emerald-900" : "text-destructive border-destructive/30 bg-destructive/5"}`}
                  >
                ...
                return (
                  <div key={row.id} className="p-3 rounded-lg border bg-card">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      ...
                      <Item
                        ok={checks.links}
                        label={`links${broken.length ? ` (${broken.length} quebrados)` : ""}`}
                      />
                    </div>
                    {broken.length > 0 && (
                      <ul className="mt-2 text-xs text-muted-foreground space-y-0.5">
                        ...
                        {broken.length > 5 && (
                          <li className="text-[11px]">+ {broken.length - 5} outros</li>
                        )}
                      </ul>
                    )}
                  </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Baseado na última auditoria salva em <code>seo_audits</code>.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-sm text-muted-foreground">
              Nenhuma auditoria com problemas. Rode uma nova.
            </div>
          ) : (
            <div className="space-y-2">
              {topProblems.map((a) => (
