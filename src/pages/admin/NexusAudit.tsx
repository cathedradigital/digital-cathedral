...
const GOLD_SHAPE: Array<{
  key: string;
  label: string;
  min: number;
  match: (r: RelationRow) => boolean;
}> = [
  ...
  { key: "cic", label: "Catecismo", min: 3, match: (r) => r.target_kind === "catechism_paragraph" },
  {
    ...
    label: "Magistério",
    min: 2,
    match: (r) => r.target_kind === "magisterium_doc",
  },
    ...
    (async () => {
      try {
        const [g, n] = await Promise.all([
          ...
          supabase.from("nexus_relations").select("source_ref, target_kind, target_ref"),
        ]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-8" data-space="claustro">
      <header className="mb-8">
        <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
          Cathedra Index · Onda A
        </p>
        <h1 className="mt-2 text-3xl font-serif">Auditoria do Grafo Sacramental</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Verifica em tempo real se cada verbete-chave atinge o padrão Nexus Ouro: ≥{GOLD_TOTAL}{" "}
          arestas distribuídas nas 10 dimensões editoriais.
        </p>
      </header>
      ...
      <div className="mb-6 rounded-lg border border-border bg-muted/30 p-4 flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
            Cathedra Index · Integridade Editorial
          </p>
          <p className="mt-1 text-sm">
            Verbetes marcados como <code>complete</code> mas com campos vitais vazios (interpretação
            profunda, bibliografia, FAQ) ou densidade Nexus abaixo do Ouro.
          </p>
        </div>
        <a
      ...
      {!loading && !error && (
        <>
          ...
          <div className="mb-8 grid grid-cols-2 gap-3 md:grid-cols-4">
            ...
            <SummaryCard
              label="Nexus Ouro"
              value={`${summary.gold} / ${summary.count}`}
              tone={summary.gold === summary.count ? "ok" : "warn"}
            />
            <SummaryCard
              label="Drafts"
              value={summary.draft}
              tone={summary.draft === 0 ? "ok" : "warn"}
            />
                ...
                <CardHeader className="flex flex-row items-start justify-between gap-4 pb-3">
                  <div>
                    <CardTitle className="text-lg">
                      <Link
                        to={resolveNexusHref("glossary", r.slug) ?? "#"}
                  ...
                  <div className="text-right">
                    ...
                    {r.gap > 0 && <p className="text-[11px] text-amber-600">faltam {r.gap}</p>}
                  </div>
                </CardHeader>
                <CardContent>
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === "draft")
    return <Badge className="bg-amber-500/20 text-amber-700 hover:bg-amber-500/20">rascunho</Badge>;
