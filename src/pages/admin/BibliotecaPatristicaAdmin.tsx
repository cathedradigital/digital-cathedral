...
export default function BibliotecaPatristicaAdmin() {
  ...
  const stats = useMemo(() => {
    const by: Record<SaintWorkStatus, number> = {
      draft: 0,
      in_review: 0,
      published: 0,
      archived: 0,
    ...
    <div className="container mx-auto py-8 space-y-6">
      ...
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Curadoria de obras dos santos. Toda obra nasce em rascunho; publicação exige licença
          declarada.
        </p>
      </header>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[220px]">
          ...
          <Input
            ...
            placeholder="Título, autor ou slug…"
          />
        </div>
        ...
        <div className="min-w-[180px]">
          ...
          <Select
            value={fichaFilter}
            onValueChange={(v: SaintWorkFichaCompleteness | "all") => setFichaFilter(v)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              ...
              {(
                Object.keys(SAINT_WORK_FICHA_COMPLETENESS_LABELS) as SaintWorkFichaCompleteness[]
              ).map((f) => (
                <SelectItem key={f} value={f}>
                  {SAINT_WORK_FICHA_COMPLETENESS_LABELS[f]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
                ...
                {filtered.map((w) => (
                  <TableRow key={w.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          w.ficha_completeness === "complete"
                            ? "default"
                            : w.ficha_completeness === "minimal"
                              ? "secondary"
                              : "outline"
                        }
                      >
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select
                value={form.category}
                onValueChange={(v: SaintWorkCategory) => setForm((f) => ({ ...f, category: v }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    year_written: e.target.value === "" ? "" : Number(e.target.value),
                  }))
                }
              />
            </div>
          ...
          <div className="border-t pt-4 space-y-3">
            ...
            <div>
              ...
              <Input
                ...
                value={form.translation_credit}
                onChange={(e) => setForm((f) => ({ ...f, translation_credit: e.target.value }))}
              />
            </div>
            <div>
          ...
          <div className="border-t pt-4 text-sm text-muted-foreground flex items-center gap-2">
            ...
            <span>
              {work.chapter_count} capítulo(s) · edite via SQL enquanto o editor de capítulos não
              estiver disponível.
            </span>
          </div>
