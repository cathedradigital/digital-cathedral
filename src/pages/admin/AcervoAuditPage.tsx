...
const AcervoAuditPage: React.FC = () => {
  ...
  if (loading)
    return <div className="p-20 text-center italic opacity-50">Auditoria em curso...</div>;
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <EditorialHero align="center" density="minimal" className="bg-primary/5">
        ...
        <EditorialHero.Subtitle>
          Certificação global de maturidade e fidelidade à Tradição.
        </EditorialHero.Subtitle>
      </EditorialHero>
      ...
      <main className="max-w-5xl mx-auto px-6 mt-12 space-y-12">
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="text-[10px] border-gold/30 text-gold uppercase tracking-widest font-bold"
            >
              Fase 10
            </Badge>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-primary/5 border-b border-border">
                <tr>
                  ...
                  <th className="p-4 font-black uppercase tracking-widest text-[10px] text-right">
                    Cobertura
                  </th>
              ...
              <tbody className="divide-y divide-border/40">
                ...
                <CoverageRow
                  label="Catecismo"
                  percent={summary?.patrimonyCoverage.catecismo || 0}
                />
                ...
                <CoverageRow
                  label="Patrística"
                  percent={summary?.patrimonyCoverage.patristica || 0}
                  status="progress"
                />
                <CoverageRow
                  label="Magistério"
                  percent={summary?.patrimonyCoverage.magisterio || 0}
                  status="progress"
                />
                <CoverageRow
                  label="Papas"
                  percent={summary?.patrimonyCoverage.papas || 0}
                  status="warning"
                />
                <CoverageRow
                  label="Dogmas"
                  percent={summary?.patrimonyCoverage.dogmas || 0}
                  status="progress"
                />
                <CoverageRow
                  label="Concílios"
                  percent={summary?.patrimonyCoverage.concilios || 0}
                  status="warning"
                />
        ...
        <div className="p-8 bg-primary/5 rounded-premium border border-primary/10 text-center space-y-4">
          ...
          <h3 className="text-xl font-serif font-bold italic">
            Fase 10.1: Constituição do Patrimônio
          </h3>
          <p className="text-muted-foreground text-sm max-w-2xl mx-auto leading-relaxed italic">
            A infraestrutura chegou à maturidade. Agora, o Cathedra opera sob uma Constituição
            Editorial rigorosa: nenhum conteúdo entra sem 100% de conformidade com o Reader V2,
            Nexus, Logos e fidelidade documental.
          </p>
        </div>
      </main>
    </div>
  ...
  return (
    <div className="bg-card border border-border/60 p-5 rounded-premium flex items-center justify-between group hover:border-primary/20 transition-all">
      <div className="space-y-1">
        <div className="text-[10px] font-black uppercase tracking-widest text-foreground group-hover:text-primary transition-colors">
          {label}
        </div>
