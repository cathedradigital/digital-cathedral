...
export default function GlobalKnowledgeAudit() {
  ...
  const calculateFinalScore = (m: ModuleScore) => {
    return (
      m.magisterio * 0.3 + m.editorial * 0.2 + m.nexus * 0.2 + m.reader * 0.15 + m.cobertura * 0.15
    );
  };
  ...
  return (
    <div className="container mx-auto py-8 max-w-6xl space-y-8 animate-in fade-in duration-700">
      <Helmet>
        <title>Auditoria Global de Conhecimento — Cathedra 3.0</title>
      </Helmet>

      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <Badge variant="outline" className="text-primary font-mono mb-2 border-primary/20">
            FASE 6.2 — AUDITORIA GLOBAL DO ACERVO
          </Badge>
          <h1 className="text-3xl font-black tracking-tight font-display text-primary">
            Discovery Audit Portal
          </h1>
          <p className="text-muted-foreground text-sm uppercase tracking-widest font-bold">
            Mapeamento de Módulos e Visibilidade do Peregrino
          </p>
        </div>
        ...
        <TabsContent value="mission-control" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="md:col-span-2 bg-primary/5 border-primary/10 overflow-hidden relative">
              ...
              <CardHeader>
                <CardTitle className="font-mono text-xs uppercase tracking-[0.3em] text-primary">
                  CATHEDRA DISCOVERY CENTER
                </CardTitle>
                <CardDescription>
                  Métricas de integração e acessibilidade ao usuário
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <Card key={mod.id} className="group hover:border-primary/30 transition-colors">
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-center">
                      <div>
                        <CardTitle className="text-xl font-black font-display uppercase tracking-tight">
                          {mod.label}
                        </CardTitle>
                        <CardDescription>
                          Auditado em {new Date().toLocaleDateString()}
                        </CardDescription>
                      </div>
                      <div className="text-right">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {[
                        ...
                        <div
                          key={stat.label}
                          className="p-4 bg-muted/30 rounded-premium border border-transparent group-hover:border-primary/5 transition-colors"
                        >
          ...
          <Card className="min-h-[500px] flex flex-col items-center justify-center p-12 bg-muted/10 border-dashed border-2">
            <div className="flex flex-col items-center gap-4 text-center max-w-md">
              <div className="flex flex-col items-center font-black uppercase tracking-[0.2em] text-primary/40 text-sm gap-8">
                <span className="p-4 border-2 border-primary/20 rounded-premium text-primary">
                  Jesus Cristo
                </span>
                ...
                <GitBranch className="w-6 h-6 rotate-180" />
                <span className="p-4 border-2 border-primary/20 rounded-premium">
                  Santos & Patrística
                </span>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-xl font-black uppercase">
                Módulos Desconectados ou Ocultos
              </CardTitle>
              <CardDescription>
                Recursos que existem no código mas são de difícil acesso
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-border/50">
                      <th className="py-4 font-bold uppercase tracking-widest text-[10px] opacity-60">
                        Módulo
                      </th>
                      <th className="py-4 font-bold uppercase tracking-widest text-[10px] opacity-60">
                        Visível na Home
                      ...
                      <th className="py-4 font-bold uppercase tracking-widest text-[10px] opacity-60 text-center">
                        Nexus
                      </th>
                      <th className="py-4 font-bold uppercase tracking-widest text-[10px] opacity-60 text-right">
                        Integração Logos
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      >
                        <td className="py-4 font-black uppercase text-xs tracking-tight">
                          {app.name}
                        </td>
                        <td className="py-4">
                        ...
                        <td className="py-4">
                          <span className="text-xs font-bold text-emerald-600">{app.approval}</span>
                        </td>
