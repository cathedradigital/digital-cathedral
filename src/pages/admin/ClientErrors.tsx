          ...
          <div className="flex gap-2 items-end">
            <div className="flex-1 max-w-sm">
              ...
              <Input
                ...
                placeholder="err_xxxxxxxxx"
              />
            </div>
            {q.isFetching && (
              <Loader2 className="w-4 h-4 animate-spin text-muted-foreground mb-3" />
            )}
          </div>
        </CardContent>
      </Card>
      ...
      {focused && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {new Date(focused.created_at).toLocaleString("pt-BR")} · user{" "}
              <code>{focused.user_id?.slice(0, 8) ?? "—"}</code>
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <div>
              <span className="text-muted-foreground">Rota:</span>{" "}
              <code>{focused.properties?.route ?? focused.url ?? "—"}</code>
            </div>
            <div>
              <span className="text-muted-foreground">Query:</span>{" "}
              <code className="break-all">{focused.properties?.query || "—"}</code>
            </div>
            <div>
              <span className="text-muted-foreground">Hash:</span>{" "}
              <code>{focused.properties?.hash || "—"}</code>
            </div>
            <div>
              <span className="text-muted-foreground">Mensagem:</span>{" "}
              <span className="text-destructive">{focused.properties?.message ?? "—"}</span>
            </div>
            <div>
              <div className="text-muted-foreground text-xs uppercase tracking-widest mt-2 mb-1">
                Stack
              </div>
            ...
            <div className="text-xs text-muted-foreground">
              UA: {focused.properties?.user_agent ?? "—"}
            </div>
          </CardContent>
        </Card>
      )}
