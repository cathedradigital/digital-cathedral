  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("partners")
        .select(
          "id, name, description, logo_url, website_url, contact_email, partner_type, status, created_at",
          { count: "exact" },
        )
      ...
      const withPreview = await Promise.all(
        ((rows as PartnerRow[]) ?? []).map(async (r) => {
    ...
    return (
      <div className="max-w-xl mx-auto py-24 px-6 text-center space-y-3">
        ...
        <p className="text-muted-foreground">
          Este painel é restrito a administradores do Cathedra.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="container max-w-6xl mx-auto py-spacing-xl px-spacing-md space-y-spacing-xl">
      <header className="space-y-spacing-xs">
        <p className="text-premium-xs uppercase tracking-widest text-secondary">
          Cathedra · Curadoria
        </p>
      ...
      <div className="flex flex-wrap gap-spacing-md items-end">
        <div className="relative min-w-[240px] flex-1">
          <label className="text-premium-xs text-muted-foreground block mb-spacing-xs">
            Buscar
          </label>
    ...
    <CardContent className="grid grid-cols-1 md:grid-cols-[120px_1fr] gap-spacing-md">
      ...
      <div className="space-y-spacing-xs text-premium-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-spacing-xs text-muted-foreground">
          <div>
            Contato: <span className="text-foreground">{partner.contact_email ?? "—"}</span>
          </div>
          <div>
            Site:{" "}
            {partner.website_url ? (
          ...
          <div>
            ...
            <span className="text-foreground">
              {new Date(partner.created_at).toLocaleString("pt-BR")}
            </span>
          </div>
        </div>
      </div>
