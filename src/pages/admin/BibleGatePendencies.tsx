  ...
  const grouped = useMemo(() => {
    ...
    for (const f of findingsQ.data ?? []) {
      ...
      const cur = map.get(key) ?? {
        abbrev: f.abbrev,
        book_name: f.book_name ?? f.abbrev,
        counts: {},
        items: [],
      };
      ...
      <header className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Últimos achados de integridade agrupados por livro. O gate bloqueia leitura pública
            quando há livros ou capítulos faltantes.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={() => {
              ...
              downloadBlob(
                `bible-diagnose-${gate?.run_id?.slice(0, 8) ?? "current"}.json`,
                "application/json",
          ...
          <Button
            ...
            onClick={() => {
              ...
              downloadBlob(
                `bible-findings-${gate?.run_id?.slice(0, 8) ?? "current"}.csv`,
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2">
            {gate?.blocked ? (
              <>
                <ShieldAlert className="w-5 h-5 text-amber-600" /> Gate bloqueando{" "}
          ...
          <CardDescription>
            Status: <code>{gate?.status ?? "…"}</code> · Última verificação:{" "}
            {gate?.last_run_at ? new Date(gate.last_run_at).toLocaleString("pt-BR") : "—"} · Run:{" "}
            <code>{gate?.run_id?.slice(0, 8) ?? "—"}</code>
          </CardDescription>
        </CardHeader>
                            ...
                            <ul className="space-y-1 text-xs">
                              {g.items.map((f) => (
                                <li key={f.id} className="flex gap-2 items-start">
                                  <Badge
                                    variant={SEV_VARIANT[f.severity] ?? "outline"}
                                    className="text-[10px]"
                                  >
                                    {f.severity}
                                  </Badge>
