...
function computeChecks(r: any): Check[] {
  ...
  return [
    ...
    {
      field: "deep_interpretation",
      label: "Interpretação profunda",
      weight: 15,
      ok: nz(r.deep_interpretation),
      generable: true,
      group: "editorial",
    },
    {
    ...
    },
    {
      ...
      weight: 8,
      ok: nz(r.historical_context),
      generable: true,
      group: "editorial",
    },
    {
      field: "practical_application",
      label: "Aplicação prática",
      weight: 8,
      ok: nz(r.practical_application),
      generable: true,
      group: "editorial",
    },
    {
      field: "logos_meditation",
      label: "Meditação Logos",
      weight: 8,
      ok: nz(r.logos_meditation),
      generable: true,
      group: "editorial",
    },
    {
    ...
    {
      field: "bibliography",
      label: "Bibliografia (≥3)",
      weight: 7,
      ok: arr(r.bibliography, 3),
      generable: true,
      group: "editorial",
    },
    {
      ...
      label: "Bíblia (≥3)",
      weight: 6,
      ok: arr(r.bible_verses, 3),
      generable: true,
      group: "nexus",
    },
    {
      field: "catechism_references",
      label: "CIC (≥2)",
      weight: 6,
      ok: arr(r.catechism_references, 2),
      generable: true,
      group: "nexus",
    },
    {
      ...
      label: "Padres (≥1)",
      weight: 5,
      ok: arr(r.fathers_refs, 1),
      generable: true,
      group: "nexus",
    },
    {
      field: "magisterium_references",
      label: "Magistério (≥1)",
      weight: 5,
      ok: arr(r.magisterium_references, 1),
      generable: true,
      group: "nexus",
    },
    {
      field: "nexus",
      label: `Nexus ${nexusCount}/${GOLD_MIN}`,
      weight: 4,
      ok: nexusCount >= GOLD_MIN,
      generable: false,
      group: "nexus",
    },
  ];
...
const SLUG_SUGGESTIONS: Record<string, Partial<Record<Check["field"], string[]>>> = {
  trindade: {
    fathers_refs: [
      ...
      "São Gregório Nazianzeno — Orações Teológicas",
    ],
  ...
  eucaristia: {
    fathers_refs: [
      "Santo Inácio de Antioquia — Ad Smyrnaeos",
      "São Justino — I Apologia 66",
      "Santo Agostinho — Sermão 227",
    ],
    catechism_references: ["CIC §§1322–1419"],
    magisterium_references: [
      "Ecclesia de Eucharistia (João Paulo II)",
      "Sacrosanctum Concilium §§47–58",
    ],
  },
  encarnacao: {
    fathers_refs: [
      "Santo Atanásio — De Incarnatione",
  ...
  cristologia: {
    fathers_refs: [
      ...
      "São Máximo Confessor — Ambigua",
      "São João Damasceno — De Fide Orthodoxa",
    ],
  },
  ressurreicao: {
    catechism_references: ["CIC §§638–658"],
    bible_verses: ["1Cor 15,3-8", "Lc 24", "Jo 20"],
  },
  purgatorio: {
    catechism_references: ["CIC §§1030–1032"],
    magisterium_references: [
      "Bento XVI — Spe Salvi §§45–48",
      "Concílio de Trento — Decreto sobre o Purgatório",
    ],
  },
  fe: {
...
const GENERIC_SUGGESTIONS: Partial<Record<Check["field"], string[]>> = {
  ...
  magisterium_references: [
    "Consultar encíclicas correlatas em vatican.va",
    "Verificar documentos conciliares",
  ],
...
export default function EditorialAuditPage() {
  ...
  const [priorityFilter, setPriorityFilter] = useState<
    "quick_win" | "red" | "orange" | "yellow" | "all"
  >("quick_win");
  ...
  const controlRef = useRef<{ paused: boolean; cancelled: boolean }>({
    paused: false,
    cancelled: false,
  });
  ...
  const loadJobs = useCallback(async () => {
    const { data } = await (supabase as any)
      .from("editorial_jobs")
      .select(
        "id,bucket,started_at,finished_at,duration_ms,tasks_total,tasks_ok,tasks_fail,ice_delta,ice_weighted_before,ice_weighted_after,status",
      )
      .eq("module", "glossary")
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const avg_editorial = list.length
        ? Math.round(list.reduce((s, r) => s + r.editorial_score, 0) / list.length)
        : 0;
      const avg_nexus = list.length
        ? Math.round(list.reduce((s, r) => s + r.nexus_score, 0) / list.length)
        : 0;
      const weightSum = list.reduce((s, r) => s + (r.doctrinal_weight || 1), 0) || 1;
      const avg_weighted = Math.round(
        list.reduce((s, r) => s + r.score * (r.doctrinal_weight || 1), 0) / weightSum,
      );
  ...
  const filtered = useMemo(() => {
    ...
    return rows
      .filter((r) => {
        if (q && !(r.term.toLowerCase().includes(q) || r.slug.toLowerCase().includes(q)))
          return false;
        ...
        switch (filter) {
          ...
          case "review":
            return tier === "review";
          case "no_deep":
            return !r.checks.find((c) => c.field === "deep_interpretation")!.ok;
          case "no_faq":
            return !r.checks.find((c) => c.field === "faq")!.ok;
          case "no_logos":
            return !r.checks.find((c) => c.field === "logos_meditation")!.ok;
          case "no_nexus":
            return r.nexus_count < GOLD_MIN;
          case "no_fathers":
            return !r.checks.find((c) => c.field === "fathers_refs")!.ok;
          case "no_mag":
            return !r.checks.find((c) => c.field === "magisterium_references")!.ok;
          default:
            return true;
        }
      })
  ...
  const generateBatch = useCallback(async () => {
    ...
    if (
      !confirm(
        `Gerar "${batchField}" para ${targets.length} verbete(s) via IA? Todos voltam para draft.`,
      )
    )
      return;
  ...
  useEffect(() => {
    try {
      ...
      if (
        ...
        c.results.length < c.tasks.length
      ) {
    ...
    async (
      ...
      if (!jobId) {
        try {
          ...
          console.warn("[editorial_jobs] insert falhou", e);
        }
      }
      ...
      for (let i = startIdx; i < tasks.length; i++) {
        ...
        const t = tasks[i];
        setBucketBatch(
          (b) => b && { ...b, current: t, done: i, paused: controlRef.current.paused },
        );
        const t0 = Date.now();
        try {
          const { data, error } = await supabase.functions.invoke("glossary-generate-deep", {
      ...
      try {
        ...
        if (g && g.length) {
          ...
          const w = Math.round(
            g.reduce(
              (s: number, r: any, i: number) => s + scores[i] * (r.doctrinal_weight || 1),
              0,
            ) / wSum,
          );
      ...
      if (jobId) {
        try {
          ...
          console.warn("[editorial_jobs] update falhou", e);
        }
      }
  ...
  const retryFailed = useCallback(async () => {
    ...
    const failed = bucketBatch.results
      .filter((r) => !r.ok)
  ...
  const resumeFromCheckpoint = useCallback(async () => {
    ...
    try {
      const c = JSON.parse(raw);
      await runQueue(c.tasks, c.label, {
        resumeResults: c.results,
        resumeJobId: c.jobId,
        skipConfirm: true,
      });
    } catch (e: any) {
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
          ...
          <code>draft</code> — revisão em{" "}
          <Link to="/admin/glossario" className="underline">
            /admin/glossario
          </Link>
          .
        </p>
        ...
        <>
          ...
          >
            <CardContent className="p-5">
              <div className="flex flex-wrap items-baseline justify-between gap-3">
                <div>
                  ...
                  <h2 className="font-serif text-2xl leading-tight">
                    {v1Certified
                      ? "Glossário Cathedra v1.0 — Certificado"
                      : "Linha de produção editorial"}
                  </h2>
                </div>
                {v1Certified && (
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3 lg:grid-cols-9">
                ...
                <Kpi
                  label="ICE ponderado"
                  value={`${totals.avg_weighted}%`}
                  tone={toneFor(totals.avg_weighted)}
                  hint="peso doutrinário"
                />
                <Kpi
                  label="Editorial"
                  value={`${totals.avg_editorial}%`}
                  tone={toneFor(totals.avg_editorial)}
                />
                <Kpi
                  label="Nexus"
                  value={`${totals.avg_nexus}%`}
                  tone={toneFor(totals.avg_nexus)}
                />
                <Kpi
                  label="Cobertura"
                  value={avgCoverage !== null ? `${avgCoverage}%` : "—"}
                  tone={avgCoverage !== null ? toneFor(avgCoverage) : "neutral"}
                />
                <Kpi
                  label="Gate"
                  value={
                    snapshot
                      ? `${snapshot.gate_passing}/${snapshot.gate_passing + snapshot.gate_failing}`
                      : "—"
                  }
                ...
                <Kpi
                  label="Quick wins"
                  value={buckets.quick_win}
                  tone={buckets.quick_win === 0 ? "ok" : "warn"}
                />
                <Kpi
                  label="Alto impacto"
                  value={buckets.red}
                  tone={buckets.red === 0 ? "ok" : "bad"}
                />
                <Kpi
                  label="Revisão"
          ...
          {(() => {
            ...
            const tier =
              ice_v >= 95
                ? {
                    label: "Ouro",
                    cls: "border-emerald-500/50 bg-gradient-to-br from-emerald-500/15 to-transparent text-emerald-800",
                  }
                : ice_v >= 85
                  ? {
                      label: "Prata",
                      cls: "border-sky-500/50 bg-gradient-to-br from-sky-500/15 to-transparent text-sky-800",
                    }
                  : ice_v >= 70
                    ? {
                        label: "Bronze",
                        cls: "border-amber-500/50 bg-gradient-to-br from-amber-500/15 to-transparent text-amber-800",
                      }
                    : {
                        label: "Revisão",
                        cls: "border-red-500/50 bg-gradient-to-br from-red-500/15 to-transparent text-red-800",
                      };
            const deltaIce =
              snapshot && prevSnapshot
                ? Number(snapshot.avg_ice) - Number(prevSnapshot.avg_ice)
                : null;
            return (
              <Card className={`mb-6 ${tier.cls}`}>
                <CardContent className="flex flex-wrap items-center gap-6 p-5">
                  ...
                  <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div>
                      <p className="opacity-70">ICE médio</p>
                      <p className="text-lg font-bold tabular-nums">
                        {ice_v.toFixed(1)}%
                        {deltaIce !== null && (
                          <span
                            className={`ml-1.5 text-[10px] ${deltaIce >= 0 ? "text-emerald-600" : "text-red-600"}`}
                          >
                    ...
                    <div>
                      <p className="opacity-70">Quality Gate</p>
                      <p className="text-lg font-bold tabular-nums">
                        {snapshot
                          ? `${snapshot.gate_passing}/${snapshot.gate_passing + snapshot.gate_failing}`
                          : "—"}
                    ...
                    <div>
                      <p className="opacity-70">Regressões</p>
                      <p className="text-lg font-bold tabular-nums flex items-center gap-1">
                        {snapshot?.regressions.length ?? 0}
                        {(snapshot?.regressions.length ?? 0) > 0 && (
                          <TrendingDown className="h-4 w-4 text-red-600" />
                        )}
                      </p>
                    </div>
                    <div>
                      <p className="opacity-70">Última auditoria</p>
                  ...
                  <CardContent className="pt-0">
                    <Collapsible>
                      <CollapsibleTrigger className="flex w-full items-center gap-1.5 rounded border border-red-500/30 bg-red-500/5 px-2 py-1.5 text-[11px] font-medium text-red-700 hover:bg-red-500/10">
                        ...
                        {snapshot.regressions.length} verbete(s) perderam qualidade desde a última
                        auditoria
          ...
          {(() => {
            ...
            const areaTier = (ice: number) =>
              ice >= 95
                ? { label: "Ouro", dot: "bg-emerald-500", cls: "text-emerald-700" }
                : ice >= 85
                  ? { label: "Prata", dot: "bg-sky-500", cls: "text-sky-700" }
                  : ice >= 70
                    ? { label: "Bronze", dot: "bg-amber-500", cls: "text-amber-700" }
                    : { label: "Revisão", dot: "bg-red-500", cls: "text-red-700" };
            const phaseB_done =
              coverage.length > 0 && coverage.every((c) => Number(c.avg_ice) >= 95);
            ...
            const certHash = allDone
              ? btoa(
                  `v1|${totals.total}|${totals.avg_weighted}|${nexusRelationCount}|${snapshot?.captured_at ?? ""}`,
                )
                  .replace(/[+/=]/g, "")
                  .slice(0, 16)
                  .toUpperCase()
            ...
            if (allDone) {
              const certDate = snapshot
                ? new Date(snapshot.captured_at).toLocaleDateString("pt-BR", {
                    day: "2-digit",
                    month: "2-digit",
                    year: "numeric",
                  })
                ...
                <Card className="mb-6 relative overflow-hidden border-2 border-amber-500/60 bg-gradient-to-br from-amber-50 via-yellow-50 to-amber-100 shadow-xl">
                  ...
                  <CardContent className="relative p-10 text-center">
                    ...
                    <p className="mt-1 text-xs uppercase tracking-widest text-amber-700">
                      Versão 1.0 · Certificada
                    </p>

                    <div className="mx-auto mt-6 grid max-w-2xl grid-cols-3 gap-6 border-y border-amber-500/30 py-4 text-amber-900">
                      <div>
                        <p className="text-[10px] uppercase opacity-70">ICE Final</p>
                        <p className="font-serif text-2xl tabular-nums">
                          {totals.avg_weighted.toFixed(1)}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase opacity-70">Verbetes</p>
                        <p className="font-serif text-2xl tabular-nums">{totals.total}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase opacity-70">Relações Nexus</p>
                        <p className="font-serif text-2xl tabular-nums">{nexusRelationCount}</p>
                      </div>
                    </div>

                    <p className="mt-5 text-xs text-amber-800">
                      Certificado em <b>{certDate}</b>
                    </p>
                    <p className="mt-1 font-mono text-[10px] tracking-widest text-amber-700/70">
                      HASH · {certHash}
                    </p>
                  </CardContent>
                </Card>
            ...
            const PhaseHeader = ({
              ...
              <div className="mb-2 flex items-center gap-2">
                ...
                >
                  {done ? <Lock className="h-3.5 w-3.5" /> : letter}
                </div>
                ...
                {done && (
                  <Badge
                    variant="outline"
                    className="border-emerald-500/50 bg-emerald-500/10 text-[10px] text-emerald-700"
                  >
                    🔒 Congelada
                  </Badge>
                )}
              ...
              <Card className="mb-6 border-l-4 border-l-primary">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    <PhaseHeader
                      letter="A"
                      title="Fase A · Fundação"
                      done={phaseA_done}
                      subtitle="Eliminar pendências críticas · 0 Quick Wins · 0 verbetes abaixo de ICE 70"
                    />
                    <div className="grid grid-cols-2 gap-3">
                      <div
                        className={`rounded border p-3 ${buckets.quick_win === 0 ? "border-emerald-500/40 bg-emerald-500/5" : "border-red-500/40 bg-red-500/5"}`}
                      >
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">
                          Quick Wins restantes
                        </p>
                        <p
                          className={`text-xl font-bold tabular-nums ${buckets.quick_win === 0 ? "text-emerald-700" : "text-red-700"}`}
                        >
                          {buckets.quick_win}
                        </p>
                      </div>
                      <div
                        className={`rounded border p-3 ${belowIce70 === 0 ? "border-emerald-500/40 bg-emerald-500/5" : "border-red-500/40 bg-red-500/5"}`}
                      >
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">
                          Verbetes ICE &lt; 70
                        </p>
                        <p
                          className={`text-xl font-bold tabular-nums ${belowIce70 === 0 ? "text-emerald-700" : "text-red-700"}`}
                        >
                          {belowIce70}
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                        {coverage.map((c) => {
                          ...
                          return (
                            <div
                              key={c.area}
                              className="flex items-center justify-between rounded border bg-muted/20 px-2.5 py-1.5"
                            >
                              ...
                              <span className={`text-[11px] font-semibold ${t.cls}`}>
                                {t.label} · {Number(c.avg_ice).toFixed(0)}
                              </span>
                            </div>
                          );
                        })}
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground tabular-nums">
                          {nexusGold} / {rows.length} verbetes
                        </span>
                        <span
                          className={`font-semibold tabular-nums ${phaseC_done ? "text-emerald-700" : "text-muted-foreground"}`}
                        >
                          {nexusPct}%
                        </span>
                      </div>
                      <div className="h-3 w-full overflow-hidden rounded-full bg-muted">
                        <div
                          className={`h-full transition-all ${phaseC_done ? "bg-emerald-500" : "bg-primary"}`}
                          style={{ width: `${nexusPct}%` }}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <CardHeader className="pb-3">
                    ...
                    {(() => {
                      ...
                      const fieldCount = tasks.reduce<Record<string, number>>((a, t) => {
                        a[t.field] = (a[t.field] ?? 0) + 1;
                        return a;
                      }, {});
                      const fieldLabels: Record<string, string> = {
                        deep_interpretation: "Interpretação",
                        faq: "FAQ",
                        logos_meditation: "Logos",
                        bible_verses: "Bíblia",
                        catechism_references: "CIC",
                        fathers_refs: "Patrística",
                      };
                      const running = bucketBatch?.running;
                      const pct =
                        bucketBatch && bucketBatch.total > 0
                          ? Math.round((bucketBatch.done / bucketBatch.total) * 100)
                          : 0;
                        ...
                        <div className="mt-3 rounded-lg border bg-muted/30 p-3">
                          ...
                          {!bucketBatch && resumable && (
                            <div className="mt-3 flex items-center justify-between gap-2 rounded border border-amber-500/40 bg-amber-500/10 p-2 text-xs">
                              <span>
                                ⚡ Checkpoint disponível — <b>{resumable.label}</b>:{" "}
                                {resumable.done}/{resumable.tasks.length} concluídas.
                              </span>
                                ...
                                <span className="tabular-nums">
                                  ...
                                  {bucketBatch.current && running && !bucketBatch.paused && (
                                    <span className="ml-2 text-muted-foreground">
                                      → {bucketBatch.current.term} ·{" "}
                                      {fieldLabels[bucketBatch.current.field] ??
                                        bucketBatch.current.field}
                                    </span>
                                  )}
                                ...
                                (() => {
                                  ...
                                  return (
                                    <div className="grid grid-cols-2 gap-2 rounded border bg-background/50 p-2 text-[10px] md:grid-cols-4">
                                      ...
                                      <div>
                                        <div className="text-muted-foreground">ICE antes</div>
                                        <div className="font-semibold tabular-nums">
                                          {iB?.weighted ?? "—"}
                                        </div>
                                      </div>
                                      <div>
                                        <div className="text-muted-foreground">ICE depois</div>
                                        <div className="font-semibold tabular-nums">
                                          {iA?.weighted ?? "—"}
                                        </div>
                                      </div>
                                      <div>
                                        ...
                                        <div
                                          className={`font-semibold tabular-nums ${delta > 0 ? "text-emerald-700" : delta < 0 ? "text-red-700" : ""}`}
                                        >
                                          {delta > 0 ? "+" : ""}
                                          {delta.toFixed(1)}
                                        </div>
                                ...
                                <div className="max-h-32 overflow-y-auto rounded border bg-background/50 p-2 text-[10px] font-mono">
                                  {bucketBatch.results.map((r, i) => (
                                    ...
                                    >
                                      ...
                                      {!r.ok && r.error && (
                                        <span className="ml-1 opacity-70">— {r.error}</span>
                                      )}
                                    </div>
                                  ))}
                                </div>
                              ...
                              <tr key={r.slug} className="hover:bg-muted/30">
                                <td className="px-3 py-2">
                                  <div className="flex items-center gap-1.5">
                                    <span
                                      className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[9px] font-semibold ${badge(r.priority)}`}
                                    >
                                ...
                                <td className="px-2 py-2 text-right tabular-nums text-muted-foreground">
                                  {r.inbound_refs}× {r.impact_tier === "high" && "🔴"}
                                  {r.impact_tier === "medium" && "🟠"}
                                </td>
                                <td className="px-2 py-2 text-[10px] text-muted-foreground">
                                  {missingLabels(r).length === 0
                                    ? "—"
                                    : missingLabels(r).join(" · ")}
                                </td>
              ...
              <CardContent className="p-0">
                <div className="max-h-64 overflow-y-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y">
                      {jobs.map((j) => {
                        const dur = j.duration_ms
                          ? `${Math.floor(j.duration_ms / 60000)}m${String(Math.floor((j.duration_ms % 60000) / 1000)).padStart(2, "0")}s`
                          : "—";
                        ...
                        return (
                          <tr key={j.id} className="hover:bg-muted/30">
                            ...
                            <td
                              className={`px-2 py-1.5 text-right tabular-nums font-semibold ${delta > 0 ? "text-emerald-700" : delta < 0 ? "text-red-700" : "text-muted-foreground"}`}
                            >
                              {delta > 0 ? "+" : ""}
          ...
          >
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
                <Snowflake
                  className={`h-5 w-5 ${frozen ? "text-sky-600" : "text-muted-foreground"}`}
                />
            ...
            <CardContent className="space-y-2">
              {modules.map((m) => {
                ...
                return (
                  <div
                    key={m.key}
                    className="grid grid-cols-[110px_1fr_90px] items-center gap-3 text-sm"
                  >
                    <span className="font-medium">{m.label}</span>
                    <Progress value={pct} className="h-2" />
                    <span className="tabular-nums text-xs text-muted-foreground text-right">
                      {m.published}/{m.total} · {pct}%{m.note && <span className="ml-1">·</span>}
                    </span>
                  </div>
                );
              })}
              <p className="pt-2 text-[11px] text-muted-foreground">
                Publicação como proxy de completude. Auditoria detalhada (score/ICE) hoje só para
                Glossário — Santos entra na Sprint 7 após o selo de Congelamento.
              </p>
            </CardContent>
          </Card>
                  ...
                  <CardHeader className="flex flex-row items-start justify-between gap-4 pb-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1.5 text-[11px] text-muted-foreground">
                        Global <b className="tabular-nums">{r.score}%</b> · {r.pending} pendência(s)
                        geráveis
                      </p>
                    </div>
                    <Link
                            ...
                            <CollapsibleContent className="mt-1.5 rounded border bg-muted/20 p-2 text-[11px]">
                              {deps.length === 0 ? (
                                <p className="text-muted-foreground">
                                  Sem dependências mapeadas em <code>nexus_refs</code>.
                                </p>
                              ) : (
                      ...
                      <Collapsible>
                        ...
                        <CollapsibleContent className="mt-1.5 rounded border bg-muted/20 p-2 text-[11px] text-muted-foreground">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                            ...
                            <span>✏️ Última edição</span>
                            <span className="tabular-nums">{fmtDate(r.updated_at)}</span>
                            <span>👤 Revisão humana</span>
                            <span className="tabular-nums">
                              {r.reviewed_at
                                ? `${fmtDate(r.reviewed_at)}${r.reviewed_by ? ` · ${r.reviewed_by.slice(0, 8)}` : ""}`
                                : "—"}
                            </span>
                            <span>📖 Publicado</span>
                            <span className="tabular-nums">{fmtDate(r.published_at)}</span>
                            <span>🔢 Versão</span>
                            <span className="tabular-nums">v{r.version ?? 1}</span>
                          </div>
                        </CollapsibleContent>
                      </Collapsible>
                    </div>
