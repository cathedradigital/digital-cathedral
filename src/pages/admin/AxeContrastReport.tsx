    ...
    <div className="p-spacing-lg space-y-spacing-lg">
      <header className="space-y-spacing-2xs">
        ...
        <p className="text-muted-foreground text-sm">
          ...
          {summary.totals.routes} rotas ·{" "}
          <strong className="text-foreground">{summary.totals.nodes}</strong> nó(s) ·{" "}
          <strong className="text-foreground">{summary.totals.enforcedFailing}</strong> enforced
          falhando · <strong className="text-foreground">{summary.totals.trackedDirty}</strong>{" "}
          tracked com violações ·{" "}
          <strong className="text-primary">{summary.totals.trackedCleaned}</strong> prontas para
          promoção
        </p>
                    ...
                    <tbody>
                      {diffRows.map((d) => (
                        <tr key={d.route} className="border-t border-border">
                          ...
                          <td
                            className={`p-2 text-right font-semibold ${d.delta < 0 ? "text-primary" : d.delta > 0 ? "text-destructive" : "text-muted-foreground"}`}
                          >
                      ...
                      {classDiff.length === 0 ? (
                        ...
                        classDiff.map((c) => (
                          <tr key={c.cls} className="border-t border-border">
                            ...
                            <td
                              className={`p-2 text-right font-semibold ${c.delta < 0 ? "text-primary" : "text-destructive"}`}
                            >
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Escolha como você quer aplicar a mudança. Clique nas palavras destacadas para abrir a
        ...
        <div className="space-y-spacing-sm text-sm">
          ...
          <div className="border border-border rounded p-spacing-sm space-y-1">
            <button
              onClick={() =>
                openDoc("opção-1--pr-automático-recomendada-quando-você-tem-muitas-rotas")
              }
            ...
            <p className="text-muted-foreground text-xs">
              ...
              <strong>Requer configuração inicial</strong> (um token do GitHub como segredo). Se
              ainda não estiver configurado, o botão apenas explica.
            </p>
          </div>
          ...
          <div className="border border-border rounded p-spacing-sm space-y-1">
            ...
            <p className="text-muted-foreground text-xs">
              Você baixa um arquivo <code>.patch</code> e aplica com <code>git apply</code>. Não
              precisa de token nem de configuração.
            </p>
            {route && (
              <a
          ...
          <div className="border border-border rounded p-spacing-sm space-y-1">
            <button
              onClick={() =>
                openDoc("opção-3--comandoedição-direta-para-quem-prefere-fazer-na-mão")
              }
              className="font-semibold text-foreground hover:text-primary text-left w-full"
            >
            ...
            <p className="text-muted-foreground text-xs">
              Abra <code>tests/e2e/axe-color-contrast-regression.spec.ts</code> e mova a linha entre
              os arrays. Total controle.
            </p>
          </div>
        </div>
