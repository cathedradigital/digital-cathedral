...
import { useLocation, Link } from "@/lib/rr-compat";
...
export default function InfrastructureDiagnosticsPage() {
  ...
  const loadHistory = async () => {
    let query = supabase
      .from("infrastructure_audit_runs" as any)
      .select("*")
      .order("created_at", { ascending: false });
  ...
  const runAudit = async () => {
    ...
    const { error } = await (supabase.from("infrastructure_audit_runs" as any) as any).insert([
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8 pb-32">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" id="main-title">
            Cathedra Mission Control
          </h1>
          <h2 className="text-muted-foreground">
            Audit 7.7.1A — Diagnostics & Multi-Language Registry
          </h2>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={runAudit} disabled={loading} className="gap-2">
            {loading ? (
              <Icons.Cross className="w-4 h-4 animate-spin" />
            ) : (
      ...
      <Tabs defaultValue="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="premium-card">
              <CardHeader className="pb-2">
                <CardTitle className="text-premium-xs font-black uppercase tracking-widest text-muted-foreground">
                  Estado do Idioma
                </CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <Badge
                            ...
                            className="rounded-premium-full uppercase text-[9px] tracking-widest"
                          >
                            {run.status}
                          </Badge>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {run.details?.areas?.map((area: any) => (
                          <div
                            key={area.area}
                            className="flex items-center gap-2 text-[10px] uppercase tracking-tighter"
                          >
                            <div
                              className={cn(
                                "w-1.5 h-1.5 rounded-full",
                                area.status === "PASS" ? "bg-green-500" : "bg-red-500",
                              )}
                            />
                            ...
                            <span
                              className={cn(
                                "font-bold",
                                area.status === "PASS" ? "text-green-600" : "text-red-600",
                              )}
                            >
                              {area.status}
                            </span>
                  ...
                  <AreaChart
                    data={useMemo(() => {
                      ...
                      return [...history]
                        ...
                        .slice(-10)
                        .map((run) => {
                          const date = new Date(run.created_at).toLocaleDateString("pt-BR", {
                            day: "2-digit",
                            month: "2-digit",
                          });
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-premium text-center">
                  ...
                  <div className="text-[10px] uppercase font-black tracking-widest text-green-600/70">
                    Pass
                  </div>
                </div>
                <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-premium text-center">
                  <div className="text-2xl font-bold text-amber-600">1</div>
                  <div className="text-[10px] uppercase font-black tracking-widest text-amber-600/70">
                    Warning
                  </div>
                </div>
                <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-premium text-center">
                  <div className="text-2xl font-bold text-red-600">1</div>
                  <div className="text-[10px] uppercase font-black tracking-widest text-red-600/70">
                    Fail
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between px-1">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
                    Críticos (Fail/Warning)
                  </h3>
                  <span className="text-[9px] text-muted-foreground italic">
                    Relatório de 19/08/2026
                  </span>
                </div>
                <div className="space-y-2">
                  <div className="p-3 rounded-premium border border-red-500/30 bg-red-500/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-xs uppercase tracking-wider text-red-700">
                          Glossário no Sitemap
                        </h4>
                        <p className="text-[10px] text-red-600/80">
                          42 verbetes publicados não encontrados no sitemap.xml
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-amber-600/80">
                          165 caracteres (limite recomendado: 160)
                        </p>
                      </div>
                    </div>
                    <Badge
                      variant="outline"
                      className="text-[9px] uppercase text-amber-600 border-amber-600"
                    >
                      P1 · WARN
                    </Badge>
                  </div>
              ...
              <div className="pt-4 border-t border-border/40">
                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-1 mb-3">
                  Auditoria de Regressão (CI Guardrail)
                </h3>
                  ...
                  return (
                    <Card key={metric} className="bg-muted/10 border-border/20">
                      <CardContent className="pt-6">
                        <div className="text-premium-xs font-black uppercase tracking-widest text-muted-foreground mb-1">
                          {metric} (AVG)
                        </div>
                ...
                <div className="space-y-2">
                  {webVitals.length > 0 ? (
                    webVitals.map((v) => (
                      <div
                        key={v.id}
                        className="p-3 rounded-premium border border-border/40 bg-muted/20 flex items-center justify-between text-xs"
                      >
                        <div className="flex items-center gap-3">
              ...
              <div className="bg-muted/20 border border-border/40 rounded-premium p-4 flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Acompanhe reversões, alterações de metadados e logs de governança.
                  </p>
                </div>
                <Button
                  asChild
                  size="sm"
                  className="rounded-premium-full h-8 px-6 uppercase text-[10px] font-black tracking-widest"
                >
                    ...
                    {selectedAudit.details?.areas?.map((area: any) => (
                      <div key={area.area} className="flex flex-col gap-1">
                        <div className="flex justify-between items-center">
                          ...
                          <Badge
                            variant={area.status === "PASS" ? "outline" : "destructive"}
                            className="text-[9px]"
                          >
                            {area.status}
                          </Badge>
                        ...
                        >
                          <Link
                            to={
                              selectedAudit.details.areas.find((a: any) => a.area === area.area)
                                ?.route || "#"
                            }
                          >
                            Ir para {area.area}
        ...
        <TabsContent value="report" className="space-y-6 outline-none">
          <Card className="premium-card">
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="text-premium-base font-black uppercase tracking-widest">
                Relatório de Auditoria 7.7.1C
              </CardTitle>
              <Badge variant="outline" className="font-mono text-[10px]">
                VERIFICAÇÃO: EVIDÊNCIA_E2E_OK
              </Badge>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <Card className="bg-muted/10 border-border/20">
                  ...
                  <CardContent className="py-2 px-4 space-y-3">
                    <p className="text-[10px] text-muted-foreground">
                      Documentação técnica das políticas de segurança aplicadas em 39 tabelas.
                    </p>
                    <Button
                      asChild
                      variant="outline"
                      size="sm"
                      className="w-full h-7 text-[9px] uppercase tracking-widest"
                    >
                ...
                <Card className="bg-muted/10 border-border/20">
                  ...
                  <CardContent className="py-2 px-4 space-y-3">
                    <p className="text-[10px] text-muted-foreground">
                      Métricas de acessibilidade (axe) e Web Vitals históricos.
                    </p>
                    <Button
                      asChild
                      variant="outline"
              ...
              <div className="space-y-4">
                ...
                <h3
                  className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-1"
                  id="status-area-title"
                >
                  Status por Área (Real-Time)
                </h3>
                  ...
                  {[
                    {
                      ...
                      status: "PASS",
                      route: "/santos",
                      action: "Navegação editorial ativa",
                    },
                    {
                      area: "Catecismo",
                      status: "PASS",
                      route: "/catecismo",
                      action: "Sincronização Nexus OK",
                    },
                    {
                      area: "Bíblia",
                      status: "PASS",
                      route: "/biblia",
                      action: "Cobertura 73 livros OK",
                    },
                    {
                      area: "Nexus",
                      status: "PASS",
                      route: "/nexus",
                      action: "Processamento dinâmico OK",
                    },
                  ].map((area) => (
                    <div
                      key={area.area}
                      className="p-3 rounded-premium border border-border/40 bg-muted/20 flex items-center justify-between gap-4"
                    >
                      <div className="flex items-center gap-3">
                  ...
                  {[
                    {
                      ...
                      action: "Aguardando semente de dados",
                      recommended: "Executar SaintWorks Seed",
                    },
                    {
                      area: "Liturgia",
                      status: "FAIL",
                      route: "/liturgia",
                      action: "Erro de renderização em mobile",
                      recommended: "Revisar tokens Harmony",
                    },
                  ].map((area) => (
                    <div
                      key={area.area}
                      className="p-3 rounded-premium border border-border/40 bg-muted/20 flex items-center justify-between gap-4 opacity-75"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={cn(
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Badge
                          variant={area.status === "FAIL" ? "destructive" : "outline"}
                ...
                <div className="space-y-4">
                  {backendErrors.length > 0 ? (
                    backendErrors.map((err) => (
                      <div
                        key={err.id}
                        className="p-4 rounded-premium border border-border/40 bg-muted/20 space-y-2"
                      >
                        <div className="flex justify-between items-start">
                          <Badge
                            variant="outline"
                            className="font-mono text-[10px] uppercase text-amber-600"
                          >
                            {err.module}
                          </Badge>
