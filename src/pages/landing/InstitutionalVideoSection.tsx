...
const InstitutionalVideoSection = () => {
  ...
  const trackEvent = useCallback(
    async (eventName: string, props: Record<string, any> = {}) => {
      try {
        const {
          data: { user },
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full py-spacing-4xl md:py-spacing-4xl bg-background overflow-hidden"
    ...
    >
      ...
      <div className="absolute inset-0 z-0 bg-background/50" aria-hidden="true">
        <motion.div
          ...
          className="absolute top-spacing-0 right-0 w-[600px] h-[600px] bg-primary/2 rounded-premium"
        />
        <motion.div
          style={{ y: shouldReduceMotion ? 0 : y2 }}
          className="absolute bottom-spacing-0 left-0 w-[500px] h-[500px] bg-secondary/2 rounded-premium"
        />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-spacing-3xl items-center">
          ...
          >
            <div className="space-y-spacing-md">
              <span className="text-premium-xs font-bold uppercase tracking-[0.4em] text-primary/40 block italic">
                Apresentação
              </span>
          ...
          >
            ...
            >
              <div className="w-spacing-xl h-spacing-xl rounded-premium bg-primary/10 flex items-center justify-center">
                {isMuted ? (
                  <Icons.VolumeX className="w-spacing-md h-spacing-md text-primary" />
                ) : (
                  <Icons.Volume2 className="w-spacing-md h-spacing-md text-primary" />
                )}
              </div>
              ...
              <div className="absolute top-spacing-0 left-0 right-0 p-spacing-lg flex justify-between items-center z-50 bg-black/60">
                <div className="flex items-center gap-spacing-md">
                  <div className="flex items-center gap-spacing-xs px-spacing-sm py-spacing-2xs bg-white/10 rounded-premium border border-white/20">
                    ...
                    >
                      {SUBTITLES.map((s) => (
                        <option key={s.lang} value={s.lang} className="bg-black">
                          {s.label}
                        </option>
                      ))}
                    </select>
                  ...
                  >
                    {isMuted ? (
                      <Icons.VolumeX className="w-spacing-md h-spacing-md text-white" />
                    ) : (
                      <Icons.Volume2 className="w-spacing-md h-spacing-md text-white" />
                    )}
                  </Button>
                </div>
