...
const registerBenefits = [
  {
    ...
    description:
      "Sincronize seu plano de leitura bíblica e estudos teológicos em todos os seus dispositivos.",
    icon: <Icons.Bookmark className="h-spacing-md w-spacing-md text-primary" />,
  },
  {
    title: "Tesouros Pessoais",
    description:
      "Guarde versículos, orações e meditações que tocam seu coração para consultas rápidas.",
    icon: <Icons.Star className="h-spacing-md w-spacing-md text-primary" />,
  },
  {
    title: "Constância",
    description:
      "Acompanhe sua jornada com metas de estudo, insígnias e lembretes para manter o hábito espiritual.",
...
const BenefitsSection = ({ onLogin }: BenefitsSectionProps) => {
  ...
  return (
    <section
      ref={sectionRef}
      className="w-full bg-foreground text-background py-spacing-4xl px-spacing-lg overflow-hidden relative"
    >
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-spacing-3xl items-center relative z-10">
        <div className="space-y-spacing-xl">
          ...
          >
            <h2 className="text-premium-4xl md:text-premium-5xl font-display font-bold leading-tight">
              Sua morada espiritual no{" "}
              <span className="text-primary italic font-serif">Cathedra</span>
            </h2>
            <p className="text-premium-lg opacity-70 leading-relaxed max-w-spacing-xl">
              Criar sua conta permite uma experiência única de crescimento, guardando sua história
              viva com a Palavra de Deus e o Magistério.
            </p>
          </motion.div>
            ...
            {registerBenefits.map((benefit, i) => (
              ...
              >
                <motion.div
                  ...
                  viewport={{ once: true }}
                  custom={i}
                  className="space-y-spacing-sm p-spacing-lg rounded-premium bg-card/50 border border-white/10 cursor-pointer h-full"
                >
                  <motion.div
                    whileHover={{ rotate: 15, scale: 1.15 }}
                    transition={{ type: "spring", stiffness: 400 }}
                    className="w-spacing-xl h-spacing-xl rounded-premium bg-primary/20 flex items-center justify-center"
                  >
                    {benefit.icon}
                  </motion.div>
                  <h3 className="font-bold text-premium-lg">{benefit.title}</h3>
                  <p className="text-premium-sm opacity-50 leading-relaxed">
                    {benefit.description}
                  </p>
                </motion.div>
              </motion.div>
          ...
          <motion.div variants={buttonHover} initial="rest" whileHover="hover" whileTap="tap">
            <HomeButton
              size="lg"
              className="h-spacing-3xl px-spacing-2xl rounded-premium-full bg-primary text-primary-foreground font-black uppercase tracking-widest w-full sm:w-auto"
              onClick={() => {
        ...
        <div className="relative order-first lg:order-last">
          <motion.div
            ...
            className="relative aspect-[4/3] lg:aspect-square rounded-[2rem] lg:rounded-[3rem] overflow-hidden border border-white/20 shadow-premium"
          >
            <motion.img
            ...
            <div className="absolute bottom-spacing-xl left-spacing-xl right-spacing-xl p-spacing-xl bg-black/60 rounded-premium border border-white/20">
              <p className="text-premium-xl font-serif mb-spacing-md">
