...
export default function BibleImportAdmin() {
  ...
  useEffect(() => {
    try {
      ...
      if (parsed?.steps && parsed?.logs) {
        ...
        setLogs(parsed.logs);
        setActiveJobId(parsed.jobId ?? null);
      }
    } catch {
      /* ignore */
    }
  ...
  const persistRun = useCallback((s: StepState[], l: LogEntry[], jobId: string | null) => {
    try {
      localStorage.setItem(LOGS_STORAGE_KEY, JSON.stringify({ steps: s, logs: l, jobId }));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  function downloadRejectedFromPreview() {
    ...
    downloadBlob(
      `preview-rejected-${file?.name ?? "dump"}.ndjson`,
      rejectedToNDJSON(preview.rejected),
    );
  }
  ...
  async function downloadRejectedFromJob(job: ImportJob) {
    ...
    if (!path) {
      toast.error("Job sem arquivo de rejeitados.");
      return;
    }
    const { data, error } = await supabase.storage.from("bible-dumps").createSignedUrl(path, 300);
    if (error || !data?.signedUrl) {
      toast.error(`URL: ${error?.message ?? "falhou"}`);
      return;
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (file) {
        ...
        await supabase
          .from("bible_translation_sources")
          .update({ file_url: `storage://bible-dumps/${path}` })
          .eq("id", sourceId);
      ...
      if (file && rawMode && filePath) {
        ...
        const r = cvData.stats?.rejected_count ?? 0;
          ...
          "conversion",
          `${v.toLocaleString("pt-BR")} versos válidos · ${r} rejeitados`,
          r > 0 ? "warn" : "success",
        );
      } else {
        skip(
          "conversion",
          file ? "NDJSON canônico — conversão desnecessária" : "Sem arquivo local",
        );
      }
  ...
  async function retryImport(s: TranslationSource) {
    if (!s.file_url) {
      toast.error("Esta fonte não tem arquivo registrado");
      return;
    }
    ...
    <div className="container mx-auto py-spacing-xl space-y-spacing-lg max-w-6xl">
      <header>
        ...
        <p className="text-muted-foreground mt-spacing-xs">
          ...
          <code className="mx-spacing-xs px-spacing-xs py-spacing-0.5 bg-muted rounded text-xs">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Metadados de licença e atribuição são obrigatórios — não importamos texto sem licença
            declarada.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid gap-spacing-md md:grid-cols-2">
            ...
            <div className="space-y-spacing-xs">
              ...
              <Input
                ...
                placeholder="ex.: Almeida Corrigida Fiel"
                required
              />
            </div>
            <div className="space-y-spacing-xs">
            ...
            <div className="space-y-spacing-xs md:col-span-2">
              ...
              <Input
                ...
                placeholder="ex.: Domínio público / CC BY 4.0 / Licença Editorial XYZ"
                required
              />
            </div>
            <div className="space-y-spacing-xs md:col-span-2">
              ...
              <Input
                ...
                placeholder='ex.: "Bíblia Sagrada — Ave-Maria, 1959 (uso autorizado)"'
                required
              />
            </div>
            <div className="space-y-spacing-xs md:col-span-2">
            ...
            <div className="space-y-spacing-xs">
              ...
              <Input
                id="file"
                type="file"
                accept=".ndjson,.jsonl,.json,.csv,.tsv"
                ref={fileInputRef}
            ...
            <div className="space-y-spacing-xs">
              ...
              <Input
                ...
                placeholder="https://exemplo.com/dump.ndjson"
                disabled={!!file}
              />
            </div>
                ...
                <div className="px-spacing-md py-spacing-sm border-b flex items-center justify-between">
                  ...
                  {preview && preview.rejected.length > 0 && (
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={downloadRejectedFromPreview}
                    >
                ...
                <div className="p-spacing-md space-y-spacing-sm">
                  {previewTruncated && (
                    <p className="text-xs text-amber-600 flex items-center gap-spacing-xs">
                      ...
                      Arquivo grande — preview limitado aos primeiros 5 MB. As contagens são
                      parciais.
                    </p>
                  )}
                  {preview ? (
                    <>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-spacing-sm text-sm">
                        <Stat
                          label="Versos válidos"
                          value={preview.validVerses.toLocaleString("pt-BR")}
                        />
                        <Stat
                          label="Rejeitados"
                          value={preview.rejectedCount.toLocaleString("pt-BR")}
                          tone={preview.rejectedCount > 0 ? "warn" : "ok"}
                        />
                        <Stat
                          label="Livros únicos"
                          value={`${preview.uniqueBooks}/73`}
                          tone={
                            preview.uniqueBooks === 73
                              ? "ok"
                              : preview.uniqueBooks === 0
                                ? "bad"
                                : "warn"
                          }
                        />
                        <Stat
                          label="Capítulos"
                          value={preview.uniqueChapters.toLocaleString("pt-BR")}
                        />
                      </div>
                          ...
                          <div className="mt-spacing-xs max-h-48 overflow-auto border rounded">
                            <table className="w-full text-xs">
                              <thead className="bg-muted/50 sticky top-0">
                                <tr>
                                  <th className="text-left px-spacing-xs py-spacing-xs">Abbr</th>
                                  <th className="text-right px-spacing-xs py-spacing-xs">
                                    Capítulos
                                  </th>
                                  <th className="text-right px-spacing-xs py-spacing-xs">Versos</th>
                                </tr>
                              </thead>
                              <tbody>
                                {preview.byBook.map((b) => (
                                  <tr key={b.abbr} className="border-t">
                                    <td className="px-spacing-xs py-spacing-xs font-mono">
                                      {b.abbr}
                                    </td>
                                    <td className="px-spacing-xs py-spacing-xs text-right">
                                      {b.chapters}
                                    </td>
                                    <td className="px-spacing-xs py-spacing-xs text-right">
                                      {b.verses}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {sources.length} fonte(s). O progresso atualiza automaticamente enquanto a importação
            roda.
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  return (
                    <TableRow key={s.id}>
                      <TableCell className="font-mono text-xs">
                        {s.code}
                        {s.is_primary && (
                          <Badge variant="outline" className="ml-spacing-xs">
                            primária
                          </Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {s.language} · {s.attribution}
                        </div>
                      </TableCell>
                      <TableCell className="text-xs max-w-[200px] truncate" title={s.license}>
                        {s.license}
                      </TableCell>
                      <TableCell>{sourceStatusBadge(s.status)}</TableCell>
                      <TableCell className="text-xs">
                        {s.books_count} / {s.chapters_count} /{" "}
                        {s.verses_count.toLocaleString("pt-BR")}
                      </TableCell>
                      <TableCell className="text-xs min-w-[200px]">
                        {job ? (
                          <div className="space-y-spacing-xs">
                            ...
                            {job.message && (
                              <div className="text-muted-foreground truncate" title={job.message}>
                                {job.message}
                              </div>
                            )}
                            {job.error && (
                              <div className="text-destructive truncate" title={job.error}>
                                {job.error}
                              </div>
                            )}
                            {job.verification && !job.verification.skipped && (
                              <div className="flex items-center gap-spacing-xs">
                                {job.verification.passed ? (
                                  <>
                                    <ShieldCheck className="h-3 w-3 text-emerald-600" /> Cobertura
                                    OK
                                  </>
                                ) : (
                                  <>
                            ...
                            {job.verification?.rejected_path &&
                              (job.verification?.rejected_count ?? 0) > 0 && (
                                <Button
                                  type="button"
                                  size="sm"
                                  variant="ghost"
...
function ImportProgressPanel({
  ...
  const copyLogs = () => {
    const txt = logs
      .map(
        (l) =>
          `[${fmtTime(l.ts)}] ${l.level.toUpperCase()}${l.step ? ` (${l.step})` : ""}: ${l.message}`,
      )
      .join("\n");
        ...
        <div>
          ...
          <CardDescription>
            ...
            {activeJobId && (
              <>
                {" "}
                · job <code className="text-xs">{activeJobId}</code>
              </>
            )}
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-spacing-md">
        ...
        <ol className="space-y-spacing-xs">
          {steps.map((s, i) => (
            <li
              key={s.key}
              className="flex items-start gap-spacing-sm rounded-md border p-spacing-sm bg-card"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {s.detail && (
                  <p
                    className={`text-xs mt-spacing-xs break-words ${
                      s.status === "error"
                        ? "text-destructive"
                        : s.status === "skipped"
            ...
            {logs.length === 0 ? (
              ...
              <ul>
                {logs.map((l, i) => (
                  <li
                    key={i}
                    className={`px-spacing-sm py-spacing-xs border-b last:border-0 flex gap-spacing-xs ${
                      l.level === "error"
                        ? "text-destructive"
                        : l.level === "warn"
                          ? "text-amber-700"
