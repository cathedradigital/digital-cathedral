...
import type { LibraryFilter, LibraryItem, LibrarySearchResult } from "@/types/library";
 ...
 */
export async function searchLibrary(filter: LibraryFilter = {}): Promise<LibrarySearchResult> {
...
export async function countLibraryByKind(): Promise<Record<string, number>> {
  const { data, error } = await supabase.from("library_items_v1").select("library_kind");
