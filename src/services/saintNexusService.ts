...
export async function getSaintRelations(saintId: string): Promise<SaintRelationGroups> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, relation_type, source_kind, source_ref, target_kind, target_ref, attributed_to, note, confidence",
    )
...
export async function getSaintCuratedEdges(saintId: string): Promise<CuratedNexusEdge[]> {
  ...
  const [outgoing, incoming] = await Promise.all([
    ...
    supabase
      .from("nexus_relations")
      .select(cols)
      .eq("target_kind", "saint")
      .filter("target_ref->>id", "eq", saintId)
      .limit(120),
  ]);
  ...
  for (const r of (outgoing.data ?? []) as unknown as Row[])
    push(r.target_kind, r.target_ref, r.note);
  for (const r of (incoming.data ?? []) as unknown as Row[])
    push(r.source_kind, r.source_ref, r.note);
