...
import type { SaintWork, SaintWorkChapter, SaintWorkCategory } from "@/types/saintWorks";
...
export async function listWorksByCategory(category: SaintWorkCategory): Promise<SaintWork[]> {
  const { data, error } = await supabase
    .from(WORKS_TABLE)
    .select("*")
...
export async function getChapter(workId: string, order: number): Promise<SaintWorkChapter | null> {
  const { data, error } = await supabase
    .from(CHAPTERS_TABLE)
    .select("*")
    .eq("work_id", workId)
...
export async function searchPatristicLibrary(
  ...
  )("search_patristic_library", { search_query: q, page_number: page, page_size: pageSize });
  if (error) {
